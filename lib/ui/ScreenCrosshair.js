...
export function createScreenCrosshair(scene) {
  ...
  function update(opts) {
    ...
    const standardFade =
      (standardOnly ? 1 : THREE.MathUtils.clamp(1 - t / 0.48, 0, 1)) *
      (1 - specialCover);
    ...
    const reticuleFade =
      THREE.MathUtils.clamp((t - 0.42) / 0.18, 0, 1) * (1 - specialCover);
    ...
    const offsetX = pixelsToWorldSize(
      opts.tuning.gunOffsetX ?? 0,
      opts.camera,
      opts.canvasHeight,
    );
    const offsetY = pixelsToWorldSize(
      opts.tuning.gunOffsetY ?? 0,
      opts.camera,
      opts.canvasHeight,
    );
    reticuleMesh.scale.set(worldW, worldH, 1);
    reticuleMesh.position.set(offsetX, offsetY, CROSSHAIR_VIEW_Z);
