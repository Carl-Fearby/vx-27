...
function spawnControlPanels(root, arena) {
  const floorY = arena.floorY ?? 0;
  const panels = [];
  for (const def of arena.props ?? []) {
    if (def.type !== "controlPanel") continue;
    const y = def.y ?? def.floorY ?? floorY;
    const roomId = resolveControlPanelRoomId(def, arena);
    ...
    panels.push(
      createControlPanel(root, def.x, def.z, y, def.rotationY ?? 0, {
        height: def.height,
        depth: def.depth,
        width: def.panelWidth,
        shelteredOutdoor,
        propDef: roomId ? { ...def, roomId } : def,
      }),
    );
  }
...
export function spawnLevelProps(group, arena, colliders) {
  ...
  const controlPanels = spawnControlPanels(group, arena);
