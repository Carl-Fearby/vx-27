...
import { WALL_VISUAL_FLOOR_EMBED } from "../level/LevelConstants.js";

/** Baked service-room arch doorway sill (world Y metres). */
export const SERVICE_ROOM_ARCH_DOOR_SILL_Y = -WALL_VISUAL_FLOOR_EMBED;
import {
  getArenaDoorInnerZ,
  getArenaWallOuterZ,
  subtractXInterval,
...
/**
 * Cut doorway holes in an extruded wall shape.
 * Flat doors use {@link WALL_VISUAL_FLOOR_EMBED}; arched doors use {@link SERVICE_ROOM_ARCH_DOOR_SILL_Y}.
 *
 * @param {THREE.Shape} shape
 * @param {DoorOpening[]} openings
 * @param {number} spanLeft
 * @param {number} spanRight
 * @param {number} [flatFloorLine]
 */
export function appendDoorwayHoles(
  shape,
  openings,
  spanLeft,
  spanRight,
  flatFloorLine = -WALL_VISUAL_FLOOR_EMBED
) {
  const lip = DOORWAY_JAMB_SOLID_OVERLAP;
  const archFloorLine = SERVICE_ROOM_ARCH_DOOR_SILL_Y;

  for (const op of openings) {
    if (op.right <= spanLeft + 0.01 || op.left >= spanRight - 0.01) continue;
    const cutLeft = op.left + lip;
    const cutRight = op.right - lip;
    const sill = op.arch ? archFloorLine : flatFloorLine;
    const hole = new THREE.Path();

    if (op.arch) {
      if (sill >= op.rectTop - 0.02) continue;
      hole.moveTo(cutLeft, sill);
      hole.lineTo(cutLeft, op.rectTop);
      hole.absarc(
        op.centerX,
        op.rectTop,
      ...
      hole.lineTo(cutRight, sill);
      hole.closePath();
    } else {
      hole.moveTo(cutLeft, sill);
      hole.lineTo(cutLeft, op.height);
      hole.lineTo(cutRight, op.height);
      hole.lineTo(cutRight, sill);
      hole.closePath();
    }
...
/** Narrow doorway holes so solid jambs overlap opening edges (avoids vertical light gaps). */
export const DOORWAY_JAMB_SOLID_OVERLAP = 0.035;
...
/** Arena / room floor — above this, doorway headroom and mouth-plane skips do not apply. */
export const DOORWAY_HEADROOM_FOOT_Y_SLACK = 2.5;
...
/**
 * Mouth-wall lintels are vertical door-wall slices (attach-wall plane), not overhead
 * soffits. Capsule overlap falsely forces crouch anywhere the radius crosses that plane.
 * Doorway passages still use {@link getDoorwayHeadroomCeilingY} first in hasHeadroom.
 *
 * @param {{ mouthPlane?: boolean }} box
 * @param {number} footY
 * @param {number} floorY
 * @param {boolean} _inRoom
 * @param {boolean} inPassage
 */
export function shouldSkipMouthPlaneHeadroom(
  box,
  footY,
  floorY,
  _inRoom,
  ...
  if (!box.mouthPlane) return false;
  if (footY >= floorY + DOORWAY_HEADROOM_FOOT_Y_SLACK) return false;
  if (inPassage) return false;
  return true;
}
