...
import { pointInRoundedBoxFootprint } from "./Collision.js";
import { WALL_STANDOFF, WALL_VISUAL_FLOOR_EMBED } from "./LevelConstants.js";
...
/** @param {import("./Collision.js").ColliderBox} box */
...
/** @param {import("./Collision.js").ColliderBox} box @param {number} edgeR @param {number} cornerR */
...
const SET_FILES = {
  side: {
    albedo: "vx27_container_side_albedo.png",
    normal: "vx27_container_side_normal.png",
    roughness: "vx27_container_side_roughness.png",
    metallic: "vx27_container_side_metallic.png",
    ao: "vx27_container_side_ao.png",
    emissive: "vx27_container_side_emissive_mask.png",
  },
  inside_wall: {
    albedo: "vx27_container_inside_wall_albedo.png",
    normal: "vx27_container_inside_wall_normal.png",
    roughness: "vx27_container_inside_wall_roughness.png",
    metallic: "vx27_container_inside_wall_metallic.png",
    ao: "vx27_container_inside_wall_ao.png",
    emissive: "vx27_container_inside_wall_emissive_mask.png",
  },
  top_bottom: {
    albedo: "vx27_container_top_bottom_albedo.png",
    normal: "vx27_container_top_bottom_normal.png",
    roughness: "vx27_container_top_bottom_roughness.png",
    metallic: "vx27_container_top_bottom_metallic.png",
    ao: "vx27_container_top_bottom_ao.png",
    emissive: "vx27_container_top_bottom_emissive_mask.png",
  },
  corner_arc: {
    albedo: "vx27_container_corner_arc_albedo.png",
    normal: "vx27_container_corner_arc_normal.png",
    roughness: "vx27_container_corner_arc_roughness.png",
    metallic: "vx27_container_corner_arc_metallic.png",
    ao: "vx27_container_corner_arc_ao.png",
  },
  endcap_square: {
    albedo: "vx27_container_endcap_square_albedo.png",
    normal: "vx27_container_endcap_square_normal.png",
    roughness: "vx27_container_endcap_square_roughness.png",
    metallic: "vx27_container_endcap_square_metallic.png",
    ao: "vx27_container_endcap_square_ao.png",
    alpha: "vx27_container_endcap_square_alpha.png",
    emissive: "vx27_container_endcap_square_emissive_mask.png",
  },
  door: {
    albedo: "vx27_container_door_albedo.png",
    normal: "vx27_container_door_normal.png",
    roughness: "vx27_container_door_roughness.png",
    metallic: "vx27_container_door_metallic.png",
    ao: "vx27_container_door_ao.png",
    alpha: "vx27_container_door_alpha.png",
    emissive: "vx27_container_door_emissive_mask.png",
  },
};
...
function makeVerticalCornerArc(r, height, thetaStart, mat, name) {
  ...
  const geo = new THREE.CylinderGeometry(r, r, height, 10, 1, true, thetaStart, thetaLength);
...
function makeRoundedFootprintSlab(width, length, thickness, radius, material, name, options = {}) {
  ...
  const curveSegments = radius > 0 ? Math.max(8, Math.ceil(radius * 48)) : 1;
  ...
  const capUv = (x, z, capDown) => {
    const u = (x / width - 0.5) / capUvScale + 0.5;
    const v = capDown
      ? (z / length - 0.5) / capUvScale + 0.5
      : (0.5 - z / length) / capUvScale + 0.5;
    return [u, v];
  };
 ...
 * @param {number} edgeRadius
 */
function buildExteriorShell(width, height, length, shell, edgeRadius) {
  const r = normalizeVx27EdgeRadius(edgeRadius, width, height, length);
  ...
  const roofExtMat = buildMaterial("top_bottom", {
    floorDarken: true,
    rotateUV90: true,
    surface: "exterior",
  });
  const floorExtMat = buildMaterial("top_bottom", {
    rotateUV90: true,
    surface: "exterior",
    shellFloor: true,
  });
  ...
  // End caps follow the rounded plan footprint by trimming back the front/back flat span.
  const backFrame = faceMesh(endcapW, endcapH, endcapMat, "vx27_container_endcap_back");
  backFrame.position.set(0, endcapY, -halfL - shell / 2);
  backFrame.rotation.y = Math.PI;
  exterior.add(backFrame);

  const frontFrame = faceMesh(endcapW, endcapH, endcapMat, "vx27_container_endcap_front");
  frontFrame.position.set(0, endcapY, halfL + shell / 2);
  exterior.add(frontFrame);
...
/** @param {THREE.Group} group @returns {import("./Vx27ContainerDoorTuning.js").Vx27ContainerDoorTuning} */
export function readVx27ContainerDoorTuning(group) {
  return normalizeVx27ContainerDoorTuning(
    group.userData.vx27DoorTuning ?? DEFAULT_VX27_CONTAINER_DOOR_TUNING
  );
}
...
function buildShell(group, width, height, length, shell, insets, edgeRadius = 0, doorTuning) {
  group.add(buildExteriorShell(width, height, length, shell, edgeRadius));
...
export function rebuildVx27ContainerExterior(group, edgeRadius) {
  ...
  const shell = vx27ShellThicknessForGroup(group);
  group.add(buildExteriorShell(width, height, length, shell, normalized));
  buildDoors(
    group,
    width,
    height,
...
/** @param {import("./Collision.js").ColliderBox[]} colliders @returns {Vx27ContainerStandoffSpec[]} */
...
function getVx27EndcapOpening(width, height, length, shell, baseY, insets) {
  ...
  const floorYLocal = -halfH + shell + INTERIOR_INSET + n.floorOffset;
  const ceilYLocal = halfH - shell - INTERIOR_INSET - n.ceilingOffset;
  ...
  return {
    ...
    openBottomY: centerY + floorYLocal,
    openTopY: centerY + ceilYLocal,
    innerHalfL,
    offsetZ,
  };
}
 ...
 * }} spec
 * @returns {import("./Collision.js").ColliderBox}
 */
 ...
 *   height?: number,
 *   doorsOpen?: boolean,
 *   propDef?: import("./loadArena.js").ArenaProp,
 * }} [options]
 */
 ...
 * @param {import("./loadArena.js").ArenaProp} [propDef]
 */
