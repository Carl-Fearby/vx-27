...
import { applyDoorwayExtrudeUVs } from "./WallBoxUV.js";
import {
...
export function buildAttachedRoom(
  ...
  // Solid cap above the interior ceiling — seals the room volume below the
  // outdoor catwalk deck (built separately in Level.js arena ceiling group).
  const outdoorDeckBottomY =
    exteriorDeck?.arenaCeilingBottomY ?? exteriorDeckBottomY;
  const outdoorDeckTopY =
    exteriorDeck?.catwalkDeckY ?? outdoorDeckBottomY + arenaCeilingThickness;
  const capBottomY = ceilingBottomY + ceilingThickness;
  if (outdoorDeckTopY > capBottomY + 0.001) {
    const capHeight = outdoorDeckTopY - capBottomY;
    const capMat = ceilingMat.clone();
    if (capMat.color) capMat.color.multiplyScalar(0.35);
    capMat.side = THREE.DoubleSide;
    const cap = new THREE.Mesh(
      new THREE.BoxGeometry(ceilingWidth, capHeight, ceilingDepth),
      capMat
    );
    cap.position.set(
      centerX,
      capBottomY + capHeight / 2,
      ceilingCenterZ
    );
