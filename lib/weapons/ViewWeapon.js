...
import { DEFAULT_RECOIL_TUNING } from "../player/RecoilTuning.js";
import { applyFireRecoilKick, springRecoil } from "../player/RecoilPhysics.js";
import {
  DEFAULT_LASER_EMITTER_TUNING,
  resolveLaserEmitterOffset,
} from "./LaserEmitterTuning.js";
import {
  DEFAULT_STAIR_WALK_TUNING,
  normalizeStairWalkTuning,
} from "../stairs/StairWalkTuning.js";
import {
  createWeaponFlashlight,
  flashlightNightScale,
  FLASHLIGHT_HIP_FORWARD_EXTRA,
  FLASHLIGHT_MUZZLE_FORWARD,
  FLASHLIGHT_SHADOW_NIGHT_SCALE,
  FLASHLIGHT_SHADOW_RAISE_BLEND,
  TORCH_SHADOW_IDLE_SEC,
} from "./WeaponFlashlight.js";
import { resolveAdsRecoilScale } from "../combat/WeaponDamage.js";
import {
  applyWeaponHolster,
  applyWeaponRaise,
...
import { createWeaponReticleDisplay } from "./WeaponReticleDisplay.js";
import {
  loadPistolRoundDisplayTuning,
  loadWeaponRoundDisplayTuning,
  resolveRoundDisplayPose,
  resolveSnappedRoundDisplayPose,
...
/** Screen reticle approximation of the viewmodel sway node, px per metre. */
const RETICLE_SWAY_PX_PER_M = 520;
/** Standing still — gentle alive sway after this delay (seconds). */
...
function prepareViewMaterials(root, maxAnisotropy = 16) {
  root.traverse((obj) => {
    ...
    // Receive sun/moon (and room key lights on VIEWMODEL) — never cast from viewmodel.
...
const FLASHLIGHT_RAISE_SPEED = 4.2;
/** Match walk-bob — below this counts as standing still for torch shadows. */
const TORCH_SHADOW_MOVE_IDLE_SPEED = 0.35;
...
export async function loadViewWeapon(
  ...
  const roundDisplaySnapAim = options.roundDisplaySnapAim === true;
  const roundDisplayFixedHip = options.roundDisplayFixedHip === true;
  ...
  const baseMuzzlePosition = muzzle.position.clone();
  const defaultRoundPose =
    fitProfile === "pistol"
      ? loadPistolRoundDisplayTuning().hip
      : loadWeaponRoundDisplayTuning().hip;
  const roundDisplay = enableRoundDisplay
    ? createWeaponRoundDisplay(sway, defaultRoundPose)
    : null;
  const weaponReticle =
    options.weaponId === "rifle" ? createWeaponReticleDisplay(sway) : null;
  ...
  let flashlightRaise = 0;
  let flashlightRaiseTarget = 0;
  let flashlightShadowCasting = false;
  let torchShadowIdleElapsed = 0;
  /** Latched after still arm pass — stays on while moving until torch off. */
  let torchShadowArmed = false;
  ...
  const getGameCore = options.getGameCore ?? (() => null);
  let activeRecoilTuning = getRecoilTuning();
  let holsterAmount = 0;
  let lastRaiseProgress = 0;
  const reticleMotion = { offsetX: 0, offsetY: 0 };

  const raiseState = createWeaponRaiseState();
  holder.visible = false;

  function applyLaserEmitterOffset(offset = null, blend = 0) {
    const weaponId = options.weaponId === "pistol" ? "pistol" : "rifle";
    const resolved = resolveLaserEmitterOffset(
      offset ?? DEFAULT_LASER_EMITTER_TUNING[weaponId],
      blend,
      weaponId,
    );
    muzzle.position.set(
      baseMuzzlePosition.x + resolved.x,
      baseMuzzlePosition.y + resolved.y,
      baseMuzzlePosition.z + resolved.z,
    );
  }

  applyLaserEmitterOffset(
    options.laserEmitterOffset,
    options.laserEmitterAimBlend ?? 0,
  );
  ...
  return {
    ...
    toggleFlashlight() {
      ...
      if (!flashlightActive) {
        ...
        flashlightRaise = 0;
        flashlightRaiseTarget = 1;
        torchShadowIdleElapsed = 0;
        torchShadowArmed = false;
        return true;
      }
      torchShadowIdleElapsed = 0;
      torchShadowArmed = false;
      flashlightRaiseTarget = 0;
      return true;
    },
    isFlashlightOn() {
      return Boolean(weaponFlashlight && flashlightActive);
    },
    ...
    getSightReadyBlend() {
      ...
      return THREE.MathUtils.clamp((settled - 0.82) / 0.16, 0, 1);
    },
    getReticleMotion() {
      return reticleMotion;
    },
    setHolsterAmount(amount) {
      holsterAmount = THREE.MathUtils.clamp(amount, 0, 1);
    },
    setLaserEmitterOffset(offset, blend = aimBlend) {
      applyLaserEmitterOffset(offset, blend);
    },
    getHolsterAmount() {
    ...
    /** Kick view-model back and up — springs to rest (same net offset as before). */
    applyFireKick(aimBlend = 0) {
      const t = getRecoilTuning();
      activeRecoilTuning = t;
      const kick = applyFireRecoilKick(
        getGameCore(),
        t.fireRecoilBack,
        resolveAdsRecoilScale(aimBlend, getGameCore()),
        t.fireRecoilKickVelScale,
        t.fireRecoilPitch,
        t.fireRecoilPitchVelScale,
      );
      fireRecoilBackVel += kick.backVelDelta;
      fireRecoilPitchVel += kick.pitchVelDelta;
    },
     ...
     *     },
     *   },
     *   laserEmitterOffset?: { hip?: { x?: number, y?: number, z?: number }, ads?: { x?: number, y?: number, z?: number } },
     *   reticleTuning?: import("./CrosshairTuning.js").CrosshairTuning,
     *   reticleTuningActive?: boolean,
     *   reticleTuningMode?: "hip" | "ads",
     * }} [options]
     */
    update(camera, aimTarget, dt, tuningRef, options = {}) {
      const raiseProgress = updateWeaponRaiseState(raiseState, dt);
      lastRaiseProgress = raiseProgress;
      ...
      applyLaserEmitterOffset(options.laserEmitterOffset, aimBlend);
      ...
      activeRecoilTuning = options.recoilTuning ?? getRecoilTuning();
      ...
      fireRecoilBack = s.value;
      fireRecoilBackVel = s.velocity;
      ...
      fireRecoilPitch = s.value;
      fireRecoilPitchVel = s.velocity;
      ...
      const playerStill = moveSpeed <= IDLE_STILL_SPEED;
      if (playerStill && holder.visible && holsterAmount < 0.05 && raiseProgress >= 0.99) {
        idleStillTime += dt;
      } else {
        idleStillTime = 0;
      }
      const idleReady = idleStillTime >= IDLE_STILL_DELAY_SEC;
      const idleTarget =
        idleReady && holder.visible
          ? (1 - aimBlend) * (1 - holsterAmount) * raiseProgress
          : 0;
      idleBlend += (idleTarget - idleBlend) * (1 - Math.exp(-IDLE_FADE_SPEED * dt));
      ...
      let idlePitch = 0;
      if (idleBlend > 0.001) {
        idlePhase += dt * IDLE_BREATHE_FREQ * Math.PI * 2;
        const idleScale = idleBlend;
        idleY = Math.sin(idlePhase) * IDLE_POS_Y * idleScale;
        idleX =
          Math.sin(idlePhase * 0.62 + 0.35) * IDLE_POS_X * idleScale;
        idleZ = Math.cos(idlePhase * 0.48) * IDLE_POS_Z * idleScale;
        idleRoll =
          Math.sin(idlePhase * 0.78 + 1.05) * IDLE_ROLL * idleScale;
        idlePitch =
          Math.sin(idlePhase * 0.41 + 0.2) * IDLE_PITCH * idleScale;
      }

      sway.rotation.set(
        (swayPitch + smoothBobRoll + idleRoll + idlePitch + fireRecoilPitch) *
          swayScale,
        swayYaw * swayScale,
        0,
        "YXZ",
      ...
      sway.position.set(
        (swayPosX + smoothBobX + idleX) * swayScale,
        (swayPosY + smoothBobY + idleY) * swayScale,
        (smoothBobZ + idleZ) * swayScale
      );
      reticleMotion.offsetX = sway.position.x * RETICLE_SWAY_PX_PER_M;
      reticleMotion.offsetY = sway.position.y * RETICLE_SWAY_PX_PER_M;
      ...
      weaponReticle?.syncToModelSurface(
        model,
        sway,
        options.reticleTuning,
        aimBlend,
        options.reticleTuningActive,
        options.reticleTuningMode ?? "hip",
      );

      if (roundDisplay) {
        const roundTuning = options.roundDisplayTuningRef?.current;
        if (roundTuning?.hip && roundTuning?.aim) {
          const displayPose = roundDisplayFixedHip
            ? roundTuning.hip
            : roundDisplaySnapAim
              ? resolveSnappedRoundDisplayPose(
                  roundTuning.hip,
        ...
        const roundHp =
          typeof options.roundDisplayHp === "number" ? options.roundDisplayHp : 100;
        const roundStamina =
          typeof options.roundDisplayStamina === "number"
            ? options.roundDisplayStamina
            : 1;
        const roundLow = Boolean(options.roundDisplayLow);
        roundDisplay.setCount(roundCount, roundLow, roundHp, roundStamina);
        weaponReticle?.setRoundCount?.(roundCount, roundLow);
      }

      if (weaponFlashlight) {
        const playerMoving = moveSpeed > TORCH_SHADOW_MOVE_IDLE_SPEED;
        const nightness = options.nightness ?? 0;

        if (flashlightActive) {
          if (flashlightRaise < flashlightRaiseTarget) {
            flashlightRaise = Math.min(
              flashlightRaiseTarget,
              flashlightRaise + FLASHLIGHT_RAISE_SPEED * dt
            );
          } else if (flashlightRaise > flashlightRaiseTarget) {
            flashlightRaise = Math.max(
              flashlightRaiseTarget,
              flashlightRaise - FLASHLIGHT_RAISE_SPEED * dt
            );
            if (flashlightRaise <= 0 && flashlightRaiseTarget === 0) {
        ...
        const torchBeamReady =
          flashlightActive &&
          flashlightRaise >= FLASHLIGHT_SHADOW_RAISE_BLEND &&
          flashlightNightScale(nightness) >= FLASHLIGHT_SHADOW_NIGHT_SCALE;

        if (!torchShadowArmed) {
          if (!torchBeamReady || playerMoving) {
            torchShadowIdleElapsed = 0;
          } else {
            torchShadowIdleElapsed += dt;
            if (torchShadowIdleElapsed >= TORCH_SHADOW_IDLE_SEC) {
              torchShadowArmed = true;
            }
          }
        }
        ...
        const hipForward =
          (1 - aimBlend) * (1 - aimBlend) * FLASHLIGHT_HIP_FORWARD_EXTRA;
        _flashlightPos.addScaledVector(
          _flashlightDir,
          FLASHLIGHT_MUZZLE_FORWARD + hipForward,
        );
        weaponFlashlight.sync(_flashlightPos, _flashlightDir, flashlightActive, {
          nightness,
          raiseBlend: flashlightRaise,
          shadowArmed: torchShadowArmed,
        });

        const castingShadow = weaponFlashlight.isCastingShadow();
        if (castingShadow && !flashlightShadowCasting) {
          options.onTorchShadowStart?.();
        }
        flashlightShadowCasting = castingShadow;
      }
    },
...
export const LASER_PALETTES = {
  normal: { core: 0xd8f4ff, glow: 0x2a9dff, muzzle: 0x55ccff },
  radioactive: { core: 0xe8ffd8, glow: 0x66ff55, muzzle: 0x99ff77 },
};
