...
/** Default overlay lift — keep 0 on slopes (avoids visible float). */
export const PROFILE_EDGE_FACE_OFFSET = 0;

/** Foot-band overlays — tiny +Y lift above the extruded plinth strip (not along -Y normal). */
export const PROFILE_EDGE_FOOT_OVERLAY_LIFT = 0.002;

/** Normalised profile Y at or below this → foot overlay lift applies. */
export const PROFILE_EDGE_FOOT_Y_MAX = 0.06;

/** Pull overlay quads slightly toward the camera in the depth buffer. */
export function applyProfileEdgePolygonOffset(material) {
  if (!material) return;
...
export function cloneProfileEdgeOverlayMaterial(baseMaterial) {
  const mat = baseMaterial.clone();
  applyProfileEdgePolygonOffset(mat);
  return mat;
}
...
export function buildControlPanelProfileEdgeQuad(
  ...
  const footLiftY =
    Math.min(profile[i0][1], profile[i1][1]) <= PROFILE_EDGE_FOOT_Y_MAX
      ? PROFILE_EDGE_FOOT_OVERLAY_LIFT
      : 0;

  const p0 = new THREE.Vector3(
    x0 + nx * faceOffset,
    y0 + ny * faceOffset + footLiftY,
    z0,
  );
  const p1 = new THREE.Vector3(
    x1 + nx * faceOffset,
    y1 + ny * faceOffset + footLiftY,
    z0,
  );
