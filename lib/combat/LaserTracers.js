...
function createTracerEntry() {
  ...
  return {
    ...
    geo,
    mat,
    positions: geo.attributes.instanceStart.data.array,
    positionBuffer: geo.attributes.instanceStart.data,
    age: 0,
    active: false,
  };
}
...
export function createLaserTracerSystem(scene) {
  ...
  preview.line.name = "laser_emitter_tune_preview";
  preview.line.renderOrder = 52;
  preview.mat.linewidth = PREVIEW_LINE_WIDTH;
  preview.mat.opacity = PREVIEW_OPACITY;
  ...
  function applyResolution(w, h) {
    ...
    preview.mat.resolution.set(w, h);
  }

  function setEntrySegment(entry, from, to) {
    entry.positions[0] = from.x;
    entry.positions[1] = from.y;
    entry.positions[2] = from.z;
    entry.positions[3] = to.x;
    entry.positions[4] = to.y;
    entry.positions[5] = to.z;
    entry.positionBuffer.needsUpdate = true;
  }
  ...
  return {
    ...
    showPreview(from, to, { radioactive = false } = {}) {
      preview.mat.color.setHex(getLaserPalette(radioactive).core);
      preview.mat.opacity = PREVIEW_OPACITY;
      setEntrySegment(preview, from, to);
      if (!preview.active) {
        preview.active = true;
        scene.add(preview.line);
      }
    },
