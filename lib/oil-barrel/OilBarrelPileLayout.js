...
export const OIL_BARREL_PILE_ID =
  /^oil_barrel_(se_)?pile_|^oil_barrel_(se_)?on_side_qa/;

/** Loose props that move/rotate with the pile hub (e.g. open-top fire barrel). */
export const OIL_BARREL_PILE_COMPANION_ID = /^oil_barrel_(se_)?textured$/;

/** @param {{ id?: string }} prop */
export function isOilBarrelPileManagedProp(prop) {
  const id = prop.id ?? "";
  return OIL_BARREL_PILE_ID.test(id) || OIL_BARREL_PILE_COMPANION_ID.test(id);
}
...
/**
 * Hand-authored dumped pile (hub −6.05, 7 — beside VX-27 container). Irregular scatter
 * on the floor; one explicit stack (`pileRole: "stack"`). Spawn uses XZ + rotation only.
 */
...
/**
 * Open-world barrels placed with the pile cluster — transformed with pile hub,
 * spawned as loose props (keep global fire tuning / no pile snap).
 */
export const LEVEL1_OIL_BARREL_PILE_COMPANION_DEFS = [
  {
    id: "oil_barrel_textured",
    type: "oilBarrel",
    x: -4.4349,
    z: 6.1131,
    rotationY: 0,
    note: "Open top on arena floor — moves with pile; fire from global tuning",
  },
];
...
/** Mirrored cluster at arena SE corner (hub 13.15, −13.15) — same order as primary pile. */
export const PILE_SPAWN_ORDER_SE = PILE_SPAWN_ORDER.map((id) =>
  id.replace(/^oil_barrel_/, "oil_barrel_se_"),
);
...
export function orderPileDefsForSpawn(pileDefs) {
  ...
  const ordered = [];
  for (const id of [...PILE_SPAWN_ORDER, ...PILE_SPAWN_ORDER_SE]) {
...
/**
 * Move + rotate hand-authored pile defs around hub.
 * @param {object[]} defs
 * @param {{ x: number, z: number }} hub
 * @param {number} [rotationY] radians — whole-pile spin around hub
 */
export function transformPileDefsAtHub(defs, hub, rotationY = 0) {
  const cos = Math.cos(rotationY);
  const sin = Math.sin(rotationY);
  return defs.map((d) => {
    const lx = d.x - PILE_HUB_DEFAULT.x;
    const lz = d.z - PILE_HUB_DEFAULT.z;
    ...
    if (d.rotationY != null) {
      next.rotationY = d.rotationY + rotationY;
    }
    return next;
  });
}
...
export function applyDefinedOilBarrelPileToArena(arena, opts = {}) {
  ...
  const defs = transformPileDefsAtHub(
    opts.defs ?? LEVEL1_OIL_BARREL_PILE_DEFS,
    hub,
    rotationY
  );
  const companions = transformPileDefsAtHub(
    opts.companionDefs ?? LEVEL1_OIL_BARREL_PILE_COMPANION_DEFS,
    hub,
    rotationY
  );
  const kept = (arena.props ?? []).filter((p) => !isOilBarrelPileManagedProp(p));
  const props = formatPilePropsForLevel(defs, arenaFloorY);
  const companionProps = formatPileCompanionPropsForLevel(companions);
  arena.props = [...kept, ...companionProps, ...props];
...
export const PILE_HUB_DEFAULT = { x: -6.05, z: 7 };
export const PILE_HUB_ROTATION_DEFAULT = 0;
export const PILE_HUB_POS_MIN = -25;
export const PILE_HUB_POS_MAX = 25;
export const PILE_HUB_POS_STEP = 0.05;
export const PILE_HUB_POS_NUDGE = 0.05;
export const PILE_HUB_ROTATION_MIN = -Math.PI;
export const PILE_HUB_ROTATION_MAX = Math.PI;
export const PILE_HUB_ROTATION_STEP = 0.01;
export const PILE_HUB_ROTATION_NUDGE = 0.05;
...
export const OIL_BARREL_PILE_HUB_ROTATION_KEY = "fps-oil-barrel-pile-hub-rotation";
...
/** @param {object[]} rawProps */
export function formatPileCompanionPropsForLevel(rawProps) {
  return rawProps.map((p) => ({
    id: p.id,
    type: "oilBarrel",
    x: round4(p.x),
...
export function loadPileWizardPrefs() {
  if (typeof localStorage === "undefined") {
    return {
      seed: 7,
      hub: { ...PILE_HUB_DEFAULT, rotationY: PILE_HUB_ROTATION_DEFAULT },
    };
  }
  ...
  const hr = parseFloat(localStorage.getItem(OIL_BARREL_PILE_HUB_ROTATION_KEY) ?? "");
  return {
    seed: Number.isFinite(seed) ? seed : 7,
    hub: {
      x: Number.isFinite(hx) ? hx : PILE_HUB_DEFAULT.x,
      z: Number.isFinite(hz) ? hz : PILE_HUB_DEFAULT.z,
      rotationY: Number.isFinite(hr) ? hr : PILE_HUB_ROTATION_DEFAULT,
    },
  };
}

/** @param {{ seed: number, hub: { x: number, z: number, rotationY?: number } }} prefs */
export function savePileWizardPrefs(prefs) {
  ...
  localStorage.setItem(
    OIL_BARREL_PILE_HUB_ROTATION_KEY,
    String(prefs.hub.rotationY ?? PILE_HUB_ROTATION_DEFAULT)
  );
}
