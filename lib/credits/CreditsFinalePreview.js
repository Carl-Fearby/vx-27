...
import { createEnemyRigPreview } from "../combat/EnemyRig.js";
...
export async function mountCreditsFinalePreview(canvas, { getPriority } = {}) {
  ...
  const enemyPreview = await createEnemyRigPreview({ targetHeight: 1.75 });
  if (enemyPreview) {
    const enemy = enemyPreview.holder;
    enemy.position.set(0, -0.02, 0);
    enemy.rotation.set(0, Math.PI * 0.15, 0);
    stage.add(enemy);
    actors.push({
      obj: enemy,
      spinY: 0.008,
  ...
  const loop = createCreditsPreviewLoop(() => {
    ...
    for (const a of actors) {
      if (a.preview) a.preview.update(1 / 60);
