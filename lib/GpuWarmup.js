...
export async function warmupGameGpu({
  ...
  if (!_gameGpuWarmed) {
    ...
    if (sky?.mesh) {
      ...
      await warmupSkyNightBlend(sky, renderer, scene, camera, layeredRender);
    }
...
async function warmupSkyNightBlend(sky, renderer, scene, camera, renderFrame) {
  if (!sky?.setNightBlend || !renderFrame) return;
  const saved = sky.mesh?.material?.uniforms?.uNightBlend?.value ?? 0;
  for (const blend of [0, 1, 0.5]) {
    sky.setNightBlend(blend);
    for (let i = 0; i < 2; i += 1) {
      renderFrame(renderer, scene, camera);
      await new Promise((resolve) => requestAnimationFrame(resolve));
    }
  }
  sky.setNightBlend(saved);
}
