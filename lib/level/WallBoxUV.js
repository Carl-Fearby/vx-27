...
import { archSillY } from "../rooms/DoorwayWall.js";
...
export function applyDeckPieceWorldUVs(
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    if (ay >= ax && ay >= az) {
      // ±Y walk surfaces — world-aligned so cut pieces tile continuously.
      uv.setXY(
        i,
        wx / tile,
        ny > 0 ? wz / tile : -wz / tile
      );
    } else if (ax >= ay && ax >= az) {
      // ±X slab edge — u along Z, v along slab thickness (local Y).
      const u = nx > 0 ? (halfD - lz) / tile : (lz + halfD) / tile;
      const v = (halfH - ly) / tile;
      uv.setXY(i, u, v);
    } else {
      // ±Z slab edge — u along X, v along slab thickness (local Y).
      const u = nz > 0 ? (lx + halfW) / tile : (halfW - lx) / tile;
      const v = (halfH - ly) / tile;
      uv.setXY(i, u, v);
    }
  }
