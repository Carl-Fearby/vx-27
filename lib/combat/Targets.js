...
import { attachEnemyRig, resetEnemyRigDeath, setEnemyRigVisualMode, shouldUseEnemyRigDeath, startEnemyRigDeath, updateEnemyRigDeath } from "./EnemyRig.js";
import { useSimpleEnemyMeshes } from "./EnemyRigPerf.js";
...
export function prebuildRagdollTemplates(targets) {
  ...
  for (const mesh of targets ?? []) {
    if (mesh?.userData?.enemyRig) continue;
...
export function spawnTargets(opts) {
  ...
  function addTarget(pos, yaw, fixedSpawn, rifleOverride) {
    ...
    const hasRifle = rifleOverride ?? (useSimpleEnemyMeshes() ? targetIndex % 2 === 0 : true);
    ...
    if (hasRifle) {
      attachEnemyRig(mesh, config.height);
    }
...
export function deactivateTarget(mesh) {
  ...
  disposeBloodMarksOnTarget(mesh);
  if (mesh.userData.enemyRig) {
    resetEnemyRigDeath(mesh);
    mesh.userData.enemyRigDeath = false;
    mesh.userData.enemyRig.holder.visible = false;
  }
  mesh.visible = false;
...
export function startDeathAnimation(mesh, bulletDir, opts) {
  ...
  if (opts?.scene) {
    ...
    if (shouldUseEnemyRigDeath(mesh) && startEnemyRigDeath(mesh, bulletDir, opts)) {
      mesh.visible = true;
      ud.enemyRigDeath = true;
      if (opts.hitZone === "grenade") {
        spawnGrenadeDeathBlood(
          mesh,
          bulletDir,
          opts.blastFalloff ?? 1,
          opts.scene,
          opts.onBloodSplatter,
        );
      }
      return;
    }
...
export function updateDeathAnimations(targets, dt, onComplete, opts) {
  ...
  for (const mesh of targets) {
    ...
    if (ud.enemyRigDeath) {
      if (updateEnemyRigDeath(mesh, dt)) {
        onComplete(mesh);
      }
      continue;
    }
