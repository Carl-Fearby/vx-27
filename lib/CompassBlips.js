/** Compass blip range — matches FpsGame.jsx */
...
export function updateCompassEnemyBlips(
  ...
  for (const t of targets) {
    ...
    const dx = t.position.x - playerX;
    const dz = t.position.z - playerZ;
    const distSq = dx * dx + dz * dz;
    if (distSq > rangeSq) continue;
    const rel = relativeBearingDeg(
      playerX,
      playerZ,
      playerYaw,
      t.position.x,
    ...
    if (Math.abs(rel) > COMPASS_BLIP_FOV_DEG) continue;
  ...
  for (let i = 0; i < visibleCount; i++) {
    ...
    const left = `${centerPx + _enemyRelScratch[i] * pxPerDeg}px`;
    if (dot.style.left !== left) dot.style.left = left;
    const dist = Math.sqrt(_enemyDistSqScratch[i]);
    const opacity = String(Math.max(0.55, 1 - (dist / COMPASS_BLIP_RANGE) * 0.4));
...
export function updateCompassRewardBlips(
  ...
  for (let i = 0; i < visibleCount; i++) {
    const dot = layer.children[i];
    const left = `${centerPx + _rewardRelScratch[i] * pxPerDeg}px`;
    if (dot.style.left !== left) dot.style.left = left;
    if (dot.style.opacity !== "0.85") dot.style.opacity = "0.85";
  }
}
