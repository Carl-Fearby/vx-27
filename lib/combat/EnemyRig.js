...
function configureEnemyPreviewMesh(mesh) {
...
function cloneEnemyPreviewMaterials(root) {
  root.traverse((obj) => {
...
function disposeEnemyPreviewInstance(root, mixer) {
  mixer?.stopAllAction();
  root?.traverse((obj) => {
  ...
  // Never mat.dispose() here — clones share GLTF textures with the live game template.
}

function buildEnemyPreviewHolder(root, targetHeight) {
  const holder = new THREE.Group();
  holder.name = "enemy-rig-preview";
  const visualScale = targetHeight / _templateHeight;
  holder.scale.setScalar(visualScale);
  holder.add(root);
  ...
  if (skinned?.geometry) {
    ...
    const armature = root.getObjectByName("Armature");
    const armScale = armature?.scale?.x ?? 1;
    const footY = skinned.geometry.boundingBox.min.y * armScale * visualScale;
    holder.position.y = -footY;
  }
  return holder;
}

/**
 * Isolated PX-27 rig for credits/marketing previews.
 * Uses geometry-height scaling — never fitObjectUniform (Meshy armature bbox lies).
 */
export async function createEnemyRigPreview({ targetHeight = 1.62 } = {}) {
  if (useSimpleEnemyMeshes()) {
    return null;
  }
  await preloadEnemyRig();
  if (!_template || !_walkClip) return null;

  const root = cloneSkeleton(_template);
  cloneEnemyPreviewMaterials(root);
  root.traverse((object) => {
    if (object.isMesh) configureEnemyPreviewMesh(object);
  });

  const holder = buildEnemyPreviewHolder(root, targetHeight);
  const mixer = new THREE.AnimationMixer(root);
  const walkAction = mixer.clipAction(_walkClip);
  ...
  return {
    ...
    dispose() {
      disposeEnemyPreviewInstance(root, mixer);
    },
  };
}
