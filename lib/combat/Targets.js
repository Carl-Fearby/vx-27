...
export function prebuildRagdollTemplates(targets) {
  for (const mesh of targets ?? []) {
    if (!mesh?.userData?.isTarget) continue;
...
export function buildWarmupRagdoll(mesh, scene) {
  return createRagdoll(mesh, _warmupBulletDir, scene, "body", null);
}
...
/** Ragdoll builds are deferred to the next frame so kill shots don't hitch the hit frame. */
const _pendingRagdolls = [];

/**
 * Spawn ragdolls queued by `startDeathAnimation` on the previous frame.
 * Call once at the start of the game loop.
 */
export function flushPendingRagdolls(maxPerFrame = 1) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const { mesh, bulletDir, opts } = pending;
    const ud = mesh.userData;
    if (!ud.dying || ud.ragdoll) continue;
...
/** Pull corpse meshes slightly above floor depth to stop z-fighting. */
const RAGDOLL_POLYGON_OFFSET = { factor: -3, units: -3 };
...
function createRagdoll(
  ...
  const ragdollMat = createRagdollMaterial();
  ...
  function makeLimb(seg, parentGroup, pivotPos) {
    ...
    const limbMesh = new THREE.Mesh(seg.geometry, ragdollMat);
...
function updateRagdollPhysics(ragdoll, dt, colliders, floorY, bounds, floorHoles, callbacks) {
  ...
  const runLimbCollisions = (Math.floor(t * 30) % 2) === 0;
