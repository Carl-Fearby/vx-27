...
/** Simulated beacon time baked during load — avoids first-frame jerk near containers. */
export const VX27_CONTAINER_GPU_WARM_SIM_SEC = 2;
export const VX27_CONTAINER_GPU_WARM_FRAMES = 48;
...
/**
 * Advance beacons + render room pass so spotlight shaders and sweeps compile on the load screen.
...
async function warmVx27ContainerSystems(ctx) {
  ...
  for (const pose of warmPoses) {
    ...
    for (let i = 0; i < framesPerPose; i += 1) {
      timeSec += dt;
      updateVx27ContainerBeaconLights(containers, timeSec, null);
...
export async function settleGpuSpawnAfterLoad({
  ...
  const containers = level?.vx27ContainerMeshes ?? [];
  let timeSec = performance.now() * 0.001;
  ...
  for (let i = 0; i < frames; i += 1) {
    if (containers.length) {
      timeSec += VX27_CONTAINER_GPU_WARM_SIM_SEC / Math.max(frames, 1);
      updateVx27ContainerBeaconLights(containers, timeSec, null);
    }
    const { inRoomPass } = applyZoneState(ctx, spawnX, spawnZ, spawnFootY);
    renderGameplayFrame(ctx, !inRoomPass);
