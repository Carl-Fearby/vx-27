...
import { hasLineOfSightToPoint } from "../combat/LineOfSight.js";
...
/** Inset for interior oil skin — thin metal shell inside the exterior wall. */
const INTERIOR_INSET = 0.006;
/** Interior wall maps tile twice around the cylinder (U). */
const INTERIOR_WALL_TILE_U = 2;
const INTERIOR_WALL_TILE_V = 1;

/** Outdoor barrels — lift albedo so sun + hemi read the rust paint in the world pass. */
const OUTDOOR_BARREL_BODY_BRIGHTNESS_SCALE = 1.55;
const OUTDOOR_BARREL_CAP_BRIGHTNESS_SCALE = 1.45;
/** Room barrels — candle-lit but legible without torch/flashlight on the shell. */
const ROOM_BARREL_BODY_BRIGHTNESS_SCALE = 1.65;
const ROOM_BARREL_CAP_BRIGHTNESS_SCALE = 0.95;
/** Room barrels: tame emissive accents so they do not bloom in the room pass. */
const ROOM_BARREL_EMISSIVE_SCALE = 0.12;
...
function applyBodySurface(mat, tuning = _tuning, brightnessScale = 1) {
  applyBrightness(mat, tuning.bodyBrightness * brightnessScale, tuning);
...
function applyCapSurface(mat, tuning = _tuning, brightnessScale = 1) {
  applyBrightness(mat, tuning.capBrightness * brightnessScale, tuning);
  if (mat.normalScale) {
    mat.normalScale.set(tuning.capNormalScale, tuning.capNormalScale);
  }
}

/** Copy shared exterior maps onto a per-barrel clone (room barrels clone before maps exist). */
function ensureExteriorMapsOnMaterial(mat) {
  if (!mat?.isMeshStandardMaterial) return;
  const maps = ensureConfiguredExteriorMaps();
  if (!maps) return;
  if (!mat.map) mat.map = maps.bodyMap;
  if (!mat.normalMap) mat.normalMap = maps.bodyNormal;
  if (!mat.emissiveMap) mat.emissiveMap = maps.bodyEmissive;
  if (!mat.emissive) mat.emissive = new THREE.Color(0xffaa44);
...
function resolveExteriorMaterial(roomId) {
  ...
  ensureExteriorMapsOnMaterial(mat);
  applyBodySurface(mat, _tuning, ROOM_BARREL_BODY_BRIGHTNESS_SCALE);
...
function resolveCapMaterial(which, roomId) {
  ...
  applyCapSurface(mat, _tuning, ROOM_BARREL_CAP_BRIGHTNESS_SCALE);
...
function refreshCapMaterials(tuning = _tuning) {
  ...
  applyCapSurface(_topMat, tuning, OUTDOOR_BARREL_CAP_BRIGHTNESS_SCALE);
  applyCapSurface(_bottomMat, tuning, OUTDOOR_BARREL_CAP_BRIGHTNESS_SCALE);
}
 ...
 * `roomId` is set so fire-light layers and interior wall mats use room tuning.
 * Flame video stays on WORLD at high renderOrder (single world+room geometry pass).
 */
...
export function syncOilBarrelBodyMaterials(root) {
  if (!_bodyMat) return;
  applyBodySurface(_bodyMat, _tuning, OUTDOOR_BARREL_BODY_BRIGHTNESS_SCALE);
  if (!root) return;
  root.traverse((obj) => {
    if (!obj.isMesh || !BARREL_BODY_MESH_NAMES.has(obj.name)) return;
    let barrel = obj;
    while (barrel && barrel.name !== "oil_barrel") barrel = barrel.parent;
    const roomId = barrel?.userData?.roomId;
    if (roomId) {
      ensureExteriorMapsOnMaterial(obj.material);
      applyBodySurface(obj.material, _tuning, ROOM_BARREL_BODY_BRIGHTNESS_SCALE);
      obj.material.emissiveIntensity =
        _tuning.emissiveIntensity * ROOM_BARREL_EMISSIVE_SCALE;
      return;
    }
...
const _barrelFireSample = new THREE.Vector3();
...
/** @param {THREE.Object3D} barrelGroup @param {THREE.Vector3} out */
function getBarrelFireSamplePoint(barrelGroup, out) {
  const fireRig =
    barrelGroup.getObjectByName("oil_barrel_fire_lights") ??
    barrelGroup.getObjectByName("oil_barrel_fire_light");
  if (fireRig) {
    fireRig.getWorldPosition(out);
    return out;
  }
  barrelGroup.getWorldPosition(out);
  out.y += OIL_BARREL_HEIGHT * 0.45;
  return out;
}
 ...
 * never hurt. Blocked by solid level geometry (container walls, arena mesh).
 * Returns whether damage applies this frame.
 *
 * @param {THREE.Object3D} root
 * @param {THREE.Vector3} playerPos Player position (typically camera / eye).
 * @param {number} dt
 * @param {import("./OilBarrelTuning.js").OilBarrelTuning} [tuning]
 * @param {THREE.Object3D[]} [hitMeshes] Level hit meshes for line-of-sight tests.
 */
export function tickOilBarrelFireProximityDamage(
  root,
  playerPos,
  dt,
  tuning = _tuning,
  ...
  root.traverse((obj) => {
    ...
    if (hitMeshes?.length) {
      getBarrelFireSamplePoint(obj, _barrelFireSample);
      if (!hasLineOfSightToPoint(playerPos, _barrelFireSample, hitMeshes)) {
        return;
      }
    }
    inRange = true;
  ...
  if (!inRange || !gameCore) return false;
  const shouldDamage = requireWasmMethod(gameCore, "tickOilBarrelFireProximityDamage")({
    dt,
    inRange,
    intervalSec: OIL_BARREL_FIRE_PROXIMITY_INTERVAL,
  });
  if (!shouldDamage) return false;
  requireWasmMethod(gameCore, "applyOilBarrelFireDamage")();
  return true;
}
...
/** Shadow-only — scaled shell casts fire penumbra without a visible inner wall. */
let _openFireShadowShellMat = null;
function ensureOpenFireShadowShellMaterial() {
  if (!_openFireShadowShellMat) {
    ...
    _openFireShadowShellMat = mat;
  }
  return _openFireShadowShellMat;
}
...
function makeBodyMaterial() {
  ...
  mat.side = THREE.DoubleSide;
  applyBodySurface(mat, _tuning, OUTDOOR_BARREL_BODY_BRIGHTNESS_SCALE);
  return mat;
}
...
function makeCapMaterial(tex, maps) {
  ...
  applyCapSurface(mat, _tuning, OUTDOOR_BARREL_CAP_BRIGHTNESS_SCALE);
  return mat;
}
...
function buildBarrelMesh(options = {}) {
  ...
  if (topCap) {
    ...
    group.userData.oilBarrelOpenFire = true;
    const shadowShell = new THREE.Mesh(
      ...
      ensureOpenFireShadowShellMaterial()
    );
    ...
    // Half-width fire shadow footprint; invisible in the beauty pass (shadow-only).
    shadowShell.scale.set(0.5, 1, 0.5);
    exterior.castShadow = false;
    group.add(shadowShell);

    const innerRadius = r - INTERIOR_INSET;
 ...
 * @param {import("../level/loadArena.js").ArenaConfig} [arena] When set, interior maps load only if needed.
 */
...
export function createOilBarrel(parent, x, z, floorY = 0, rotationY = 0, options = {}) {
  ...
  barrel.traverse((obj) => {
    ...
    if (obj.name === "oil_barrel_exterior" && barrel.userData.oilBarrelOpenFire) {
      obj.castShadow = false;
...
/**
 * World pass for outdoor barrels; room-attached barrels use the room pass only so
 * exterior albedo is lit by room point lights (world pass has no room lights).
 * Open-fire arena barrels stay on world + room for doorway visibility.
 * @param {THREE.Mesh} mesh
 * @param {string | null} roomId
 * @param {boolean} openFire
 */
function applyBarrelWorldAndRoomLayers(mesh, roomId, openFire = false) {
  if (roomId) {
    setRoomInteriorLayer(mesh);
    return;
  }
  setWorldLayer(mesh);
  if (openFire) {
...
function applyBarrelRenderLayers(barrel, roomId) {
  const openFire = barrel.userData?.oilBarrelOpenFire === true;
  const roomPass = !!(roomId || openFire);
  barrel.traverse((obj) => {
    if (!obj.isMesh) return;
    ...
    if (
      ...
      applyBarrelWorldAndRoomLayers(obj, roomId, openFire);
      obj.renderOrder = 3;
      return;
    }
    ...
    if (roomPass && BARREL_INTERIOR_ROOM_MESH_NAMES.has(obj.name)) {
      setRoomInteriorLayer(obj);
      if (openFire && !roomId) {
        obj.layers.enable(WORLD_LAYER);
      }
...
export function spawnLevelOilBarrels(root, arena) {
  ...
  refreshOilBarrelMaterials(_tuning, root);
  return meshes;
}

/** @param {import("../level/loadArena.js").ArenaProp} def @param {number} floorY */
