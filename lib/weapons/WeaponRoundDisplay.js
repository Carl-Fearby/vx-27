...
/** Match `.hudHealthFill` — blue base, orange ≤50, red ≤25, green >100. */
function hpBarMetrics(hp) {
 ...
 * @param {import("./WeaponRoundDisplayTuning.js").WeaponRoundDisplayPose} [defaultPose]
 */
export function createWeaponRoundDisplay(sway, defaultPose = DEFAULT_HIP_ROUND_DISPLAY) {
  ...
  function drawMeterBar(cx, barTopY, metrics) {
  ...
  function drawHpBar(cx, barTopY, hp) {
    drawMeterBar(cx, barTopY, hpBarMetrics(hp));
  }
  ...
  resizeCanvasForFontSize(fontSize);
  primeHudFont(fontSize);
  applyVisualTuning(defaultPose);
  draw(100, 88, 1, false);

  return {
    mesh,
    getSuggestedPose: () => ({ ...defaultPose }),
