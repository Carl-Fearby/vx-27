...
/** @param {THREE.Mesh} mesh */
function canSpawnRagdollImmediately(mesh) {
  ensureTargetRagdollTemplate(mesh);
  return mesh.userData.predictiveKillGpuWarmed === true;
}

/**
 * Spawn ragdolls queued by `startDeathAnimation` when not spawned on the kill frame.
...
export function startDeathAnimation(mesh, bulletDir, opts) {
  ...
  if (opts?.scene) {
    ...
    if (canSpawnRagdollImmediately(mesh)) {
      spawnRagdollForDeath(mesh, bulletDir, pendingOpts);
      return;
    }

    ud.ragdollPending = true;
    _pendingRagdolls.push({ mesh, bulletDir, opts: pendingOpts });
    return;
  }
