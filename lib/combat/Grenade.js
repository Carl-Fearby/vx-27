...
const PROJECTILE_MAX_MOVE = 0.018;
const PROJECTILE_MAX_SUBSTEPS = 6;
...
function computeThrowVelocityInto(aimDir, out) {
  const loftRad = (_params.loftAngle * Math.PI) / 180;
  _previewThrowRight.crossVectors(aimDir, _previewWorldUp).normalize();
  _previewThrowUp.crossVectors(_previewThrowRight, aimDir).normalize();
  return out
    .copy(aimDir)
    .multiplyScalar(Math.cos(loftRad))
    .add(_previewThrowUp.multiplyScalar(Math.sin(loftRad)))
    .normalize()
    .multiplyScalar(_params.throwSpeed);
}
...
export function updateTrajectoryPreview(
  ...
  computeThrowVelocityInto(_previewAimDir, _previewVelocity);
  ...
  const bounceCircle = ensurePreviewCircle(group, "bounce");
  bounceCircle.visible = sim.hasBouncePos;
  if (sim.hasBouncePos) {
    bounceCircle.position.set(
      sim.bouncePos.x,
      sim.bouncePos.y + 0.02,
      sim.bouncePos.z
    );
    bounceCircle.scale.setScalar(_params.blastRadius * 0.6);
  }
}
  ...
  for (let i = grenades.length - 1; i >= 0; i--) {
    ...
    if (!g.detonated) {
      ...
      const substeps = Math.min(
        ...
        Math.max(1, Math.ceil((speed0 * dt) / PROJECTILE_MAX_MOVE)),
      );
      ...
      const overHole =
        ...
        pointInFloorHole(px0, pz0, g.floorHoles, GRENADE_RADIUS);
      if (overHole) beginHoleFall(g);

      for (let step = 0; step < substeps; step++) {
        g.vel.y -= _params.gravity * subDt;
        g.mesh.position.x += g.vel.x * subDt;
        g.mesh.position.y += g.vel.y * subDt;
        g.mesh.position.z += g.vel.z * subDt;
        ...
        const floorTop = effectiveSupportY + GRENADE_RADIUS;
        if (!g.fallingThroughHole && py <= floorTop) {
