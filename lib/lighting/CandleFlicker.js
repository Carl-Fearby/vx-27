...
function buildFlickerCfg(opts) {
  return {
    ...
    minFactor: opts.minFactor ?? 0.2,
    blinkMinGap: opts.blinkMinGap ?? null,
    blinkMaxGap: opts.blinkMaxGap ?? null,
    blinkDuration: opts.blinkDuration ?? 0.08,
    blinkFloor: opts.blinkFloor ?? 0,
  };
}
...
function attachCandleFlicker(light, cfg, baseWobbleSpeed, overrides = {}) {
  ...
  light.userData.candleFlicker = {
    ...
    blinkEndTime: -1,
    nextBlinkTime:
      cfg.blinkMinGap != null
        ? timeOffset +
          cfg.blinkMinGap +
          (overrides.blinkStartOffset ??
            Math.random()) *
            Math.max((cfg.blinkMaxGap ?? cfg.blinkMinGap) - cfg.blinkMinGap, 0.001)
        : -1,
  };
}
...
export function deriveBarrelFlickerSeeds(barrelSeed, side = 0) {
  ...
  return {
    ...
    blinkStartOffset: frac01(Math.sin(s * 6.19 + sideSalt * 1.3) * 90210.17),
  };
}
...
export function updateCandleFlicker(lights, time) {
  for (const light of lights) {
    ...
    if (!data || !light.visible) continue;
    const cfg = data.cfg;
    ...
    if (cfg.blinkMinGap != null && t >= data.nextBlinkTime && t >= data.blinkEndTime) {
      data.blinkEndTime = t + cfg.blinkDuration;
      const blinkSpan = (cfg.blinkMaxGap ?? cfg.blinkMinGap) - cfg.blinkMinGap;
      data.nextBlinkTime =
        t + cfg.blinkMinGap + Math.random() * Math.max(blinkSpan, 0.001);
    }
    if (t < data.blinkEndTime) {
      factor = cfg.blinkFloor;
    }

    factor = THREE.MathUtils.clamp(factor, cfg.minFactor, 1.05);
