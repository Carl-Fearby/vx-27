...
import { updateOilBarrelFireShadowBudget } from "@/lib/oil-barrel/OilBarrelFireLight.js";
import {
...
import {
  pickHackableControlPanelUnderCrosshair,
  getControlPanelHackPromptContent,
} from "@/lib/control-panel/ControlPanelHackInteract.js";
...
import { getOtherPrimaryWeaponId } from "@/lib/weapons/PrimaryWeapons.js";
import {
...
export function runGameFrame(ctx, now) {
  ...
  if (
    ...
    if (propId && ctx.handleOilBarrelPlacementPickRef?.current) {
  ...
  const consoleInteractFocus = consoleHackUiOpen || showHackPrompt;
  if (doorTarget) {
    setCenterPromptPersistent(
      centerPromptState,
      getVx27DoorInteractPrompt(
  ...
  } else if (showHackPrompt) {
    setCenterPromptPersistent(
      centerPromptState,
      getControlPanelHackPromptContent(ctx.bindingsRef.current),
    );
  } else {
  ...
  if (!frozen && !ctx.consoleHackOpenRef.current) {
    ...
    ctx.weaponSwap.update(dt, ctx.primaryWeapons, () => ctx.activePrimaryId, (id) => {
    ...
    ctx.weapon?.update(ctx.camera, aimTarget, dt, ctx.getActiveTuningRef(), {
      snapAim: !locked && !touchMode,
      moveSpeed: ctx.player.getHorizontalSpeed(),
      onStairs: ctx.player.isOnStairs(),
      walkBobTuning: resolveWalkBobTuning(ctx.walkBobTuningRef.current),
      stairWalkTuning: normalizeStairWalkTuning(ctx.stairWalkTuningRef.current),
      nightness: ctx.dayNightCurNightnessRef.current,
      roundCount: rounds,
      roundDisplayLow:
        rounds < cfg.lowAmmoThreshold || (rounds === 0 && spare === 0),
      roundDisplayHp: ctx.playerHealthRef.current,
      roundDisplayStamina: ctx.player.getStamina(),
      roundDisplayTuningRef: ctx.roundDisplayTuningRef,
      onTorchShadowStart: () => {
    ...
    for (const id of ["rifle", "pistol"]) {
      ...
      w.update(ctx.camera, 0, dt, id === "pistol" ? ctx.pistolTuningRef : ctx.weaponTuningRef, {
        snapAim: true,
        roundDisplayTuningRef:
          id === "rifle" ? ctx.roundDisplayTuningRef : undefined,
      });
    }
  }
  ...
  ctx.screenCrosshairRef.current?.update({
    aimBlend,
    tuning: ctx.crosshairTuningRef.current,
    showGunReticule,
  ...
  if (
    ...
    const slotPick = getPrimaryWeaponIdFromSlotInput(ctx.input);
    const swapToggle =
      wasBindingPressed(ctx.input, ctx.bindingsRef.current, "swapWeapon") ||
      wasPrimarySwapPressed(ctx.input);
    const nextId =
      slotPick ??
      (swapToggle ? getOtherPrimaryWeaponId(ctx.activePrimaryId) : null);
    if (nextId && nextId !== ctx.activePrimaryId) {
