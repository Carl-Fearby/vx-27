...
/** VX-27 blue — interior ceiling fill (room pass only). */
const VX27_CEILING_LIGHT_COLOR = 0x5eaaff;
const VX27_CEILING_LIGHT_INTENSITY = 12;
const VX27_CEILING_LIGHT_DISTANCE = 9;
...
function buildInteriorShell(width, height, length, shell, insets, edgeRadius = 0) {
  ...
  ceilingLight.position.set(offsetX, ceilY - 0.12, offsetZ);
  ceilingLight.userData.vx27ContainerLight = true;
  ceilingLight.userData.vx27BaseIntensity = VX27_CEILING_LIGHT_INTENSITY;
  pinLightToRoomInteriorLayer(ceilingLight);
  interior.add(ceilingLight);
  interior.userData.vx27CeilingLight = ceilingLight;
  ...
  for (const end of layout.ends) {
    for (const leaf of end.leaves) {
      ...
      addShadowOccluderBox(
        ...
        layout.leafW * 0.96,
        layout.leafH * 0.96,
        Math.max(layout.thickness * 0.92, 0.05),
        0,
        0,
        0
      );
