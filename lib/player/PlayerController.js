...
export function createPlayerController(camera, bounds, floorY, options = {}) {
  ...
  function clampPitch() {
    ...
    // Slide base pitch limits with recoil so pitch + recoilPitchAnim keeps full vertical range.
    pitch = THREE.MathUtils.clamp(
      pitch,
      -limit - recoilPitchAnim,
      limit - recoilPitchAnim,
    );
  ...
  function resolveColliders() {
    ...
    for (const box of colliders) {
      ...
      // A target only blocks the player while it is a live, visible dummy. Dead,
      // dying, hidden, or hole-falling targets can keep their collider `active`
      // for a frame or more (e.g. a target fading out as it sinks through a floor
      // hole) — colliding with one is the "invisible wall on open floor" bug.
      if (box.targetMesh) {
        const tud = box.targetMesh.userData;
        if (
          !box.targetMesh.visible ||
          tud.dying ||
          tud.fallingThroughHole ||
          (tud.health ?? 0) <= 0
        ) {
          continue;
        }
  ...
  return {
    update(input, dt) {
      ...
      const jumpRampY = sampleRawRampFootY(position.x, position.z);
      const stairSupported =
        jumpRampY != null &&
        velocity.y <= 0.05 &&
        Math.abs(headroomFootY - jumpRampY) <= 0.12;
      const movementGates = requireWasmMethod(gameCore, "computePlayerMovementGates")({
        ...
        stairSupported: Boolean(stairSupported),
      ...
      const jumpGrounded = grounded || (jumpPressed && stairSupported);
      const vertical = requireWasmMethod(gameCore, "tickPlayerVertical")({
        dt,
        y: position.y,
        grounded: Boolean(jumpGrounded),
        jumpPressed: Boolean(jumpPressed),
    ...
    addAimRecoil(strength = 1) {
      const t = getRecoilTuning();
      const kick = requireWasmMethod(gameCore, "planAimRecoilKick")({
        pitch: t.aimRecoilPitch,
        yaw: t.aimRecoilYaw,
        strength,
        kickVelScale: t.kickVelScale,
        pitchRoll: Math.random(),
        yawRoll: Math.random(),
      });
      recoilPitchTarget += kick.pitchTargetDelta;
      recoilYawTarget += kick.yawTargetDelta;
      recoilPitchVel += kick.pitchVelocityDelta;
      recoilYawVel += kick.yawVelocityDelta;
      clampPitch();
    },
    tickAimRecoil(dt) {
      stepAimRecoil(dt);
