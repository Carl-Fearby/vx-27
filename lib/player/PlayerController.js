...
/** Collider kinds ignored by headroom / ceiling checks (not overhead — walk beside them). */
...
export function createPlayerController(camera, bounds, floorY, options = {}) {
  ...
  function applyStaminaMaxChange() {
    if (gameCore?.syncStaminaMax) {
      gameCore.syncStaminaMax(Math.max(SPRINT_STAMINA_BASE, getStaminaMax()));
      const state = gameCore.getPublicState?.();
      if (state) {
    ...
    const gainCap = Math.max(SPRINT_STAMINA_BASE, getStaminaMax());
    if (gainCap > lastStaminaMax) {
      const delta = gainCap - lastStaminaMax;
      if (stamina <= gainCap) {
        stamina = Math.min(gainCap, stamina + delta);
      }
    } else if (gainCap < lastStaminaMax) {
      if (gainCap <= SPRINT_STAMINA_BASE) {
        stamina = Math.min(stamina, SPRINT_STAMINA_BASE);
      }
    }
    lastStaminaMax = gainCap;
  }
  ...
  return {
    update(input, dt) {
      ...
      let isMoving = false;
      const coreMove = gameCore?.tickPlayerCore?.({
        dt,
        forward: isBindingDown(input, bindings, "forward"),
        backward: isBindingDown(input, bindings, "backward"),
        strafeLeft: isBindingDown(input, bindings, "strafeLeft"),
        strafeRight: isBindingDown(input, bindings, "strafeRight"),
        sprint: isBindingDown(input, bindings, "sprint"),
        crouching,
        aiming,
      ...
      if (coreMove) {
        ...
        } else if (stamina < SPRINT_STAMINA_BASE) {
          stamina = Math.min(
            SPRINT_STAMINA_BASE,
        ...
        if (stamina > SPRINT_STAMINA_BASE) {
          const decayPerSec =
            RADIOACTIVE_OVERFLOW_DECAY_PCT /
            (RADIOACTIVE_OVERFLOW_DECAY_INTERVAL_SEC * 100);
          stamina = Math.max(
            SPRINT_STAMINA_BASE,
            stamina - decayPerSec * dt
          );
        }
      ...
      const vertical = gameCore?.tickPlayerVertical?.({
        dt,
        y: position.y,
        grounded,
        jumpPressed,
      ...
      gameCore?.syncPlayerVertical?.(position.y, velocity.y, grounded);
    ...
    respawn() {
      ...
      if (gameCore?.resetPlayerCore) {
        gameCore.resetPlayerCore(Math.max(SPRINT_STAMINA_BASE, getStaminaMax()));
      } else {
        applyStaminaMaxChange();
      }
      arrowLookVel.yaw = 0;
