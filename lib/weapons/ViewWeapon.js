...
export async function loadViewWeapon(
  ...
  function applyLaserEmitterOffset(offset = null) {
    ...
    muzzle.position.set(
      baseMuzzlePosition.x + x,
      baseMuzzlePosition.y + y,
      baseMuzzlePosition.z + z
    );
  }

  applyLaserEmitterOffset(options.laserEmitterOffset);
  ...
  return {
    ...
    setLaserEmitterOffset(offset) {
      applyLaserEmitterOffset(offset);
    },
    getHolsterAmount() {
    ...
    applyFireKick(aimBlend = 0) {
      fireRecoilBack = FIRE_RECOIL_BACK * resolveAdsRecoilScale(aimBlend);
    },
     ...
     *     },
     *   },
     *   laserEmitterOffset?: { x?: number, y?: number, z?: number },
     * }} [options]
     */
    update(camera, aimTarget, dt, tuningRef, options = {}) {
      ...
      sway.rotation.set(
        (swayPitch + smoothBobRoll + idleRoll + idlePitch) * swayScale,
        swayYaw * swayScale,
        0,
        "YXZ"
      );
