...
export function createWeaponFlashlight(scene) {
  ...
  /** After load warmup — keep the 1024² shadow map allocated; only pause updates when off. */
  ...
  function primeShadowMaps(renderer) {
    ...
    hotspot.shadow.autoUpdate = false;
  ...
  function sync(origin, direction, on, opts = {}) {
    ...
    if (shadowMapsPrimed) {
      hotspot.castShadow = true;
      hotspot.shadow.autoUpdate = casting;
    } else {
      hotspot.castShadow = casting;
    }
