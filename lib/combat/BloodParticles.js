...
let _bloodPointsMatTemplate = null;
const _sprayDir = new THREE.Vector3();
const _bloodSpawnRight = new THREE.Vector3();
const _bloodSpawnUp = new THREE.Vector3();
const _bloodSpawnDir = new THREE.Vector3(0, 0, -1);
...
function getBloodPointsMaterialTemplate() {
  if (!_bloodPointsMatTemplate) {
    _bloodPointsMatTemplate = new THREE.PointsMaterial({
      size: 0.085,
  ...
  return _bloodPointsMatTemplate;
}
...
export function spawnBloodMarkOnTarget(targetMesh, hitPoint, hitFace, bulletDir, damageDealt) {
  ...
  // PX-27 rigs don't match the procedural hitbox — decals parented to the mesh misalign.
  if (targetMesh.userData?.enemyRig) return;
...
/** More HP lost → more droplets (~5–32, ~20% of the old 24–160 range). */
export function bloodParticleCountForDamage(damage) {
  if (!Number.isFinite(damage) || damage <= 0) return 0;
  const full = Math.min(160, Math.max(24, Math.round(18 + damage * 6)));
  return Math.max(5, Math.round(full * 0.2));
}
...
export function spawnBloodSplatter(scene, hitPoint, bulletDir, damageDealt) {
  ...
  const dir = bulletDir?.lengthSq() > 1e-6 ? _bloodSpawnDir.copy(bulletDir).normalize() : _bloodSpawnDir.set(0, 0, -1);
  _bloodSpawnRight.crossVectors(dir, _worldUp);
  if (_bloodSpawnRight.lengthSq() < 1e-6) _bloodSpawnRight.set(1, 0, 0);
  else _bloodSpawnRight.normalize();
  _bloodSpawnUp.crossVectors(_bloodSpawnRight, dir).normalize();
  ...
  for (let i = 0; i < count; i++) {
    ...
    _sprayDir.copy(dir)
      ...
      .addScaledVector(_bloodSpawnRight, (Math.random() - 0.5) * 1.1)
      .addScaledVector(_bloodSpawnUp, Math.random() * 0.65)
      .normalize();

    const speed = THREE.MathUtils.lerp(2.0, 7.5, Math.random()) * (0.85 + damageDealt * 0.04);
    velocities[i * 3] = _sprayDir.x * speed;
    velocities[i * 3 + 1] = _sprayDir.y * speed + Math.random() * 0.8;
    velocities[i * 3 + 2] = _sprayDir.z * speed;

    lifetimes[i] = THREE.MathUtils.lerp(0.35, 0.95, Math.random());
    if (lifetimes[i] > maxLife) maxLife = lifetimes[i];
    ages[i] = 0;
  ...
  const mat = getBloodPointsMaterialTemplate().clone();
...
export function updateBloodSplatters(splatters, dt, scene) {
  for (let i = splatters.length - 1; i >= 0; i--) {
    ...
    const fadeT = Math.min(1, s.age / s.maxLife);
    s.mat.opacity = 0.95 * (1 - fadeT * fadeT);

    if (alive === 0 || s.age >= s.maxLife + 0.05) {
...
const _warmupMarkPoint = new THREE.Vector3();
const _warmupMarkDir = new THREE.Vector3(0, 0, -1);

/**
 * Spawn throwaway blood marks on a target and render so first hits don't hitch.
 * @param {THREE.Mesh[]} targets
 * @param {{ renderFrame?: (r: THREE.WebGLRenderer, s: THREE.Scene, c: THREE.Camera) => void, frames?: number }} [opts]
 */
export async function warmupBloodMarksGpu(targets, opts = {}) {
  if (!targets?.length) return;

  getBloodMarkTexture();
  getBloodMarkMaterial();

  const target = targets[0];
  target.updateMatrixWorld(true);
  target.getWorldPosition(_warmupMarkPoint);
  _warmupMarkPoint.y += (target.userData.height ?? 2) * 0.45;

  spawnBloodMarkOnTarget(target, _warmupMarkPoint, null, _warmupMarkDir, 14);
  ...
  disposeBloodMarksOnTarget(target);
}
