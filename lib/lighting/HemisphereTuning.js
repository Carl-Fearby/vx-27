...
/** Daylight hemisphere fill — tuned for PX-27 readability under the arena roof. */
export const DEFAULT_HEMI_DAY = Object.freeze({
  temperature: 7000,
  intensity: 0.97,
});

/** Deep blue night fill — silhouettes only; flashlight owns detail. */
...
function migrateHemiDay(settings) {
  if (
    (settings.temperature === 9400 && settings.intensity === 0.52) ||
    (settings.temperature === 9700 && settings.intensity === 0.44)
  ) {
    return { ...DEFAULT_HEMI_DAY };
  }
  return settings;
}
...
function loadHemi(key, fallback) {
  ...
  try {
    ...
    const settings = sanitize(JSON.parse(raw), fallback);
    if (key !== HEMI_DAY_KEY) return settings;
    const migrated = migrateHemiDay(settings);
...
/**
 * Ground bounce vs sky — lower lum = steeper normal falloff (vertical corners darker).
 * @param {{ indoor?: boolean, sheltered?: boolean }} options
 */
function hemiGroundCoupling(options = {}) {
  if (options.indoor) {
