...
export function attachCombatRuntime(ctx) {
  ...
  function scheduleRespawn(mesh) {
    ...
    const respawnPlan =
      ctx.gameCore?.planTargetRespawn?.(targetConfig.respawnDelay) ?? {
        delayMs: targetConfig.respawnDelay * 1000,
        shouldSchedule: true,
      };
    ...
    const dropPlan =
      ctx.gameCore?.planKillDrops?.(
        ...
        hp:
          ...
          explosiveKill ||
          zone === "head" ||
          ctx.playerHealthRef.current < 50,
        ammo:
          DEV_DROP_ALL_REWARDS ||
          ctx.spareMagsRef.current <= ctx.ammoDropSpareThresholdRef.current,
        grenade:
          DEV_DROP_ALL_REWARDS ||
          ctx.rollGrenadeDrop(ctx.grenadeCountRef.current),
      };
  ...
  function awardCombatScoreAt(mesh, hitResult, hitPoint) {
    ...
    if (ctx.gameCore) {
      ...
      scoreResult = ctx.gameCore.calculateCombatScore(
        hitResult.zone ?? "body",
        hitResult.damage ?? 0,
      ...
      scoreResult = applyCombatScore(mesh, hitResult);
    }
  ...
  function applyGrenadeHit(mesh, hitPoint, blastDir, damage) {
    ...
    const damageResult =
      ctx.gameCore?.applyTargetDamage(ud.health, ud.maxHealth, damage) ??
      (() => {
        const health = Math.max(0, ud.health - damage);
        return {
          health,
          ratio: ud.maxHealth > 0 ? health / ud.maxHealth : 0,
          killed: health <= 0,
        };
      })();
  ...
  function fireOneRound() {
    ...
    const recoilScale = resolveAdsRecoilScale(ads);
    const damageScale = resolveAdsDamageScale(ads);
