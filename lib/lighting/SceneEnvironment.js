...
/** Force shader rebuild after warmup or shadow pipeline changes. */
...
/**
 * Drop warmup shadow atlases and shader variants so the first gameplay frame
 * matches a cold renderer (skipWarmup path is error-free).
 * @param {THREE.WebGLRenderer} renderer
 * @param {THREE.Scene} scene
 * @param {{ camera?: THREE.Camera, renderFrame?: (r: THREE.WebGLRenderer, s: THREE.Scene, c: THREE.Camera) => void, cleanFrames?: number }} [options]
 */
export function finalizeGpuWarmupRendererState(renderer, scene, options = {}) {
  if (!renderer?.shadowMap || !scene) return;

  const preserveShadowMaps = options.preserveShadowMaps === true;
  if (!preserveShadowMaps) {
    scene.traverse((obj) => {
  ...
  const { camera, renderFrame } = options;
  const cleanFrames = options.cleanFrames ?? 2;
  if (camera && renderFrame) {
    for (let i = 0; i < cleanFrames; i += 1) {
...
/** @deprecated alias */
export const primeGameplayShadowPipeline = finalizeGpuWarmupRendererState;
...
export function syncLightLayersForZone(scene, inRoom, outdoorLights, roomLights) {
  ...
  if (_lastMainLightZoneInRoom === inRoom) return;
  _lastMainLightZoneInRoom = inRoom;
...
export function createOutdoorLights(scene, options = {}) {
  ...
  const ambient = new THREE.AmbientLight(0xd0dce8, sheltered ? 0.04 : 0.14);
  scene.add(ambient);

  /** Shadowless fill — kept low under a roof so unlit surfaces stay dark. */
  const fill = new THREE.DirectionalLight(0xc8d4e8, sheltered ? 0.07 : 0.48);
  ...
  const westFill = new THREE.DirectionalLight(0xb8c8e0, sheltered ? 0.05 : 0.32);
 ...
 * @param {{ skyRoot?: THREE.Object3D | null, outdoorShadowLights?: THREE.Light[] }} [options]
 */
...
/** Pin shadow pipeline after warmup — r182+ must not leave a stale VSM atlas. */
