...
import { setWorldLayer } from "@/lib/lighting/LightingLayers.js";
...
export function updateSnow(snow, camera, dt, time, opts = {}) {
  ...
  const intensity = opts.intensity ?? DEFAULT_SNOW_INTENSITY;
  const stickRate = opts.stickRate ?? DEFAULT_SNOW_STICK_RATE;
  const visibleCount = active
    ? Math.min(
        FALLING_COUNT,
        Math.max(1, Math.round(FALLING_BASE * intensity))
      )
    : 0;
  snow.fallingRoot.visible = active && visibleCount > 0;
  ...
  const allowSettle = opts.allowSettle === true;
  const enclosed = opts.enclosed === true;
  const fallMul = 0.35 + intensity * 0.42;
  const fallOpacity = Math.min(0.98, 0.18 + intensity * 0.16);
  const vistaOpacity = Math.min(0.9, 0.14 + intensity * 0.14);
  ...
  for (let i = 0; i < FALLING_COUNT; i++) {
    ...
    if (landingY !== null) {
      if (allowSettle) {
        settleFlake(snow, wx, wz, landingY, stickRate, intensity, camX, camZ);
      }
