...
import {
  overlapsTargets as overlapsTargetsCore,
  pickRandomSpawnPositionCore,
...
function overlapsTargets(x, z, radius, margin, targets, skip, gameCore = null) {
  return overlapsTargetsCore(gameCore, x, z, radius, margin, targets, skip);
}
...
function validateSpawnAt(x, z, opts) {
  ...
  if (overlapsTargets(x, z, radius, spawnMargin, targets, skip, opts.gameCore)) return null;
  if (!skipHoleCheck && floorHoles && pointInFloorHole(x, z, floorHoles, radius, opts.gameCore ?? null)) {
    return null;
  }
  return { x, z, y: footY };
}
...
export function resolveAuthoredSpawnPosition(x, z, opts) {
  const { bounds, config, gameCore = null } = opts;
  if (
  ...
  if (spawnPoint?.y != null && Number.isFinite(spawnPoint.y)) {
    ...
    if (spawnBlockedAt(x, z, footY, bodyTop, config.radius, opts.colliders, gameCore)) {
      return null;
    }
    return { x, z, y: footY };
  }
...
export function pickRandomSpawnPosition(opts) {
  const { bounds, config, gameCore = null } = opts;
  return pickRandomSpawnPositionCore(gameCore, {
    bounds,
    radius: config.radius,
    margin: config.spawnMargin,
    targets: opts.targets,
    skip: opts.skip,
    validate: (x, z) => validateSpawnAt(x, z, opts),
  });
}
...
export function updateTargetsRepair(targets, dt, gameCore) {
  for (const mesh of targets) {
    if (!mesh.visible || mesh.userData.health <= 0) continue;
    const ud = mesh.userData;
    const rate = ud.repairPerSecond ?? DEFAULT_TARGET_CONFIG.repairPerSecond;
    const repair = requireWasmMethod(gameCore, "tickTargetRepair")(
      dt,
      ud.health,
      ud.maxHealth,
      ud.repairCooldown ?? 0,
...
function createRagdoll(
  ...
  const impulse = gameCore
    ...
    : null;
  const deathDir = impulse?.deathDir ?? resolveDeathToppleDir(bulletDir, zoneId);
  const toppleSeverity = impulse?.toppleSeverity ?? hitZoneToppleSeverity(zoneId);
  ...
  const ragdoll = {
    ...
    core: {
      ...
      bounced: false,
      launchY: 0,
      launchVelY: impulse?.launchVelY ?? (LAUNCH_UP_VEL + Math.random() * 1.5) * profile.launchMul * blastKnockback,
      launchVelX: impulse?.launchVelX ?? (bulletDir ? bulletDir.x * LAUNCH_BACK_VEL * profile.launchMul * blastKnockback : 0),
      launchVelZ: impulse?.launchVelZ ?? (bulletDir ? bulletDir.z * LAUNCH_BACK_VEL * profile.launchMul * blastKnockback : 0),
      airborne: true,
      spinAngle: 0,
      spinVel: impulse?.spinVel ?? (Math.random() < 0.5 ? -1 : 1) *
...
function clampToBounds(px, pz, r, bounds, gameCore) {
  return clampToBoundsCore(gameCore, px, pz, r, bounds);
}
...
function probeLimbContact(limb, frac, floorY, colliders, bounds, skipFloor, gameCore) {
  ...
  const bP = clampToBounds(
    _ragdollLimbEnd.x, _ragdollLimbEnd.z, limbR, bounds, gameCore,
  );
...
function updateRagdollPhysics(ragdoll, dt, colliders, floorY, bounds, floorHoles, callbacks) {
  ...
  let { x: oX, z: oZ } = pushOutOfWalls(
    core.originX, core.originZ, coreCollR, colliders, ragdollFootY, ragdollBodyTop, gameCore,
  );

  const midX = oX + sinD * slide;
  const midZ = oZ + cosD * slide;
  const midPush = pushOutOfWalls(midX, midZ, coreCollR, colliders, ragdollFootY, ragdollBodyTop, gameCore);
  oX += midPush.x - midX;
  oZ += midPush.z - midZ;

  const endX = oX + sinD * slide * 2;
  const endZ = oZ + cosD * slide * 2;
  const endPush = pushOutOfWalls(endX, endZ, coreCollR, colliders, ragdollFootY, ragdollBodyTop, gameCore);
  oX += endPush.x - endX;
  oZ += endPush.z - endZ;

  const clamped = clampToBounds(oX, oZ, coreCollR, bounds, gameCore);
...
export function updateDeathAnimations(targets, dt, onComplete, opts) {
  ...
  for (const mesh of targets) {
    ...
    if (!ud.fallingThroughHole && floorHoles.length && ud.deathSettled) {
      ...
      if (pointInFloorHole(px, pz, floorHoles, 0, opts?.gameCore ?? null)) {
