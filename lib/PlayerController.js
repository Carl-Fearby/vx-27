...
import { getDoorwayHeadroomCeilingY } from "./DoorwayWall.js";
...
export function createPlayerController(camera, bounds, floorY, options = {}) {
  ...
  const getKeyboardLookSpeed = options.getKeyboardLookSpeed ?? (() => 10);
  const getKeyboardLookEase = options.getKeyboardLookEase ?? (() => 10);
  ...
  function findCatwalkWalkBounds(footY, x, z) {
    ...
    // Union arena L-deck + room exterior deck so the approach over the dark
    // room stays walkable (arena-only union stopped at innerHalf ≈ −13.15).
    const unionAllCatwalkSlabs = onCatwalk;
    ...
    for (const surf of surfaces) {
      ...
      const isArenaDeck = Boolean(surf.arenaCatwalkDeck);
      const isRoomDeck = Boolean(surf.catwalkWalk);
      const include =
        (unionAllCatwalkSlabs && (isArenaDeck || isRoomDeck)) ||
      ...
      if (surf.edgeStandoff && isArenaDeck) {
  ...
  function computeResolvedWalkBounds(x, z, footY) {
    ...
    const inRoomAtFloor = inRoomFootprint && !onCatwalkHeight;
    const catwalkBounds =
      !inRoomAtFloor ? findCatwalkWalkBounds(footY, x, z) : null;

    if (catwalkBounds) {
      const es = catwalkBounds.edgeStandoff;
      // Union level bounds with every catwalk slab — intersection used to
      // clamp at innerHalf (−13.15) and block the room extension deck.
      minX = Math.min(bounds.minX + r, catwalkBounds.minX + r);
      maxX = Math.max(bounds.maxX - r, catwalkBounds.maxX - r);
      minZ = Math.min(bounds.minZ + r, catwalkBounds.minZ + r);
      maxZ = Math.max(bounds.maxZ - r, catwalkBounds.maxZ - r);
      if (es) {
    ...
    } else if (arenaBounds && !inRoomFootprint) {
    ...
    return { minX, maxX, minZ, maxZ, catwalkBounds, inRoom: inRoomFootprint };
  }
  ...
  function hasHeadroom(x, z, footY, desiredHeight) {
    ...
    for (const box of getColliders()) {
      ...
      const maxHalf = Math.max(box.halfX, box.halfZ) + reach;
      if (Math.abs(x - box.x) > maxHalf || Math.abs(z - box.z) > maxHalf) continue;
      if (rotatedBoxOverlapsCircle(box, x, z, reach)) {
        return false;
      }
    }
    return true;
