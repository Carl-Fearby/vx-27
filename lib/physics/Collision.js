...
export function pushCircleOutOfColliders(x, z, radius, colliders, opts = {}) {
  if (!opts.gameCore) {
    return pushCircleOutOfCollidersJs(x, z, radius, colliders, {
      footY: opts.footY,
...
export function spawnFootYContextFromLevel(level) {
  return {
    groundSurfaces: (level.groundSurfaces ?? []).filter(
      (surf) => !surf.arenaCatwalkDeck && !surf.roomCatwalkDeck,
    ),
