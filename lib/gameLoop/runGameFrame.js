...
import { buildSnowOccluderSlabs, updateSettledSnowMelt, updateSnow } from "@/lib/Snow.js";
import {
...
export function runGameFrame(ctx, now) {
  ...
  const rainWanted =
    ...
    outdoorWeather;
  const snowWanted = ctx.snowEnabledRef.current && outdoorWeather;
  const weatherTransition = ctx.weatherTransitionRef.current;
  tickWeatherTransition(weatherTransition, dt, {
    rainWanted,
    snowWanted,
    hasSettledSnow: (ctx.snow?.settled?.length ?? 0) > 0,
  });
  ...
  updateSnow(ctx.snow, ctx.camera, dt, now * 0.001, {
    fade: weatherTransition.snowFade,
    enclosed: inRoomBody,
    allowSettle: snowWanted && !inRoomBody,
  ...
  updateSettledSnowMelt(
    ctx.snow,
    dt,
    weatherTransition.snowMelt,
  );
