...
/** Match in-room / doorway shadow occluder state during GPU preload. */
 ...
 * require door alignment so the east deck edge does not pick up dark room
 * geometry (x ≈ 4.8, z ≈ −12). At the deck hole / door mouth the player stands
 * outside the room footprint — frustum-only, same as the arena floor rule.
 * Uses shell.visible; world+room share one geometry pass.
 *
 * Room sun occluders (invisible world-layer depth boxes) cast when the player is
 * inside at floor level, or when peeking at a doorway. Pillar-shell rooms also
 * cast while the shell is visible (not on the catwalk) so sun through the deck
 * hole is blocked without phantom shadows on the outdoor catwalk.
 *
...
export function updateRoomCulling(
  ...
  const atDoorMouth = canPeekIntoAttachedRoom(
    playerPos.x,
    playerPos.z,
    doorOpenings,
    attachWall,
  ...
  for (const cullable of cullables) {
    ...
    const blockSun = cullable.room.pillarShell
      ? insideAtFloor || (visible && !onCatwalk)
      : insideAtFloor || atDoorMouth;
    if (blockSun !== cullable.sunOccludersCast) {
      cullable.sunOccludersCast = blockSun;
      setSunOccludersCast(cullable.sunOccluders, blockSun);
    }
