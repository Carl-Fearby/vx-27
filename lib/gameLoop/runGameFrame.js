...
function syncGameCoreFrame(ctx, dt) {
  const core = ctx.gameCore;
  if (!core) return;
  const state = core.tickFrame({ dt });
...
function applyPlayerDeath(ctx, kind, now) {
  ctx.gameCore?.syncPlayerLives?.(ctx.playerLivesRef.current);
  const death =
    ctx.gameCore?.applyPlayerDeath?.(kind, now, DEATH_MIN_DISPLAY_MS) ??
    (() => {
      const reason =
        kind === "fall"
      ...
      const playerLives = Math.max(0, ctx.playerLivesRef.current - 1);
      return {
        died: true,
        reason,
        playerLives,
...
function applyPlayerRespawn(ctx, now) {
  const respawn =
    ctx.gameCore?.planPlayerRespawn?.(now, DEATH_FADE_MS) ??
    {
      canRespawn: true,
      playerHealth: 100,
  ...
  ctx.gameCore?.syncThrowableCounts?.(
...
export function runGameFrame(ctx, now) {
  ...
  if (dt > 0) syncGameCoreFrame(ctx, dt);
  ...
  const devTuningActive = ctx.devTuningActiveRef?.current === true;
  const aimHeld =
    !ctx.rebindActionRef.current &&
    isBindingDown(ctx.input, ctx.bindingsRef.current, "aim");
  let devTuneAimPreviewActive = false;
  if (devTuningActive) {
    devTuneAimPreviewActive =
  ...
  const aimTarget =
    aimHeld || devTuneAimPreviewActive ? 1 : 0;
  ...
  const canUseWeapons =
    ...
    !ctx.controlsOpenRef.current &&
    !ctx.consoleHackOpenRef.current &&
    !(devTuningActive && ctx.oilBarrelPlacementTuneEnabledRef?.current);
  ...
  if (!frozen && !ctx.consoleHackOpenRef.current) {
    ...
    if (
      ...
      ctx.gameCore?.setPlayerHealth(newHp);
    ...
    if (
      ...
      const death = applyPlayerDeath(ctx, "fall", now);
      if (death) {
    ...
    if (
      ...
      const deathKind = ctx.grenadeSuicideRef.current ? "suicide" : "enemy";
  ...
  if (devTuningActive) {
    ctx.tuningPreviewWasActive = true;
    updateWeaponTuningLaserPreview(ctx);
  } else if (ctx.tuningPreviewWasActive) {
    ctx.laserTracers?.hidePreview?.();
    ctx.tuningPreviewWasActive = false;
  }
  ...
  const pendingPrimaryWeaponTuneWeapon = devTuningActive
  ...
  if (ctx.grenadeHeld && !gDown) {
    ...
    if (!frozen && canThrowSecondary) {
      ...
      const throwResult =
        ctx.gameCore?.tryThrowThrowable(throwKind, GRENADE_THROW_COOLDOWN_SEC) ??
        (() => {
          if (throwingGrenade) {
            ctx.grenadeCountRef.current--;
  ...
  updateGrenades(
    ...
    },
    {
      ...
      onExplode: (pos, isFlashbang) => {
        ...
        const newHp =
          ctx.gameCore?.damagePlayer(60) ??
          Math.max(0, ctx.playerHealthRef.current - 60);
        ctx.playerHealthRef.current = newHp;
        ctx.setPlayerHealth(newHp);
        ctx.triggerPlayerHurtFeedback(ctx.hurtVignetteFlashEndRef, ctx.sounds);
      ...
      calculateGrenadeBlastHit: (distance, blastRadius, maxDamage, falloffPower) =>
        ctx.gameCore?.calculateGrenadeBlastHit(
          distance,
          blastRadius,
          maxDamage,
          falloffPower,
  ...
  updateTargetsRepair(ctx.level.targets, dt, ctx.gameCore);
  updateLiveTargetsFloorHoles(
    ctx.level.targets,
    dt,
    ctx.level.floorY,
  ...
  updateHpOrbs(
    ...
    (value) => {
      const reward =
        ctx.gameCore?.applyPickupReward("hp", value, value, Number.POSITIVE_INFINITY) ??
        { playerHealth: ctx.playerHealthRef.current + value };
  ...
  updateAmmoDrops(
    ...
    (value, drop) => {
      ...
      const ammo =
        ctx.gameCore?.addWeaponRounds(ctx.activePrimaryId, value) ?? null;
      if (ammo) {
  ...
  updateLevelCollectibles(
    ...
    (value, drop, entry) => {
      ...
      if (kind === "hp") {
        const reward =
          ctx.gameCore?.applyPickupReward("hp", value ?? 0, 10, 100) ??
          {
        ...
        const reward =
          ctx.gameCore?.applyPickupReward("grenade", value ?? 0, 1, 100) ??
          { grenadeCount: ctx.grenadeCountRef.current + (value ?? 1) };
        ...
        const reward =
          ctx.gameCore?.applyPickupReward("flashbang", value ?? 0, 1, 100) ??
          { flashbangCount: ctx.flashbangCountRef.current + (value ?? 1) };
        ...
        const reward =
          ctx.gameCore?.applyPickupReward(
            "score",
            value ?? 0,
            SCORE_PACK_DEFAULT_VALUE,
            100,
          ...
          {
            value: value ?? SCORE_PACK_DEFAULT_VALUE,
            playerScore:
              ctx.playerScoreRef.current + (value ?? SCORE_PACK_DEFAULT_VALUE),
          };
        ...
        const ammo =
          ctx.gameCore?.addWeaponRounds(ctx.activePrimaryId, value ?? 10) ??
          null;
        if (ammo) {
          ctx.roundsInMagRef.current = ammo.rounds;
          ctx.spareMagsRef.current = ammo.spare;
  ...
  updateGrenadeDrops(
    ...
    (value) => {
      const reward =
        ctx.gameCore?.applyPickupReward("grenade", value, 1, 100) ??
        { grenadeCount: ctx.grenadeCountRef.current + value };
      ctx.grenadeCountRef.current = reward.grenadeCount;
      ctx.setGrenadeCount(ctx.grenadeCountRef.current);
      ctx.pickupFlashLayerRef.current?.show("grenade");
      ctx.sounds.playSupplyPickup();
