...
export const WALK_BOB_TUNING_VERSION = 5;
...
export const WALK_SPEED_DEFAULT = 5;
...
function migrateStoredTuning(parsed) {
  if (
    ...
    const simple = normalizeWalkBobSimple(parsed);
    const storedVersion =
      typeof parsed.version === "number" ? parsed.version : 0;
    if (storedVersion < 5 && Math.abs(simple.walkSpeed - 4) < 0.001) {
      simple.walkSpeed = WALK_SPEED_DEFAULT;
    }
    return simple;
  }
