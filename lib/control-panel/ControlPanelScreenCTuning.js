...
/** Surface C screen — day/night brightness + UV rotation. */

export const CONTROL_PANEL_SCREEN_DAY_BRIGHTNESS_KEY =
  "fps-control-panel-screen-day-brightness";
export const CONTROL_PANEL_SCREEN_NIGHT_BRIGHTNESS_KEY =
  "fps-control-panel-screen-night-brightness";

export const CONTROL_PANEL_SCREEN_DAY_BRIGHTNESS = 50;
export const CONTROL_PANEL_SCREEN_NIGHT_BRIGHTNESS = 21;
export const CONTROL_PANEL_SCREEN_DAY_BRIGHTNESS_MIN = 5;
export const CONTROL_PANEL_SCREEN_DAY_BRIGHTNESS_MAX = 50;
export const CONTROL_PANEL_SCREEN_NIGHT_BRIGHTNESS_MIN = 5;
export const CONTROL_PANEL_SCREEN_NIGHT_BRIGHTNESS_MAX = 40;
...
export const CONTROL_PANEL_SCREEN_ROT_U_DEFAULT = 175;
export const CONTROL_PANEL_SCREEN_ROT_V_DEFAULT = 5;

function clampNightness(nightness) {
  return Math.min(1, Math.max(0, nightness ?? 0));
}

function clampBrightness(value, min, max, fallback) {
...
export function loadControlPanelScreenBrightness() {
  return loadControlPanelScreenDayBrightness();
}

export function saveControlPanelScreenDayBrightness(value) {
  if (typeof localStorage === "undefined") return;
  localStorage.setItem(
    CONTROL_PANEL_SCREEN_DAY_BRIGHTNESS_KEY,
    String(
      clampBrightness(
        value,
        CONTROL_PANEL_SCREEN_DAY_BRIGHTNESS_MIN,
        CONTROL_PANEL_SCREEN_DAY_BRIGHTNESS_MAX,
        CONTROL_PANEL_SCREEN_DAY_BRIGHTNESS
      )
    )
  );
...
export function saveControlPanelScreenNightBrightness(value) {
  if (typeof localStorage === "undefined") return;
  localStorage.setItem(
    CONTROL_PANEL_SCREEN_NIGHT_BRIGHTNESS_KEY,
    String(
      clampBrightness(
        value,
        CONTROL_PANEL_SCREEN_NIGHT_BRIGHTNESS_MIN,
        CONTROL_PANEL_SCREEN_NIGHT_BRIGHTNESS_MAX,
        CONTROL_PANEL_SCREEN_NIGHT_BRIGHTNESS
      )
    )
  );
...
export function saveControlPanelScreenBrightness(value) {
  saveControlPanelScreenDayBrightness(value);
}
...
export function resolveControlPanelScreenBrightness(nightness = 0) {
  ...
  const day = loadControlPanelScreenDayBrightness();
  const night = loadControlPanelScreenNightBrightness();
