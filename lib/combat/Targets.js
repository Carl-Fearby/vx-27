...
function headshotDamageFor(mesh, profile) {
...
function bodyDamageFor(profile, zoneId, zoneMult = 1) {
  return resolveBodyZoneDamage(profile, zoneId, zoneMult);
}

function zoneDamage(mesh, hitPoint, bulletDir, weaponId = "rifle") {
  ...
  if (frac < 0.08) {
    return { zone: "head", damage: headshotDamageFor(mesh, profile) };
  }
  ...
  if (bulletDir) {
    ...
    if (frac >= armZone.fromTop && frac <= armZone.toTop) {
      ...
      for (const sign of [-1, 1]) {
        ...
        if (dist <= armR) {
          return {
            zone: "arm",
            damage: bodyDamageFor(profile, "arm", armZone.mult),
          };
        }
      }
    }
      ...
      if (LEG_ZONE_IDS.has(zone.id)) {
        ...
        for (const sign of [-1, 1]) {
          ...
          if (dist <= legR) {
            return {
              zone: zone.id,
              damage: bodyDamageFor(profile, zone.id, zone.mult),
            };
          }
        }
        continue;
    ...
    if (fb) {
      return {
        zone: fb.id,
        damage: bodyDamageFor(profile, fb.id, fb.mult),
      };
    }
    return { zone: "body", damage: bodyDamageFor(profile, "body", 1) };
  }
...
export function applyTargetHit(mesh, hitPoint, bulletDir, weaponId = "rifle") {
  const ud = mesh.userData;
  const profile = getWeaponDamageProfile(weaponId);
  const result = hitPoint
    ? zoneDamage(mesh, hitPoint, bulletDir, weaponId)
    : { zone: "body", damage: bodyDamageFor(profile, "body", 1) };
