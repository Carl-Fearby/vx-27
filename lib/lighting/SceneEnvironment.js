...
/** @returns {[THREE.Light, boolean][]} */
function suppressOutdoorShadowMaps(outdoorShadowLights) {
  /** @type {[THREE.Light, boolean][]} */
  const saved = [];
  if (!outdoorShadowLights) return saved;
  for (const light of outdoorShadowLights) {
    if (!light?.castShadow) continue;
    if (!light.isDirectionalLight && !light.isSpotLight) continue;
    saved.push([light, true]);
    light.castShadow = false;
  }
  return saved;
}

function restoreOutdoorShadowMaps(saved) {
  for (const [light, castShadow] of saved) {
 ...
 * block outdoor lights from room meshes. Sky billboards use depthTest:false on
 * SKY_LAYER and render in a dedicated pass before world geometry composites on
 * top (catwalk ceilings, room shells). Room interiors use a third pass on
 * ROOM_INTERIOR_LAYER (PCF only — no mid-frame VSM toggle on r182+).
 *
 * @param {THREE.WebGLRenderer} renderer
 * @param {THREE.Scene} scene
...
export function renderSceneWithLayeredLighting(renderer, scene, camera, options = {}) {
  ...
  const outdoorShadowLights = options.outdoorShadowLights ?? null;
  ...
  try {
    ...
    if (!skipRoomPass) {
      const savedOutdoorShadow = suppressOutdoorShadowMaps(outdoorShadowLights);
      renderer.autoClear = false;
      camera.layers.set(ROOM_INTERIOR_LAYER);
      renderer.render(scene, camera);
      restoreOutdoorShadowMaps(savedOutdoorShadow);
    }
  } finally {
...
export function resetRendererShadowPipeline(renderer) {
