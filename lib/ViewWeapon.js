...
import { pinLightToLayers, setViewmodelLayer, setWorldLayer, WORLD_LAYER, VIEWMODEL_LAYER, ROOM_INTERIOR_LAYER } from "./LightingLayers.js";
...
export async function loadViewWeapon(
  ...
  return {
    ...
    /** Prime blue + green muzzle flash lights so first shots don't hitch. */
    async warmupMuzzleFlash(renderer, scene, camera, opts = {}) {
      if (!renderer || !scene || !camera) return;
...
/** Blue rifle bolt vs green overload bolt when HP > 100. */
export const LASER_PALETTES = {
  normal: { core: 0x66eeff, muzzle: 0x66ccff },
...
/** Remove a warmup bolt from the scene without disposing shared pool materials. */
export function disposeWarmupBulletBolt(bullet) {
  bullet?.mesh?.parent?.remove(bullet.mesh);
}

/**
 * Pre-compile bullet materials through gameplay render pipeline.
 * Keeps bullets in scene longer to ensure full shader compilation.
 * @param {object} bulletPool - From createBulletPool()
 * @param {THREE.Scene} scene
 * @param {THREE.Camera} camera
 * @param {THREE.WebGLRenderer} renderer
 * @param {function} renderGameplayFrame - Full render pipeline (e.g. layeredRenderWithRoom)
...
export async function warmupBulletMaterialsGpu(
  ...
  if (!bulletPool || !renderer || !scene || !camera || !renderGameplayFrame) return;

  // Spawn test bullets and keep them in scene for compilation
  const testBullets = [];
  const positions = [
  ...
  // Spawn blue bullet
  const blueBullet = bulletPool.spawn(scene, positions[0], new THREE.Vector3(0, 0, -1));
  blueBullet.mesh.position.copy(positions[0]);
  testBullets.push(blueBullet);

  // Spawn green bullet
  const greenBullet = bulletPool.spawn(
    scene,
    positions[1],
    new THREE.Vector3(0, 0, -1),
    { radioactive: true }
  );
  greenBullet.mesh.position.copy(positions[1]);
  testBullets.push(greenBullet);

  // Third bullet (blue) for extra passes
  const blueBullet2 = bulletPool.spawn(scene, positions[2], new THREE.Vector3(0, 0, -1));
  blueBullet2.mesh.position.copy(positions[2]);
  testBullets.push(blueBullet2);

  // Render through full gameplay pipeline multiple times
  // This ensures the materials are compiled in the exact render context they'll be used in
  for (let i = 0; i < 12; i++) {
    renderGameplayFrame(renderer, scene, camera);
    await new Promise((resolve) => requestAnimationFrame(resolve));
  }

  // Clean up test bullets (remove from scene)
...
export function createBulletPool() {
  const boltGeo = createLaserGeometry(LASER_LENGTH, 0.028, 0.04);
  ...
  return {
    spawn(scene, origin, direction, { radioactive = false } = {}) {
      const dir = direction.clone().normalize();
      const root = new THREE.Group();
      const core = new THREE.Mesh(
        boltGeo,
        radioactive ? radioactiveMat : normalMat
      );
