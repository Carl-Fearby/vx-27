...
import { spawnAmmoDrop } from "@/lib/pickups/AmmoCrate.js";
  ...
  function scheduleKillDrops(deathPos, zone) {
    ...
    if (DEV_DROP_ALL_REWARDS) {
      dropAt(rndAngle, hpDelay, (p) =>
        ctx.hpOrbs.push(spawnHpOrb(ctx.scene, p, ctx.level.floorY)),
      );
      dropAt(rndAngle + Math.PI * 0.66, ammoDelay, (p) =>
        ctx.ammoDrops.push(spawnAmmoDrop(ctx.scene, p, ctx.level.floorY)),
      );
      dropAt(rndAngle + Math.PI * 1.33, grenDelay, (p) =>
        ctx.grenadeDrops.push(spawnGrenadeDrop(ctx.scene, p, ctx.level.floorY)),
      );
      return;
    }
