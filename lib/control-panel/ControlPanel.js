...
/**
 * World pass always; room consoles also join the interior pass so the room
 * lighting pass redraws them without outdoor sun.
 * @param {THREE.Mesh} mesh
 * @param {string | null} roomId
 */
function applyControlPanelMeshLayers(mesh, roomId) {
  setWorldLayer(mesh);
  if (roomId) {
...
export function applyControlPanelRenderLayers(group, roomId = null) {
  ...
  group.traverse((obj) => {
    ...
    applyControlPanelMeshLayers(obj, id);
  });
}
...
export function applyControlPanelGroupMaterials(group, options = {}) {
  ...
  const inRoom = !!group.userData?.roomId;
  const brightnessScale = inRoom ? CONTROL_PANEL_ROOM_BRIGHTNESS_SCALE : 1;
  ...
  group.traverse((obj) => {
    ...
    for (const material of materials) {
      ...
      applyControlPanelRoomHullMaterial(material);
    }
  });
}
