 ...
 * block outdoor lights from room meshes. Sky uses depthTest:false and low
 * renderOrder so one world pass draws it behind geometry; room interiors use
 * a second pass on ROOM_INTERIOR_LAYER (same PCF shadow map — no mid-frame
 * VSM toggle; r182's modernized shadows corrupt the next world pass when type
 * is swapped between passes).
 *
 * @param {THREE.WebGLRenderer} renderer
 * @param {THREE.Scene} scene
...
export function renderSceneWithLayeredLighting(renderer, scene, camera, options = {}) {
  ...
  try {
    ...
    if (hasSky) {
      // Sky uses depthTest:false and renderOrder -1000, so a single world pass
      // draws it behind geometry. Avoid toggling mesh.visible — that left the
      // level invisible when a pass failed under r182's modernized shadows.
