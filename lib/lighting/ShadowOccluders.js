...
/**
 * Rotated ramp slab — blocks directional sun through stair tread gaps with one
 * shadow-map draw instead of many thin tread casters.
 * @param {THREE.Object3D} group
 * @param {number} width Width along local X (m)
 * @param {number} run Horizontal run before rotation (local Z, m)
 * @param {number} rise Vertical rise (m)
 * @param {number} [thickness] Slab thickness perpendicular to the slope (m)
 * @param {number} [startZ] Local Z where the ramp begins (m)
 * @param {number} [startY] Local Y at the start of the ramp (m)
 */
export function addShadowOccluderSlopedRamp(
  group,
  width,
  run,
  rise,
  ...
  if (run < 0.05 || rise < 0.05) return null;
  ...
  mesh.position.set(0, startY + rise / 2, startZ + run / 2);
