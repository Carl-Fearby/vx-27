...
import { loadGpuPreloadEnabled } from "./GpuPreloadTuning.js";
...
export function getGpuPreloadLoadLabel(enabled = isGpuPreloadEnabled()) {
  return enabled ? GPU_PRELOAD_LOAD_LABEL : GPU_PRELOAD_SKIPPED_LABEL;
}

/** @returns {boolean} */
export function isGpuPreloadEnabled() {
  return GPU_PRELOAD_ENABLED && loadGpuPreloadEnabled();
}
...
export async function preloadGameGpu({
  ...
  if (!isGpuPreloadEnabled() || !renderer || !scene || !camera) return;
