...
const SHELF_EMISSIVE_COLOR = 0x3db8ff;
...
export function updateControlPanelShelfDBrightness(brightness) {
  applyControlPanelShelfDBrightness(_shelfMat, { brightness });
}
...
export function preloadControlPanelShelfDTextures() {
  ...
  _preloadPromise = (async () => {
    try {
      ...
      _shelfMat.polygonOffset = true;
      _shelfMat.polygonOffsetFactor = -2;
      _shelfMat.polygonOffsetUnits = -2;
...
export function attachControlPanelShelfD(group) {
  ...
  const mesh = new THREE.Mesh(geo, _shelfMat);
