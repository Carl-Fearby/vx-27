  ...
  return (
    ...
    >
      ...
      <p className="stairTuneHint">
        Fixed flight · {STAIRS_STEP_COUNT} steps · {STAIRS_WIDTH.toFixed(2)} m wide · rise {STAIRS_EFFECTIVE_TOTAL_RISE.toFixed(2)} m ({STAIRS_TOTAL_RISE.toFixed(2)} m steps + top) · run {STAIRS_TOTAL_RUN.toFixed(2)} m · anchor = bottom
        of first tread · 0° faces +Z · floor Y ≈ {floorDeckY.toFixed(3)} m · catwalk ≈ {catwalkDeckY.toFixed(3)} m · ± nudge {STAIR_NUDGE_STEP} m / {STAIR_ROTATION_NUDGE}°
      </p>
    </div>
  );
}
