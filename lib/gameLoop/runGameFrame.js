...
import {
  getRifleShopPrompt,
  isPlayerWithinRifleShopRange,
  pickRifleShopUnderCrosshair,
  tryPurchaseRifle,
} from "@/lib/weapons/RifleShop.js";
import { tickPendingWallWeaponEquip } from "@/lib/weapons/WallWeaponShop.js";
...
export function runGameFrame(ctx, now) {
  ...
  const aimTarget =
    aimHeld || rifleRoundDisplayAimTabActive || pistolRoundDisplayAimTabActive
      ? 1
      : 0;
  ...
  if (ctx.rifleShopRef?.current?.visible && ctx.rifleShopRef.current.group) {
    ctx.hitRaycaster.setFromCamera(ctx.screenCenter, ctx.camera);
    rifleShopTarget = pickRifleShopUnderCrosshair(
      ctx.hitRaycaster,
      ctx.rifleShopRef.current.group,
    );
  }
  if (
    !ctx.consoleHackOpenRef.current &&
  ...
  const showRifleShopPrompt =
    Boolean(rifleShopTarget) &&
    !doorTarget &&
    isPlayerWithinRifleShopRange(ctx);
  let rifleShopAffordable = false;
  if (showRifleShopPrompt) {
    ctx.rifleShopRef?.current?.syncWallPrice?.(ctx);
    rifleShopAffordable = getRifleShopPrompt(ctx).affordable;
  }
  const showRifleShopPurchaseIcon =
    showRifleShopPrompt && canInteract && rifleShopAffordable;
  if (
    !frozen &&
    !ctx.rebindActionRef.current &&
    !ctx.settingsOpenRef.current &&
  ...
  if (touchMode) {
    const showDoor = Boolean(doorTarget) || Boolean(rifleShopTarget);
  ...
  if (
    ...
    showRifleShopPurchaseIcon &&
    (wasBindingPressed(ctx.input, ctx.bindingsRef.current, "interact") ||
      ctx.input.consumeShoot())
  ) {
    tryPurchaseRifle(ctx, now);
  }
  ...
  if (ctx.screenCrosshairRef.current) {
    ...
    crosshairUpdateOptions.purchaseTarget = showRifleShopPurchaseIcon;
  ...
  const rifleShopBlocksFire = showRifleShopPurchaseIcon;
