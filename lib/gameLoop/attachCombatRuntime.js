...
export function attachCombatRuntime(ctx) {
  ...
  function tryReload(force) {
    syncWeaponAmmoToCore(ctx.activePrimaryId);
    const ammo = requireWasmMethod(ctx.gameCore, "tryReloadWeapon")(
      ctx.activePrimaryId,
      Boolean(force),
    );
  ...
  function processWeaponFire(dt) {
    ...
    const core = requireWasmMethod(ctx.gameCore, "tickWeaponFire");
    const burstActive = requireWasmMethod(ctx.gameCore, "isWeaponBurstActive")();
    const shootHeld = ctx.input.isShootHeld();
    const acceptsPressedInput =
      !burstActive && (mode !== "auto" || shootHeld);
    const shootPressed = acceptsPressedInput
      ? ctx.input.consumeShoot()
      : false;

    syncWeaponAmmoToCore(ctx.activePrimaryId);
    const fire = core({
      weaponId: ctx.activePrimaryId,
      mode,
      dt,
      shootPressed,
      shootHeld,
      burstShotCount: BURST_SHOT_COUNT,
      burstInterval: BURST_INTERVAL,
      autoFireInterval: AUTO_FIRE_INTERVAL,
    });
    applyWeaponAmmoOutput(ctx.activePrimaryId, fire);
    if (fire.reloaded) ctx.sounds.playSupplyPickup();
    if (fire.shotsFired > 0 || fire.reloaded) {
      ctx.scheduleGameplayHudSyncRef.current();
    }
    for (let i = 0; i < fire.shotsFired; i += 1) {
      applyScheduledRound();
    }
  }
