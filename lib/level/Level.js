...
function syncRoomCatwalkDeck(
  ...
  for (const room of rooms) {
    ...
    if (lastMesh) {
      ...
      lastMesh.userData.arenaCatwalkDeck = true;
...
function addArenaCeilingDeckPiece(
  ...
  ceiling.userData.arenaCatwalkDeck = true;
  // Solid deck: cast the footprint onto the arena floor in sun shadow space.
  // The top face points at the sun so it never self-shadows; light cannot pass
...
export function createLevelFromArena(scene, arena, textureLibrary = null, gameCore = null) {
  ...
  // Cast so the deck occludes the wall base — without this there is no contact
  // shadow and a sun-bright strip runs along the bottom of every perimeter wall.
  floor.userData.shadowCast = true;
  floor.userData.shadowReceive = true;
  floor.userData.rainWetSurface = true;
  group.add(floor);
  ...
  function rebuildArenaCeiling(stairPlacement) {
    ...
    // Shadow-only virtual ceiling extension — invisible to every camera pass
    // (lives on SHADOW_CASTER_LAYER which no camera ever renders) but included
    // in the sun's shadow map so diagonal rays at the arena corners are blocked.
    // The physical ceiling's z-extent (~±14.75 m) is too narrow for the sun's
    // azimuth/elevation; extending by 12 m each side gives ~±26 m coverage which
    // analytically closes the gap at any realistic sun angle.
    {
      const extSize = ceilingFullWidth + 24;
      const shadowGeo = new THREE.PlaneGeometry(extSize, extSize);
      const shadowMat = new THREE.MeshBasicMaterial({
        colorWrite: false,
        depthWrite: false,
      });
      const shadowMesh = new THREE.Mesh(shadowGeo, shadowMat);
      shadowMesh.name = "arena_shadow_ceiling_ext";
      shadowMesh.rotation.x = -Math.PI / 2;
      shadowMesh.position.y = ceilingBottomY + CEILING_THICKNESS * 0.5;
      shadowMesh.layers.set(SHADOW_CASTER_LAYER);
      shadowMesh.castShadow = true;
      shadowMesh.receiveShadow = false;
      ceilingGroup.add(shadowMesh);
    }
  }
  ...
  refreshOilBarrelMaterials(getOilBarrelTuning(), group);
