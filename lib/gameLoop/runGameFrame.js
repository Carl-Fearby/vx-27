...
export function runGameFrame(ctx, now) {
  ...
  const showRifleShopInteract = showRifleShopPrompt && canInteract;
  const showRifleShopPurchaseIcon =
    showRifleShopInteract && rifleShopAffordable;
  if (
    !frozen &&
    !ctx.rebindActionRef.current &&
    !ctx.settingsOpenRef.current &&
  ...
  if (ctx.screenCrosshairRef.current) {
    ...
    crosshairUpdateOptions.purchaseTarget = showRifleShopInteract;
  ...
  const rifleShopBlocksFire = showRifleShopInteract;
