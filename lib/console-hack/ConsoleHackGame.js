...
/** Chance of any ammo mag reward (pistol or rifle, not both). */
...
export const HACK_REWARD_CHANCES = {
  credits: 1,
  /** @deprecated use HACK_REWARD_AMMO_MAG_CHANCE — kept for older call sites */
...
export function rollHackRewards(seed, salt = 0) {
  ...
  const ammoHit = rand() < HACK_REWARD_AMMO_MAG_CHANCE;
  const pistolMag = ammoHit && rand() < 0.5;
  return {
    credits: HACK_REWARD_CREDITS,
    pistolAmmo: pistolMag ? HACK_REWARD_PISTOL_MAG_ROUNDS : 0,
    rifleSpareMag: ammoHit && !pistolMag ? HACK_REWARD_RIFLE_SPARE_MAGS : 0,
