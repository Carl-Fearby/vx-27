...
export function runGameFrame(ctx, now) {
  ...
  if (deathState) {
    if (deathState.gameOver) {
      ...
      if (canRespawn && ctx.input.consumeShoot()) {
        ...
        ctx.gameCore?.setPlayerHealth(100);
  ...
  if (!frozen && !ctx.consoleHackOpenRef.current) {
    ...
    if (
      ...
      ctx.playerHealthRef.current = 0;
      ctx.gameCore?.setPlayerHealth(0);
      ctx.setPlayerHealth(0);
      const gameOver = ctx.playerLivesRef.current <= 0;
      ctx.deathStateRef.current = {
        reason,
        respawned: false,
        gameOver,
  ...
  if (ctx.grenadeHeld && !gDown) {
    ...
    if (!frozen && canThrowSecondary) {
      ...
      ctx.gameCore?.setGrenadeCooldown(GRENADE_THROW_COOLDOWN_SEC);
      ctx.updateGrenadeCooldownHud();
    }
  }
  ...
  updateHpOrbs(
    ...
    (value) => {
      ...
      ctx.gameCore?.setPlayerHealth(ctx.playerHealthRef.current);
      ctx.player.syncStaminaMaxFromHp();
