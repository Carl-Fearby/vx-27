...
import { disposeWarmupBulletBolt } from "./ViewWeapon.js";
...
export async function warmupGameGpu({
  ...
  for (const mesh of levelCollectibleMeshes) {
    ...
    if (bulletPool) {
      ...
      disposeWarmupBulletBolt(bullet);
      ...
      await warm(radBullet.mesh, boltWarmOpts);
      disposeWarmupBulletBolt(radBullet);
    }
