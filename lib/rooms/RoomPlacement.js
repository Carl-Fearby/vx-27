...
/**
 * Viewmodel indoor zone — strict footprint plus a small pad on the arena-facing mouth
 * only (north: +Z, south: −Z). Does not use doorway peek bands or frustum edge distance.
 */
export function isInsideAttachedRoomForViewmodel(
  x,
  z,
  room,
  arenaHalf,
  attachWall,
  arenaWallThickness = 0.5,
  mouthPad = ROOM_VIEWMODEL_MOUTH_PAD
) {
  const halfW = room.width / 2;
  const halfD = room.depth / 2;
  const roomCenterZ = getAttachedRoomCenterZ(
  ...
  let zMin = roomCenterZ - halfD;
  let zMax = roomCenterZ + halfD;
  if (attachWall === "north") zMax += mouthPad;
  else zMin -= mouthPad;
  return (
    x >= room.centerX - halfW &&
    x <= room.centerX + halfW &&
...
/** @param {import("../level/loadArena.js").ArenaRoom[]} rooms */
export function isPointInsideAnyRoomForViewmodel(
  x,
  z,
  rooms,
  arenaHalf,
  ...
  for (const room of rooms) {
    if (
      isInsideAttachedRoomForViewmodel(
        x,
        z,
        room,
        arenaHalf,
...
/**
 * @deprecated Viewmodel zone uses {@link ROOM_VIEWMODEL_MOUTH_PAD} on the mouth edge only.
 */
export const ATTACHED_ROOM_VIEWMODEL_EDGE_DIST = 1.5;

/**
 * How far past the walk footprint mouth (toward arena) still counts as indoor for the
 * viewmodel layer — matches door plane (~0.35 m), not doorway peek / frustum bands.
 */
export const ROOM_VIEWMODEL_MOUTH_PAD = 0.35;
...
/**
 * Viewmodel + barrel-fire layers: feet inside room mouth pad only.
 * Room-pass culling stays camera-based ({@link updateRoomCulling}).
 * Elevated catwalk always uses outdoor sun/moon on the viewmodel layer.
 * Ignores doorway peek bands and frustum “near shell” distance (those caused early black guns).
 *
 * Prefer {@link import("../lighting/LightingZones.js").resolveViewmodelLightingZone} for
 * container interiors and per-zone ambient recipes.
 */
export function resolveViewmodelIndoorLightingZone(
  ...
  attachWall,
  arenaWallThickness = 0.5,
  _edgeDist = ATTACHED_ROOM_VIEWMODEL_EDGE_DIST,
  footY = null,
  catwalkDeckY = null
) {
  if (isOnCatwalkDeck(footY, catwalkDeckY)) return false;
  return isPointInsideAnyRoomForViewmodel(
    x,
    z,
    rooms,
    arenaHalf,
