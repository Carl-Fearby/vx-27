...
export function updateRain(rain, camera, dt, opts = {}) {
  ...
  const intensity = opts.intensity ?? DEFAULT_RAIN_INTENSITY;
  const visibleCount = active
    ...
    : 0;
  rain.root.visible = active && visibleCount > 0;
  ...
  const fallMul = 0.4 + intensity * 0.52;
  const outdoorOpacity = Math.min(0.92, 0.06 + intensity * 0.17);
  const vistaOpacity = Math.min(0.85, 0.05 + intensity * 0.14);
