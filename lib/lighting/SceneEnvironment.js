...
export const SUN_SHADOW_MAP_SIZE = 1536;
...
export function syncOilBarrelFireLightLayers(barrelFireLights, inRoom) {
  ...
  for (const light of barrelFireLights) {
    ...
    const roomId = light.userData?.roomId;
    if (!roomId) {
      pinLightToLayers(
        light,
        WORLD_LAYER,
...
export function createOutdoorLights(scene, options = {}) {
  ...
  /** Shadowless fill — kept low under a roof so corners stay in shadow. */
  const fill = new THREE.DirectionalLight(0xc8d4e8, sheltered ? 0.05 : 0.48);
  ...
  const westFill = new THREE.DirectionalLight(0xb8c8e0, sheltered ? 0.034 : 0.32);
