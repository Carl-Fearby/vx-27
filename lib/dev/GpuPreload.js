...
import { loadGpuPreloadMode } from "./GpuPreloadTuning.js";
...
export const GPU_PRELOAD_THREE_COMPILE_LABEL = "Three.js GPU compile";
export const GPU_PRELOAD_TORCH_RAISING_LABEL = "Torch raising (night)";
export const GPU_PRELOAD_TORCH_SHADOW_LABEL = "Torch shadows (night)";
export const GPU_PRELOAD_TORCH_LOWERING_LABEL = "Torch lowering (night)";
...
/** @param {import("./GpuPreloadTuning.js").GpuPreloadMode} [mode] */
export function getGpuPreloadLoadLabel(mode = loadGpuPreloadMode()) {
  if (mode === "off") return GPU_PRELOAD_SKIPPED_LABEL;
  if (mode === "three") return GPU_PRELOAD_THREE_COMPILE_LABEL;
  return GPU_PRELOAD_LOAD_LABEL;
}

/** @returns {import("./GpuPreloadTuning.js").GpuPreloadMode} */
export function getGpuPreloadMode() {
  return GPU_PRELOAD_ENABLED ? loadGpuPreloadMode() : "off";
}

/** @returns {boolean} */
export function isGpuPreloadEnabled() {
  return getGpuPreloadMode() !== "off";
}

/** @returns {boolean} */
export function isFullGpuPreloadEnabled() {
  return getGpuPreloadMode() === "full";
}
...
/** Full-res offscreen target — real GPU warm without painting the load canvas. */
let _torchWarmTarget = null;
const _torchWarmSize = new THREE.Vector2();
...
function renderGameplayFrame(ctx, skipRoomPass) {
  ...
  let prevTarget = null;
  if (ctx.torchOffscreenWarm) {
    const size = renderer.getDrawingBufferSize(_torchWarmSize);
    if (
      !_torchWarmTarget ||
      _torchWarmTarget.width !== size.x ||
      _torchWarmTarget.height !== size.y
    ) {
      _torchWarmTarget?.dispose();
      _torchWarmTarget = new THREE.WebGLRenderTarget(size.x, size.y, {
        depthBuffer: true,
      });
    }
    prevTarget = renderer.getRenderTarget();
    renderer.setRenderTarget(_torchWarmTarget);
  }
...
export async function preloadGameGpu({
  ...
  if (!isFullGpuPreloadEnabled() || !renderer || !scene || !camera) return;
...
/**
 * Three.js built-in shader precompile only — {@link THREE.WebGLRenderer.compileAsync}.
 * Uploads level textures, poses the spawn camera, then compiles programs.
 */
export async function preloadThreeJsCompileGpu({
  renderer,
  scene,
  camera,
  level,
  ...
  if (getGpuPreloadMode() !== "three" || !renderer || !scene || !camera) return;
  const epoch = _gpuPreloadEpoch;
  const ctx = { renderer, isActive, epoch };
  if (!gpuPreloadActive(ctx)) return;
...
/**
 * Re-bake night torch shadow shaders after pickup-shadow refresh (that pass
 * invalidates material program cache and was undoing the load-screen torch warm).
 */
export async function warmNightTorchShadows({
  renderer,
  scene,
  camera,
  level,
  ...
  primeDirectionalShadow = null,
  applyDayNightNightness = null,
  restoreDayNightNightness = null,
  getShadowFrameOpts = null,
  onPreloadLabel = null,
  frames = 10,
  isActive,
}) {
  if (!renderer || !scene || !camera || typeof weapon?.warmTorchShadowGpu !== "function") {
    return;
  }
  const epoch = _gpuPreloadEpoch;
  if (isActive?.() === false) return;
  ...
  ctx.torchBakePass = true;
  ctx.torchOffscreenWarm = true;
  applyDayNightNightness?.(1);
  primeDirectionalShadow?.();
  requestShadowMapUpdate(renderer);
  spawnLookAt(camera, spawnX, spawnEyeY, spawnZ, spawnYaw);
  applyZoneState(ctx, spawnX, spawnZ, spawnFootY);
  await compileScene(renderer, scene, camera, ctx);
  await weapon.warmTorchShadowGpu(renderer, scene, camera, {
    castingFrames: frames,
    renderFrame: () => {
      const { inRoomPass } = applyZoneState(ctx, spawnX, spawnZ, spawnFootY);
      renderGameplayFrame(ctx, !inRoomPass);
    },
  ...
  ctx.torchBakePass = false;
  ctx.torchOffscreenWarm = false;
  restoreDayNightNightness?.();
  primeDirectionalShadow?.();
  requestShadowMapUpdate(renderer);
  spawnLookAt(camera, spawnX, spawnEyeY, spawnZ, spawnYaw);
  applyZoneState(ctx, spawnX, spawnZ, spawnFootY);
