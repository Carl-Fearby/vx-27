...
const FLASHLIGHT_SHADOW_NIGHT_SCALE = 0.55;
...
export function flashlightShadowEligibleNightness(nightness) {
  return flashlightNightScale(nightness) >= FLASHLIGHT_SHADOW_NIGHT_SCALE;
}
...
export function createWeaponFlashlight(scene) {
  ...
  /** Shadow shaders/maps baked during load — F-key only flips castShadow back on. */
  let shadowMapsPrimed = false;
  ...
  function markShadowMapsPrimed(renderer) {
    shadowMapsPrimed = true;
    hotspot.castShadow = true;
    if (hotspot.shadow) hotspot.shadow.autoUpdate = false;
   ...
   * @param {boolean} on
   * @param {{ nightness?: number }} [opts]
   */
  function sync(origin, direction, on, opts = {}) {
    const scale = on ? flashlightNightScale(opts.nightness ?? 1) : 0;
    hotspot.intensity = HOTSPOT_INTENSITY * scale;
    spill.intensity = SPILL_INTENSITY * scale;
    fill.intensity = FILL_INTENSITY * scale;

    const casting = on && scale > 0.05;
    const shadowCasting = casting && scale >= FLASHLIGHT_SHADOW_NIGHT_SCALE;
    const nightShadowArmed =
      shadowMapsPrimed &&
      !shadowCasting &&
      flashlightShadowEligibleNightness(opts.nightness ?? 0);

    if (shadowMapsPrimed) {
      hotspot.castShadow = shadowCasting || nightShadowArmed;
      if (hotspot.shadow) {
      ...
      if (hotspot.shadow) hotspot.shadow.autoUpdate = shadowCasting;
    }
    wasShadowCasting = shadowCasting;

    beam.visible = casting;
    if (beam.material) {
      beam.material.opacity = 0.011 + 0.02 * scale;
    }
    if (!on || scale <= 0) return;
  ...
  function isCastingShadow() {
    return hotspot.castShadow && hotspot.intensity > 0.05;
  }
