...
export function runGameFrame(ctx, now) {
  ...
  const reticleWizardOpen = Boolean(ctx.rifleReticleWizardOpenRef?.current);
  const reticleWizardAds =
    reticleWizardOpen && ctx.rifleReticleTuningModeRef?.current === "ads";
  const weaponAimTarget = reticleWizardAds ? 1 : aimTarget;
  ...
  if (!frozen && !ctx.consoleHackOpenRef.current) {
    ...
    activeWeaponUpdateOptions.snapAim = reticleWizardAds;
    ...
    activeWeaponUpdateOptions.reticleTuning = ctx.crosshairTuningRef?.current;
    activeWeaponUpdateOptions.reticleTuningActive = Boolean(
      ctx.rifleReticleWizardOpenRef?.current,
    );
    activeWeaponUpdateOptions.reticleTuningMode =
      ctx.rifleReticleTuningModeRef?.current ?? "hip";
    ctx.weapon?.update(
      ctx.camera,
      weaponAimTarget,
      dt,
    ...
    for (const id of ["rifle", "pistol"]) {
      ...
      idleWeaponUpdateOptions.reticleTuning = ctx.crosshairTuningRef?.current;
      idleWeaponUpdateOptions.reticleTuningActive = Boolean(
        ctx.rifleReticleWizardOpenRef?.current,
      );
      idleWeaponUpdateOptions.reticleTuningMode =
        ctx.rifleReticleTuningModeRef?.current ?? "hip";
      w.update(
        ctx.camera,
        0,
  ...
  const reticuleBlend = ctx.weapon?.getSightReadyBlend?.() ?? aimBlend;
  const reticuleMotion = ctx.weapon?.getReticleMotion?.() ?? null;
  ...
  const standardCrossFadeWithAim = Boolean(
    activeWeaponCfg.viewOptions.standardCrossFadeWithAim,
  );
  if (ctx.screenCrosshairRef.current) {
    ...
    crosshairUpdateOptions.reticuleBlend = reticuleBlend;
    crosshairUpdateOptions.reticuleMotion = reticuleMotion;
    ...
    crosshairUpdateOptions.standardCrossFadeWithAim = standardCrossFadeWithAim;
  ...
  if (
    ...
    if (swap.allowed && nextId) {
      if (!ctx.primaryWeapons[nextId]) {
        ctx.ensurePrimaryWeaponLoaded?.(nextId)?.then?.((loadedWeapon) => {
          if (
            !loadedWeapon ||
            ctx.isDisposed?.() ||
            ctx.weaponSwap.isBusy() ||
            ctx.activePrimaryId === nextId
          ) {
            return;
          }
          ctx.persistActiveAmmo();
