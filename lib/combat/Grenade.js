...
function resolveFloorTop(x, z, refY, floorY, groundSupport) {
  const supportY = groundSupport
    ? sampleSupportYAt(x, z, refY, groundSupport)
    : floorY;
  return supportY + GRENADE_RADIUS;
}
...
function simulateTrajectory(
  ...
  for (let i = 0; i < MAX_SIM_STEPS; i++) {
    ...
    const floorTop = resolveFloorTop(pos.x, pos.z, pos.y, floorY, groundSupport);
    if (pos.y <= floorTop) {
      pos.y = floorTop;
      if (!landed) {
        landed = true;
...
export function updateGrenades(grenades, dt, scene, getLiveTargets, applyHitFn, startDeathFn, deathOpts) {
  ...
  for (let i = grenades.length - 1; i >= 0; i--) {
    ...
    if (!g.detonated) {
      ...
      const supportY = groundSupport
        ? sampleSupportYAt(px, pz, py, groundSupport)
        : g.floorY;
      const overHole =
        supportY <= g.floorY + 0.02 &&
        pointInFloorHole(px, pz, g.floorHoles, GRENADE_RADIUS);
      if (overHole) beginHoleFall(g);

      // Floor bounce — only resolve a landing while airborne (avoids rest stutter)
      const floorTop = supportY + GRENADE_RADIUS;
