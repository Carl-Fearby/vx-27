...
/** Plane UV (0=bottom, 1=top) where smoke starts fading out. */
const VIDEO_TOP_FADE_START = 0.48;
/** Plane UV where the flame is fully transparent at the top. */
const VIDEO_TOP_FADE_END = 0.9;
...
const INTERIOR_VIDEO_SHADER_GEN = 12;
/** Upload video textures every N visible frames — halves GPU upload cost. */
const VIDEO_TEXTURE_UPLOAD_INTERVAL = 2;
let _videoUploadFrame = 0;
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Plane UV — not barrel Y. Centre offset moves the mesh without updating
        // layout bounds; world-Y mapping smears the top video row vertically.
 ...
 */
function cloneInteriorVideoMaterial(template, innerRadius, clipTopY, tuning) {
...
function configureVideoTexture(tex, video, { colorSpace = THREE.SRGBColorSpace } = {}) {
  ...
  _loadPromise = new Promise((resolve, reject) => {
    ...
    const finish = () => {
      ...
      requestAnimationFrame(() => {
        if (_colorTex) _colorTex.needsUpdate = true;
        if (_alphaTex) _alphaTex.needsUpdate = true;
      });
 ...
 * @returns {{ fireLights: THREE.Object3D[], videoMeshes: THREE.Mesh[] }}
 */
export function buildOilBarrelRuntimeIndex(root) {
  /** @type {THREE.Object3D[]} */
  const fireLights = [];
  /** @type {THREE.Mesh[]} */
  const videoMeshes = [];
  if (!root) return { fireLights, videoMeshes };
  root.traverse((obj) => {
    if (obj.name === "oil_barrel" && obj.isGroup) {
      const fireRig = obj.getObjectByName("oil_barrel_fire_lights");
      const fireLight = fireRig ?? obj.getObjectByName("oil_barrel_fire_light");
      if (fireLight) fireLights.push(fireLight);
    }
    if (obj.isMesh && obj.name === "oil_interior_video") {
      videoMeshes.push(obj);
    }
  });
  return { fireLights, videoMeshes };
}

/**
 * Per-frame video update + yaw billboards + cylindrical clip matrix.
 * @param {THREE.Camera} [camera]
 * @param {THREE.Object3D | { videoMeshes: THREE.Mesh[] }} [rootOrIndex]
 */
export function tickOilBarrelInteriorVideo(camera, rootOrIndex) {
  if (!camera || !rootOrIndex) return;

  const indexed = rootOrIndex.videoMeshes;
  let needsVideoUpload = false;

  const tickMesh = (obj) => {
    if (!obj.visible) return;
    needsVideoUpload = true;
  ...
  if (indexed) {
    ...
    rootOrIndex.traverse((obj) => {
      if (!obj.isMesh || obj.name !== "oil_interior_video") return;
      tickMesh(obj);
    });
  }

  if (!needsVideoUpload) return;
  _videoUploadFrame += 1;
  if (_videoUploadFrame % VIDEO_TEXTURE_UPLOAD_INTERVAL !== 0) return;
...
function applyInteriorVideoMeshLayout(mesh, tuning) {
  ...
  if (mesh.material?.uniforms?.sampleV0) {
    applyFlameTexVUniforms(mesh.material, tuning);
    mesh.material.uniforms.clipRadius.value = ud.innerRadius * VIDEO_CLIP_RADIUS_FACTOR;
    mesh.material.uniforms.clipTopY.value = ud.clipTopY;
  } else if (_videoMatTemplate && ud.innerRadius != null) {
 ...
 * @param {import("./OilBarrelTuning.js").OilBarrelTuning} tuning
 * @param {number} [barrelSeed=0] Per-barrel phase for flame video + light flicker.
...
export function createOilBarrelInteriorVideoMesh(
  ...
  mesh.userData.barrelSeed = barrelSeed;
  return mesh;
}
...
/**
 * Store per-barrel seed for fire-light flicker (video stays on shared textures).
 * @param {THREE.Mesh} mesh
 * @param {import("./OilBarrelTuning.js").OilBarrelTuning} tuning
 * @param {number} barrelSeed
 */
export function applyBarrelInteriorVideoSeed(mesh, tuning, barrelSeed) {
  if (!mesh?.isMesh || mesh.name !== "oil_interior_video") return;
  mesh.userData.barrelSeed = barrelSeed;
  if (_videoMatTemplate && mesh.userData.innerRadius != null) {
    mesh.material = cloneInteriorVideoMaterial(
      _videoMatTemplate,
      mesh.userData.innerRadius,
      mesh.userData.clipTopY,
      tuning
    );
  }
}
