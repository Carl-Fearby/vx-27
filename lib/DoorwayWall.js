import { pointInDoorwayPassage } from "./Collision.js";
import {
  getArenaDoorInnerZ,
  getArenaWallOuterZ,
  subtractXInterval,
...
export function canPeekIntoAttachedRoom(
  ...
  if (attachWall === "north") {
    if (z < arenaInnerZ - 0.5) return false;
  } else if (z > arenaInnerZ + 0.5) {
    return false;
  }
  for (const op of openings) {
    if (x >= op.left - pad && x <= op.right + pad) return true;
  }
  return false;
}
...
export function getDoorwayHeadroomCeilingY(
  ...
  for (const op of openings) {
    if (x >= op.left && x <= op.right) return archSillY(op, x);
  }
  return null;
}
