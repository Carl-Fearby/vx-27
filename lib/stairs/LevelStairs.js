 ...
 */
export function getStairCeilingCutoutPreloadSamples(
  placement,
  catwalkDeckY,
  standEyeHeight = 1.62
) {
 ...
 * @returns {{ x: number, z: number, eyeY: number, lookX: number, lookY?: number, lookZ: number, frames: number }[]}
 */
export function getStairCatwalkExitPreloadSamples(
  placement,
  catwalkDeckY,
  standEyeHeight = 1.62
) {
  ...
  return [
    ...samples,
    ...getStairCeilingCutoutPreloadSamples(placement, catwalkDeckY, standEyeHeight),
  ];
}
...
export function buildStairFlight(
  ...
  for (const side of [-1, 1]) {
    ...
    // Walkable stringer cap at the flight exit only. A full-length slab at
    // sideWalkTopY creates an invisible shelf above the front opening.
    const topWalkCenterLocalZ =
      STAIRS_TOTAL_RUN - STAIRS_STEP_RUN * 0.15;
    const topWalkHalfDepth =
      STAIR_STRINGER_DEPTH_OVERHANG + STAIRS_STEP_RUN * 0.85;
    ...
    const topCapCenter = localCenterToWorldXZ(
      flight,
      topCenterLocalX,
      topWalkCenterLocalZ
    );
