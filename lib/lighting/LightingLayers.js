...
/**
 * Shadow-caster-only objects: invisible to all main camera passes (no pass ever sets
 * this layer), but the sun/moon shadow cameras see them (sun.layers.enable calls add
 * it to the light's mask, which Three.js copies to the shadow camera). Used for
 * virtual ceiling extensions that close the diagonal sun-ray gap at arena corners.
 */
export const SHADOW_CASTER_LAYER = 6;
/** Rain particles — drawn after world + room geometry so they sit in front. */
export const WEATHER_LAYER = 6;
...
/** Falling rain — rendered in a late pass in front of world + room shells. */
export const WEATHER_RENDER_ORDER = 50;

export function setWeatherLayer(object) {
  object.layers.set(WEATHER_LAYER);
}
