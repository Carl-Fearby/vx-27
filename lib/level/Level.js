...
import { buildAttachedRoom } from "../rooms/LevelRoom.js";
import {
  getPillarGeometry,
  resolvePillarShape,
...
import { assignWorldLayers } from "../lighting/LightingLayers.js";
import { enableShadowsOn } from "../lighting/SceneEnvironment.js";
import { addShadowOccluderHorizontalSlab } from "../lighting/ShadowOccluders.js";
import {
...
/**
 * Bake each attached room's outdoor catwalk into the arena ceiling group —
 * one continuous deck layer above the capped interior, not a second mesh
 * stacked on the room shell.
 */
...
function addArchDoorwayJambPatches(
  ...
  const wallZ = getArenaWallCenterZ(attachWall, half, wallThickness);
  ...
  for (const op of openings) {
    ...
    const leftX = op.left + ARCH_JAMB_PATCH_THICKNESS / 2 + ARCH_JAMB_PATCH_INSET;
    const leftGeo = new THREE.BoxGeometry(
      ARCH_JAMB_PATCH_THICKNESS,
      patchH,
      wallThickness
    );
    leftGeo.translate(leftX, yMid, wallZ);
    applyCentredBoxWorldUVs(
      leftGeo,
      ARCH_JAMB_PATCH_THICKNESS,
      patchH,
    ...
    const rightX = op.right - ARCH_JAMB_PATCH_THICKNESS / 2 - ARCH_JAMB_PATCH_INSET;
    const rightGeo = new THREE.BoxGeometry(
      ARCH_JAMB_PATCH_THICKNESS,
      patchH,
      wallThickness
    );
    rightGeo.translate(rightX, yMid, wallZ);
    applyCentredBoxWorldUVs(
      rightGeo,
      ARCH_JAMB_PATCH_THICKNESS,
      patchH,
...
export function createLevelFromArena(scene, arena, textureLibrary = null) {
  ...
  const { containers: vx27ContainerMeshes } = spawnLevelProps(group, arena, colliders);
