...
import {
  ...
  CONTROL_PANEL_CONTAINER_HULL_COLOR_SCALE,
  CONTROL_PANEL_CONTAINER_HULL_EMISSIVE_SCALE,
...
/**
 * World pass always; room + container consoles also join the interior pass.
...
/** @param {THREE.MeshStandardMaterial} material */
function applyControlPanelContainerHullMaterial(material) {
  material.color.setScalar(CONTROL_PANEL_CONTAINER_HULL_COLOR_SCALE);
  material.emissiveIntensity =
    CONTROL_PANEL_HULL_EMISSIVE_INTENSITY *
    CONTROL_PANEL_CONTAINER_HULL_EMISSIVE_SCALE;
  material.needsUpdate = true;
...
export function applyControlPanelGroupMaterials(group, options = {}) {
  ...
  const inVx27Container = !!group.userData?.inVx27Container;
  const inRoom = !!group.userData?.roomId && !inVx27Container;
  const shelteredOutdoor = !!group.userData?.controlPanelShelteredOutdoor;
  const brightnessScale = inVx27Container
    ? CONTROL_PANEL_CONTAINER_BRIGHTNESS_SCALE
    : inRoom
      ? CONTROL_PANEL_ROOM_BRIGHTNESS_SCALE
      : shelteredOutdoor
  ...
  if (!inRoom && !inVx27Container && !shelteredOutdoor) return;
  ...
  group.traverse((obj) => {
    ...
    for (const material of materials) {
      ...
      if (inVx27Container) applyControlPanelContainerHullMaterial(material);
      else if (inRoom) applyControlPanelRoomHullMaterial(material);
