...
function pickWarmupBulletSurfaces(meshes) {
  const picked = [];
  const seen = new Set();
  const add = (mesh) => {
  ...
  add(
    meshes.find(
      (m) =>
        meshUsesMaterial(m, (mat) => mat?.userData?.vx27SetKey === "door"),
    ),
  );
...
export async function warmupBulletHolesGpu(
  ...
  const surfaces = pickWarmupBulletSurfaces([
  ...
  for (let i = 0; i < surfaces.length; i += 1) {
    ...
    spawnBulletHole(mesh, point, null, dir);
    if (i === 0) {
