...
import { VX27_DOOR_COLLIDER_OPEN_THRESHOLD } from "../vx27-container/Vx27ContainerDoors.js";
import { computeInteriorFlameLayout } from "./OilBarrelInteriorVideo.js";
import { loadOilBarrelTuning } from "./OilBarrelTuning.js";
...
/** Fire lights inside a room shell or container — room pass only (no world spill). */
...
/** @param {string | null} roomId @param {THREE.Object3D[]} containers */
function vx27ContainerForInteriorRoomId(roomId, containers) {
  if (!roomId) return null;
  for (const container of containers ?? []) {
    const ud = container.userData;
    // Interior props (barrel, console) use the container prop id as roomId; the
    // container mesh often has no roomId field on its level def.
    if (ud?.roomId === roomId || ud?.vx27PropId === roomId) return container;
  }
  return null;
}

/**
 * Ensure the barrel's interior video flame mesh is kept off the world pass.
 * @param {THREE.Object3D} barrel
 */
function disableBarrelInteriorVideoWorldSpill(barrel) {
  if (!barrel) return;
  barrel.traverse((obj) => {
    if (obj.isMesh && obj.name === "oil_interior_video") {
      obj.layers.disable(WORLD_LAYER);
    }
  });
}

/**
 * Remove any previously created outdoor spill light from this barrel.
 * @param {THREE.Object3D} barrel
 */
function removeBarrelSpillLight(barrel) {
  const spill = barrel.userData.doorSpillPoint;
  if (spill?.parent) spill.parent.remove(spill);
  barrel.userData.doorSpillPoint = null;
}

/**
 * Container cargo barrels: all fire lights and the flame video stay on
 * ROOM_INTERIOR_LAYER only. No outdoor cast — point lights bleed through
 * closed door geometry on the world pass so spill must be off entirely.
 *
 * @param {THREE.Light[]} lights
 * @param {THREE.Object3D[]} containers
 * @param {boolean} inEnclosedZone
 * @param {number} [nowSec]
 */
export function updateOilBarrelContainerDoorFireSpill(
  lights,
  containers,
  inEnclosedZone,
  nowSec,
  ...
  for (const light of lights ?? []) {
    ...
    const vmLayers = inEnclosedZone ? [VIEWMODEL_LAYER] : [];
    pinLightToLayers(light, ROOM_INTERIOR_LAYER, ...vmLayers);

    const barrel = getOilBarrelFromFireLight(light);
    if (!barrel || barrelsDone.has(barrel)) continue;
    barrelsDone.add(barrel);

    disableBarrelInteriorVideoWorldSpill(barrel);
    removeBarrelSpillLight(barrel);
  }
}
