...
export function pickHackableControlPanelUnderCrosshair(
  ...
  for (const hit of _hits) {
    ...
    if (!group?.userData?.controlPanel) return null;
    if (!isControlPanelHackInteractMesh(hit.object)) return null;
    if (isControlPanelScreenCHackBlocked(group)) return null;
    if (!isControlPanelFrontFaceHit(hit, raycaster)) return null;
    if (!isPlayerInFrontOfControlPanel(group, camera)) return null;
    ...
    if (_camFwd.dot(_toPanel) < CONTROL_PANEL_HACK_MIN_FACING) return null;
...
export function getControlPanelHackPromptContent(bindings) {
  return `Press ${formatControlPanelHackKeyLabel(bindings)} to hack`;
}
