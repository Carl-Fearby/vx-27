...
function validateSpawnAt(x, z, opts) {
  const { colliders, targets, config, skip, floorHoles, skipHoleCheck, spawnCtx } =
    opts;
  const { radius, height, spawnMargin } = config;

  const footY = resolveSpawnFootY(x, z, height, radius, colliders, {
    ...spawnCtx,
    floorHoles: spawnCtx?.floorHoles ?? floorHoles,
  });
...
export function resolveAuthoredSpawnPosition(x, z, opts) {
  ...
  const spawnPoint = opts.spawnPoint;
  if (spawnPoint?.y != null && Number.isFinite(spawnPoint.y)) {
    const footY = spawnPoint.y;
    const bodyTop = footY + config.height;
    if (overlapsTargets(x, z, config.radius, config.spawnMargin, opts.targets, opts.skip)) {
      return null;
    }
    if (spawnBlockedAt(x, z, footY, bodyTop, config.radius, opts.colliders)) {
      return null;
    }
    return { x, z, y: footY };
  }
...
export function spawnTargets(opts) {
  const { group, bounds, colliders, config, floorHoles, spawnCtx } = opts;
  const targetColliderSink = opts.targetColliderSink ?? colliders;
  const targets = [];
  const sharedGeo = buildHumanoidGeometry(config.height);
  const spawnOpts = { bounds, colliders, targets, config, floorHoles, spawnCtx };
  ...
  for (const pt of config.spawnPoints) {
    ...
    if (pt.random === true && Math.random() >= (pt.chance ?? 0.5)) continue;
    const pos = resolveAuthoredSpawnPosition(pt.x, pt.z, { ...spawnOpts, spawnPoint: pt });
    if (!pos) continue;
    const yaw = pt.yaw ?? 0;
...
/** Full white blind, then gradual fade (see getFlashbangOverlayOpacity in FpsGame). */
export const FLASHBANG_BLIND_FULL_SEC = 3;
/** @deprecated Dim phase removed — kept at 0 for duration math compatibility. */
...
export function getFlashbangBlindDurationSec() {
  return (
...
export function blindTargetFromFlashbang(mesh, simTime) {
  const fadeEnd = simTime + getFlashbangBlindDurationSec();
  const ud = mesh.userData;
  if (!ud.flashbangBlinding || simTime >= (ud.flashbangBlindFadeEnd ?? 0)) {
    ud.flashbangBlindStart = simTime;
  }
  ud.flashbangBlindFadeEnd = Math.max(ud.flashbangBlindFadeEnd ?? 0, fadeEnd);
  ud.flashbangBlinding = true;
}

/** Clear expired flashbang-blind state on targets (visuals use CSS overlay for player). */
export function updateFlashbangBlindVisuals(targets, simTime) {
  for (const mesh of targets) {
    const ud = mesh.userData;
    if (!ud.flashbangBlinding) continue;
    const fadeEnd = ud.flashbangBlindFadeEnd ?? 0;
    if (simTime >= fadeEnd) {
...
function headshotDamageFor(mesh, profile, shotDistance = 0) {
...
function bodyDamageFor(profile, zoneId, zoneMult = 1, shotDistance = 0) {
  return resolveBodyZoneDamage(profile, zoneId, zoneMult, shotDistance);
}
...
function zoneDamage(
  ...
  if (frac < 0.08) {
    return {
      zone: "head",
      damage: headshotDamageFor(mesh, profile, shotDistance),
    };
  }
  ...
  if (bulletDir) {
    ...
    if (frac >= armZone.fromTop && frac <= armZone.toTop) {
      ...
      for (const sign of [-1, 1]) {
        ...
        if (dist <= armR) {
          return {
            zone: "arm",
            damage: bodyDamageFor(profile, "arm", armZone.mult, shotDistance),
          };
        }
      }
    }
    ...
    for (const zone of BODY_ZONES) {
      ...
      if (radialDist <= torsoR) {
        return {
          zone: zone.id,
          damage: bodyDamageFor(profile, zone.id, zone.mult, shotDistance),
        };
      }
      continue;
    }
    ...
    if (fb) {
      return {
        zone: fb.id,
        damage: bodyDamageFor(profile, fb.id, fb.mult, shotDistance),
      };
    }
    return {
      zone: "body",
      damage: bodyDamageFor(profile, "body", 1, shotDistance),
    };
  }
...
export function applyTargetHit(
  ...
  const result = hitPoint
    ? zoneDamage(mesh, hitPoint, bulletDir, weaponId, shotDistance)
    : {
        zone: "body",
        damage: bodyDamageFor(profile, "body", 1, shotDistance),
      };
  ...
  const damageResult = gameCore?.applyTargetDamage?.(
    ud.health,
    ud.maxHealth,
    damage,
  ) ?? {
    health: Math.max(0, ud.health - damage),
    ratio: ud.maxHealth > 0 ? Math.max(0, ud.health - damage) / ud.maxHealth : 0,
    killed: Math.max(0, ud.health - damage) <= 0,
  };
...
export function updateTargetsRepair(targets, dt, gameCore = null) {
  for (const mesh of targets) {
    if (!mesh.visible || mesh.userData.health <= 0) continue;
    const ud = mesh.userData;
    const rate = ud.repairPerSecond ?? DEFAULT_TARGET_CONFIG.repairPerSecond;
    const repair =
      gameCore?.tickTargetRepair?.(
        dt,
        ud.health,
        ud.maxHealth,
        ud.repairCooldown ?? 0,
    ...
    if (repair) {
      ud.health = repair.health;
      ud.repairCooldown = repair.repairCooldown;
      if (repair.repaired) setTargetHealthVisual(mesh, repair.ratio);
      continue;
    }
...
export function resetAllTargetsToSpawn(targets, opts) {
  const { config, bounds, colliders, floorHoles, spawnCtx } = opts;
  const spawnOpts = { bounds, colliders, targets, config, floorHoles, spawnCtx };
  for (const mesh of targets) {
    ...
    const pos = pickRandomSpawnPosition({ ...spawnOpts, skip: mesh });
    if (pos) {
      activateTargetAt(mesh, pos.x, pos.z, config, pos.y);
    } else {
...
const RAGDOLL_PHYSICS_MAX_SUBSTEPS = 4;
...
function applyGrenadeLimbSevering(
  ...
  for (const rootId of GRENADE_SEVERABLE_ROOTS) {
    ...
    const severDamage = Math.max(8, Math.round(8 + blastFalloff * 16));
    onBloodSplatter?.(
      spawnBloodSplatter(scene, _ragdollLimbEnd.clone(), blastDir, severDamage),
    );
...
function probeLimbContact(limb, frac, floorY, colliders, bounds, skipFloor) {
  const limbR = 0.06;
  _ragdollLimbEnd
    .copy(limb.defaultDir)
