...
function surfaceRefMaterial(surfaceMesh) {
  return Array.isArray(surfaceMesh.material)
...
function createHoleMaterial(tex, surfaceMesh) {
  const refMat = surfaceRefMaterial(surfaceMesh);
...
function placePlaneHoleLocal(hole, surfaceMesh, hitPoint, hitFace, bulletDir, offset) {
  ...
  // Inset into the face (normal points outward) so decals stay flush with the bolt impact.
...
export function spawnBulletHole(surfaceMesh, hitPoint, hitFace, bulletDir) {
  ...
  hole.renderOrder = HOLE_RENDER_ORDER_BASE + (seq % HOLE_RENDER_ORDER_SPAN);
