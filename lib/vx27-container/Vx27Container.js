 ...
 * @param {string | null} [flickerSeedSource]
 */
function buildInteriorShell(
  width,
  height,
  length,
  shell,
  ...
  const beaconRigs = buildVx27ContainerBeaconRigs(
    offsetX,
    ceilY,
    offsetZ,
    innerW,
...
/** @param {THREE.Group} container @param {boolean} enabled @param {{ fromDevPanel?: boolean }} [options] */
export function applyVx27ContainerCeilingLightEnabled(
  container,
  enabled,
  options = {}
) {
  if (options.fromDevPanel) {
    if (enabled) {
  ...
  const alwaysOn = container.userData?.vx27CeilingLightAlwaysOn === true;
  const effective = alwaysOn && !options.fromDevPanel ? true : enabled;
  const rigs = vx27ContainerBeaconRigsForContainer(container);
  if (rigs.length) {
    applyVx27ContainerAllBeaconsEnabled(container, effective);
    updateVx27ContainerDoorLightingLayers(container);
    return;
  }
  const light = container.userData?.vx27CeilingLight;
  ...
  light.visible = effective;
  light.intensity = effective ? base : 0;
  updateVx27ContainerDoorLightingLayers(container);
}

/** @param {THREE.Group[]} containers @param {boolean} enabled @param {{ fromDevPanel?: boolean }} [options] */
export function setVx27ContainerCeilingLightEnabled(containers, enabled, options = {}) {
  for (const container of containers ?? []) {
    applyVx27ContainerCeilingLightEnabled(container, enabled, options);
  }
}
...
export function rebuildVx27ContainerDoors(group, doorTuning) {
  ...
  wireVx27DoorEgressBeaconSources(group);
...
function buildShell(group, width, height, length, shell, insets, edgeRadius = 0, doorTuning) {
  ...
  const interior = buildInteriorShell(
    ...
    insets,
    edgeRadius,
    group.userData.vx27PropId ?? null
  );
  group.add(interior);
...
export function createVx27Container(
  ...
  group.userData.vx27CeilingLightAlwaysOn =
    options.propDef?.ceilingLightAlwaysOn !== false;
