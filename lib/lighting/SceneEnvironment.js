...
/**
 * Sky (SKY_LAYER) → world (WORLD, outdoor sun) → room (ROOM, point lights only) →
 * barrel flame (BARREL_FLAME, unlit video on top). Room pass skipped when culling
 * hides all shells; flame pass always runs so open barrels stay visible outdoors.
 *
 * @param {THREE.WebGLRenderer} renderer
 * @param {THREE.Scene} scene
...
export function renderSceneWithLayeredLighting(renderer, scene, camera, options = {}) {
  ...
  try {
    ...
    renderer.autoClear = false;
    camera.layers.set(BARREL_FLAME_LAYER);
    renderer.render(scene, camera);
  } finally {
