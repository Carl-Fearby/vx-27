...
export const BINDING_ROWS = [
  ...
  { id: "swapWeapon", label: "Swap primary weapon" },
  { id: "grenade", label: "Throw grenade" },
  { id: "interact", label: "Interact" },
  { id: "hack", label: "Hack console" },
...
export function loadBindings() {
  try {
    ...
    if (typeof merged.dayNightToggle !== "string") {
      merged.dayNightToggle = DEFAULT_BINDINGS.dayNightToggle;
    }
    if (typeof merged.interact !== "string") {
      merged.interact = DEFAULT_BINDINGS.interact;
    }
    if (typeof merged.hack !== "string") {
      merged.hack = DEFAULT_BINDINGS.hack;
    }
    if (typeof merged.swapWeapon !== "string") {
      merged.swapWeapon = DEFAULT_BINDINGS.swapWeapon;
    }
    if (merged.hack === "KeyK") {
      merged.hack = DEFAULT_BINDINGS.hack;
      saveBindings(merged);
    }
    // B is a primary weapon toggle; free it from older cycle-fire default.
    if (merged.cycleFireMode === "KeyB") {
      merged.cycleFireMode = DEFAULT_BINDINGS.cycleFireMode;
      saveBindings(merged);
    }
    if (merged.dayNightToggle === "KeyM") {
      merged.dayNightToggle = DEFAULT_BINDINGS.dayNightToggle;
      saveBindings(merged);
    }
    if (merged.swapWeapon === "Digit0" || merged.swapWeapon === "Digit1") {
      merged.swapWeapon = DEFAULT_BINDINGS.swapWeapon;
      saveBindings(merged);
    }
    return merged;
...
export function isBindingDown(input, bindings, action) {
  if (input.isVirtualDown?.(action)) return true;
...
export function wasBindingPressed(input, bindings, action) {
  if (input.wasVirtualPressed?.(action)) return true;
  const code = bindings[action];
  if (!code) return false;
