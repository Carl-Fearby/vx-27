...
import { getCargoModulePreviewPropDef } from "./CargoModulePropsTuning.js";
import {
...
import {
  applyVx27ContainerDoorPanelLayers,
  captureVx27DoorPanelEmissiveBase,
  isVx27DoorInteriorLinerMesh,
  isVx27DoorPanelMesh,
  updateVx27ContainerDoorLightingLayers,
} from "./Vx27ContainerDoorLighting.js";
import {
...
import {
  applyVx27ContainerAllBeaconsEnabled,
  buildVx27ContainerBeaconRigs,
...
function applyVx27MaterialSurface(mat, tuning) {
  ...
  if (mat.userData.vx27DoorInteriorLiner) {
...
function cloneVx27ContainerMaterial(source, { floorInstance = false } = {}) {
  ...
  mat.userData.vx27DoorInteriorLiner = source.userData.vx27DoorInteriorLiner;
  ...
  if (source.color?.isColor) {
    mat.color = source.color.clone();
  } else if (!mat.color?.isColor) {
    mat.color = new THREE.Color(0xffffff);
  }
  if (source.emissive?.isColor) {
    mat.emissive = source.emissive.clone();
  } else if (!mat.emissive?.isColor) {
    mat.emissive = new THREE.Color(0x000000);
  }
  if (floorInstance) {
...
/** @param {string} name */
function isVx27EndcapInteriorMesh(name) {
  return (
    name.startsWith("vx27_container_endcap_") && name.endsWith("_interior")
  );
}

/** @param {string} name */
function isVx27ExteriorShellMesh(name) {
  if (isVx27EndcapInteriorMesh(name) || isVx27DoorPanelMesh(name)) return false;
  return (
...
export { updateVx27ContainerDoorLightingLayers } from "./Vx27ContainerDoorLighting.js";
...
function addVx27EndcapFrameMeshes(
  ...
  const frameShadow = { castShadow: true, receiveShadow: true };
...
/**
 * @param {THREE.BufferGeometry} geo
 * @param {(i0: number, i1: number, i2: number) => void} fn
 */
function forEachGeometryTriangle(geo, fn) {
...
/** Keep only cap triangles whose normals face along ±Z (container interior side). */
function extractDoorInteriorCapGeometry(geo, wantNormalZ) {
  const pos = geo.attributes.position;
  const norm = geo.attributes.normal;
  const uv = geo.attributes.uv;
  const wantSign = Math.sign(wantNormalZ);
  const newPos = [];
  const newNorm = [];
  const newUv = [];

  forEachGeometryTriangle(geo, (i0, i1, i2) => {
    const nz = norm.getZ(i0);
    if (Math.abs(nz) < 0.5 || Math.sign(nz) !== wantSign) return;
    for (const i of [i0, i1, i2]) {
      newPos.push(pos.getX(i), pos.getY(i), pos.getZ(i));
      newNorm.push(norm.getX(i), norm.getY(i), norm.getZ(i));
      newUv.push(uv.getX(i), uv.getY(i));
    }
  });

  const cap = new THREE.BufferGeometry();
  cap.setAttribute("position", new THREE.Float32BufferAttribute(newPos, 3));
  cap.setAttribute("normal", new THREE.Float32BufferAttribute(newNorm, 3));
  cap.setAttribute("uv", new THREE.Float32BufferAttribute(newUv, 2));
  return cap;
}
...
/**
 * Room-pass liner material — same maps/transform as the shell cap, cloned so
 * opacity tweaks never touch the exterior draw.
 * @param {THREE.MeshStandardMaterial} panelFaceMat
 */
function cloneVx27DoorLinerMaterial(panelFaceMat) {
  const mat = panelFaceMat.clone();
  for (const key of MAP_CHANNELS) {
    if (mat[key]) mat[key] = mat[key].clone();
  }
  mat.userData.vx27DoorInteriorLiner = true;
  ...
  mat.emissiveIntensity = 0;
  applyMapTransform(mat, {
    rotateUV90: mat.userData.vx27RotateUV90 ?? false,
    mirrorU: mat.userData.vx27MirrorU ?? false,
    flipV: mat.userData.vx27FlipV ?? false,
  });
  return mat;
}

/**
 * Room-pass inner cap copied from the door mesh so UVs match when shut.
 * @param {number} width
 * @param {number} height
 * @param {number} thickness
 * @param {"front" | "back"} endKey
 ...
 * @param {THREE.Material} material
 */
function makeDoorInteriorLinerMesh(
  width,
  height,
  thickness,
  endKey,
  ...
  const inset = 0.001;
  const wantNormalZ = endKey === "front" ? -1 : 1;
  let source = null;

  if (openingEdgeRadius > 1e-6) {
    source = buildDoorExtrudedPanelGeometry(
      width,
      height,
      thickness,
      openingEdgeRadius,
    ...
    source = new THREE.BoxGeometry(width, height, thickness);
    applyDoorPanelUVs(source, width, height, thickness);
  }

  const geo = extractDoorInteriorCapGeometry(source, wantNormalZ);
  source.dispose();

  const mesh = new THREE.Mesh(geo, material);
  mesh.castShadow = false;
  mesh.receiveShadow = true;
  mesh.position.z = wantNormalZ < 0 ? -inset : inset;
  return mesh;
}
...
/**
 * Exterior on WORLD (+ ROOM when in attached room). Interior shell always uses the
 * room pass so outdoor sun, hemisphere, and barrel fire stay outside the shell.
...
export function applyVx27ContainerRenderLayers(container, roomId = null) {
  container.traverse((obj) => {
    ...
    if (isVx27EndcapInteriorMesh(obj.name)) {
      setRoomInteriorLayer(obj);
      obj.castShadow = false;
      obj.receiveShadow = true;
      obj.renderOrder = 3;
    ...
    if (isVx27DoorInteriorLinerMesh(obj.name)) {
      setRoomInteriorLayer(obj);
      obj.castShadow = false;
      obj.receiveShadow = true;
      obj.renderOrder = 5;
    ...
    if (isVx27DoorPanelMesh(obj.name)) {
      applyVx27ContainerDoorPanelLayers(obj, container);
      return;
    }
...
function buildInteriorShell(width, height, length, shell, insets, edgeRadius = 0) {
  ...
  const frontGap = halfL - shell / 2 - (offsetZ + sideWallL / 2);
  const backGap = offsetZ - sideWallL / 2 - (-halfL + shell / 2);
  if (frontGap > 0.002) {
    for (const { key, x } of [
      { key: "left", x: -halfW + insets.left - wallThick / 2 },
      { key: "right", x: halfW - insets.right + wallThick / 2 },
    ]) {
      const seal = shellBoxMesh(
        wallThick,
        innerH,
        frontGap,
        sideIntMat,
        `vx27_container_interior_wall_${key}_end_front`,
        { castShadow: false, receiveShadow: true }
      );
      seal.position.set(x, wallCenterY, offsetZ + sideWallL / 2 + frontGap / 2);
      interior.add(seal);
    }
  }
  if (backGap > 0.002) {
    for (const { key, x } of [
      ...
      const seal = shellBoxMesh(
        ...
        backGap,
        sideIntMat,
        `vx27_container_interior_wall_${key}_end_back`,
        { castShadow: false, receiveShadow: true }
      );
      seal.position.set(x, wallCenterY, offsetZ - sideWallL / 2 - backGap / 2);
      interior.add(seal);
    }
  }
  ...
  if (r > 0.001) {
    ...
    const cornerCenters = [
      { key: "fl", x: offsetX - innerW / 2 + r, z: offsetZ + innerL / 2 - r },
      { key: "fr", x: offsetX + innerW / 2 - r, z: offsetZ + innerL / 2 - r },
      { key: "br", x: offsetX + innerW / 2 - r, z: offsetZ - innerL / 2 + r },
      { key: "bl", x: offsetX - innerW / 2 + r, z: offsetZ - innerL / 2 + r },
    ];
    for (const corner of cornerCenters) {
      const def = vx27VerticalCornerDefs().find((d) => d.key === corner.key);
      if (!def) continue;
      const seal = makeSolidCornerArcQuarter(
        r,
        innerH,
        def.thetaStart,
        sideIntMat,
        `vx27_container_interior_corner_${corner.key}_seal`,
        { castShadow: false, receiveShadow: true }
      );
      seal.position.set(corner.x, wallCenterY, corner.z);
      interior.add(seal);
    }
  }

  const beaconRigs = buildVx27ContainerBeaconRigs(offsetX, ceilY, offsetZ, innerL);
  for (const rig of beaconRigs) {
    interior.add(rig);
  }
  interior.userData.vx27BeaconRigs = beaconRigs;
  interior.userData.vx27BeaconRig = beaconRigs[0] ?? null;
  interior.userData.vx27CeilingLight = beaconRigs[0]?.userData?.ceilingLight ?? null;

  return interior;
}

/**
 * Invisible world-layer slabs — block directional sun through shell, end caps, and doors.
 * @param {number} width
 * @param {number} height
 * @param {number} length
 * @param {number} shell
 * @param {Vx27InteriorInsets} insets
 */
function buildVx27ContainerSunOccluders(width, height, length, shell, insets) {
  const halfW = width / 2;
  const halfH = height / 2;
  const halfL = length / 2;
  const r = normalizeVx27EdgeRadius(0, width, height, length);
  ...
  const wallBodyH = Math.max(0.05, wallTopY - floorBottomY);
  const wallBodyY = (wallTopY + floorBottomY) / 2;
  const endcapH = Math.max(
    0.05,
    roofVisualThickness > 0 ? height - roofVisualThickness - shell : height
  );
  const endcapY = (-halfH + shell + (halfH - roofVisualThickness)) / 2;
  const endcapHalfW = Math.max(0.05, r > 0 ? width / 2 - r : width / 2);
  const opening = getVx27EndcapOpeningLocal(width, height, length, shell, insets);
  const occluderDepth = 0.08;

  const occluders = new THREE.Group();
  occluders.name = "vx27_container_sun_occluders";
  occluders.userData.vx27SunOccluders = true;

  addShadowOccluderBox(occluders, shell, wallBodyH, wallL, -halfW + shell / 2, wallBodyY, 0);
  addShadowOccluderBox(occluders, shell, wallBodyH, wallL, halfW - shell / 2, wallBodyY, 0);
  addShadowOccluderHorizontalSlab(
    occluders,
    width - shell * 2,
    length - shell * 2,
    0,
    halfH - roofVisualThickness - 0.04,
    0
  );

  const endCapDefs = [
    { key: "front", z: halfL + shell / 2, rotY: 0 },
    { key: "back", z: -halfL - shell / 2, rotY: Math.PI },
  ];
  for (const end of endCapDefs) {
    const frame = new THREE.Group();
    frame.position.set(0, 0, end.z);
    frame.rotation.y = end.rotY;
    setWorldLayer(frame);

    const endcapBottomY = endcapY - endcapH / 2;
    const endcapTopY = endcapY + endcapH / 2;

    if (opening.openLeft + endcapHalfW > 0.004) {
      const panelW = opening.openLeft + endcapHalfW;
      const panelCenterX = (-endcapHalfW + opening.openLeft) / 2;
      addShadowOccluderBox(
        frame,
        panelW,
        endcapH,
    ...
    if (endcapHalfW - opening.openRight > 0.004) {
      const panelW = endcapHalfW - opening.openRight;
      const panelCenterX = (opening.openRight + endcapHalfW) / 2;
      addShadowOccluderBox(
        frame,
        panelW,
        endcapH,
...
export function collectVx27ContainerRoomLights(containers) {
  const lights = [];
  for (const container of containers ?? []) {
    ...
    const light = container.userData?.vx27CeilingLight;
    if (light?.isLight) lights.push(light);
  }
  return lights;
}

/** @param {THREE.Group} container @param {boolean} enabled */
export function applyVx27ContainerCeilingLightEnabled(container, enabled) {
  const rigs = vx27ContainerBeaconRigsForContainer(container);
  if (rigs.length) {
    applyVx27ContainerAllBeaconsEnabled(container, enabled);
    updateVx27ContainerDoorLightingLayers(container);
    return;
  }
  const light = container.userData?.vx27CeilingLight;
...
/** @param {THREE.Group[]} containers @param {boolean} enabled */
export function setVx27ContainerCeilingLightEnabled(containers, enabled) {
  for (const container of containers ?? []) {
    applyVx27ContainerCeilingLightEnabled(container, enabled);
  }
}

/** @param {THREE.Group} group */
function refreshVx27ContainerSunOccluders(group) {
  const old = group.getObjectByName("vx27_container_sun_occluders");
  if (old) {
  ...
  group.add(
    buildVx27ContainerSunOccluders(
      width,
      height,
      length,
      shell,
...
/**
 * Opaque edge strips on each leaf — blocks sun through door alpha cutouts and rounded
 * profile corners; moves with the panel when the door opens.
 * @param {THREE.Mesh} panel
 * @param {ReturnType<import("./Vx27ContainerDoors.js").computeVx27DoorLayout>} layout
 * @param {"left" | "right"} side
 * @param {THREE.Material} sealMat
 */
function addVx27DoorPanelEdgeSeals(panel, layout, side, sealMat) {
  const depth = Math.max(layout.thickness + 0.02, 0.05);
  const edgeW = 0.02;
  const sillH = 0.024;
  const lintelH = 0.02;

  const sill = shellBoxMesh(
    layout.leafW + 0.012,
    sillH,
    depth,
    sealMat,
    `${panel.name}_sill_seal`,
    { castShadow: false, receiveShadow: true }
  );
  sill.position.set(0, -layout.leafH / 2 - sillH / 2 + 0.004, 0);
  sill.userData.skipBulletSurface = true;
  setWorldLayer(sill);
  panel.add(sill);

  const lintel = shellBoxMesh(
    layout.leafW + 0.012,
    lintelH,
    depth,
    sealMat,
    `${panel.name}_lintel_seal`,
    { castShadow: false, receiveShadow: true }
  );
  lintel.position.set(0, layout.leafH / 2 + lintelH / 2 - 0.004, 0);
  lintel.userData.skipBulletSurface = true;
  setWorldLayer(lintel);
  panel.add(lintel);

  const hingeX = side === "left" ? -layout.leafW / 2 - edgeW / 2 : layout.leafW / 2 + edgeW / 2;
  const jamb = shellBoxMesh(
    edgeW,
    layout.leafH + 0.016,
    depth,
    sealMat,
    `${panel.name}_hinge_jamb_seal`,
    { castShadow: false, receiveShadow: true }
  );
  jamb.position.set(hingeX, 0, 0);
  jamb.userData.skipBulletSurface = true;
  setWorldLayer(jamb);
  panel.add(jamb);

  const meetX = side === "left" ? layout.leafW / 2 + edgeW / 2 : -layout.leafW / 2 - edgeW / 2;
  const meet = shellBoxMesh(
    edgeW,
    layout.leafH + 0.016,
    depth,
    sealMat,
    `${panel.name}_meet_seal`,
    { castShadow: false, receiveShadow: true }
  );
  meet.position.set(meetX, 0, 0);
  meet.userData.skipBulletSurface = true;
  setWorldLayer(meet);
  panel.add(meet);
}
...
function buildDoors(group, width, height, length, shell, insets, edgeRadius, doorTuning) {
  ...
  const doorSealMat = buildMaterial("corner_arc", {
    surface: "exterior",
    doubleSided: false,
  });
  ...
  for (const end of layout.ends) {
    for (const leaf of end.leaves) {
      ...
      const panelFaceMat = faceMat.clone();
      ...
      captureVx27DoorPanelEmissiveBase(panel);
      panel.position.set(leaf.panelOffsetX, 0, leaf.panelOffsetZ);

      const linerMat = cloneVx27DoorLinerMaterial(panelFaceMat);
      const liner = makeDoorInteriorLinerMesh(
        layout.leafW,
        layout.leafH,
        layout.thickness,
        end.key,
      ...
      liner.name = `vx27_container_door_${end.key}_${leaf.side}_interior`;
      panel.add(liner);
      addVx27DoorPanelEdgeSeals(panel, layout, leaf.side, doorSealMat);

      addShadowOccluderBox(
        panel,
        layout.leafW,
        layout.leafH,
        Math.max(layout.thickness, 0.05),
        0,
        0,
        0
      );
...
export {
  ...
  pinVx27DoorEgressLightLayers,
  updateVx27ContainerBeaconLights,
  updateVx27ContainerDoorEgressLights,
} from "./Vx27ContainerBeaconLight.js";
...
function buildShell(group, width, height, length, shell, insets, edgeRadius = 0, doorTuning) {
  ...
  const interior = buildInteriorShell(width, height, length, shell, insets, edgeRadius);
  group.add(interior);
  group.userData.vx27BeaconRigs = interior.userData.vx27BeaconRigs ?? null;
  group.userData.vx27BeaconRig = interior.userData.vx27BeaconRig ?? null;
  group.userData.vx27CeilingLight = interior.userData.vx27CeilingLight ?? null;
  group.add(buildVx27ContainerSunOccluders(width, height, length, shell, insets));
...
function removeVx27ContainerShell(group) {
  for (const name of [
    ...
    "vx27_container_door_egress",
    "vx27_container_sun_occluders",
  ]) {
...
export function rebuildVx27ContainerExterior(group, edgeRadius) {
  ...
  const interior = buildInteriorShell(
    width,
    height,
    length,
    shell,
...
export function rebuildVx27ContainerInterior(group, insets) {
  ...
  const shell = vx27ShellThicknessForGroup(group);
  const edgeRadius = readVx27ContainerEdgeRadius(group);
  const interior = buildInteriorShell(width, height, length, shell, normalized, edgeRadius);
  group.add(interior);
  group.userData.vx27BeaconRigs = interior.userData.vx27BeaconRigs ?? null;
  group.userData.vx27BeaconRig = interior.userData.vx27BeaconRig ?? null;
  group.userData.vx27CeilingLight = interior.userData.vx27CeilingLight ?? null;
...
/** @type {import("../physics/Collision.js").ColliderBox[] | null} */
let _vx27StandoffSpecsCacheKey = null;
/** @type {Vx27ContainerStandoffSpec[]} */
let _vx27StandoffSpecsCache = [];

/** @param {import("../physics/Collision.js").ColliderBox[]} colliders @returns {Vx27ContainerStandoffSpec[]} */
export function getVx27ContainerStandoffSpecs(colliders) {
  if (colliders === _vx27StandoffSpecsCacheKey) return _vx27StandoffSpecsCache;
  _vx27StandoffSpecsCacheKey = colliders;
  _vx27StandoffSpecsCache = extractVx27ContainerStandoffSpecs(colliders);
  return _vx27StandoffSpecsCache;
}

export function invalidateVx27ContainerColliderCache() {
  _vx27StandoffSpecsCacheKey = null;
  _vx27StandoffSpecsCache = [];
}
...
export function resolveVx27ContainerStandoffs(position, radius, colliders) {
  for (const spec of getVx27ContainerStandoffSpecs(colliders)) {
...
export function isPointInsideVx27ContainerPlayVolume(worldX, worldZ, colliders) {
  ...
  for (const spec of getVx27ContainerStandoffSpecs(colliders)) {
    if (isPointInsideVx27ContainerStandoffSpec(worldX, worldZ, spec)) {
      return true;
    }
  }
  return false;
...
/** @param {number} worldX @param {number} worldZ @param {Vx27ContainerStandoffSpec} spec */
function isPointInsideVx27ContainerStandoffSpec(worldX, worldZ, spec) {
  ...
  return Math.abs(relX) <= inPlayX + 0.01 && Math.abs(relZ) <= inPlayZ + 0.01;
}

/**
 * @param {THREE.Group} group
 * @returns {Vx27ContainerStandoffSpec | null}
 */
export function vx27ContainerStandoffSpecFromGroup(group) {
  if (!group?.userData?.vx27Container) return null;
  const width = group.userData.vx27Width;
  const height = group.userData.vx27Height;
  const length = group.userData.vx27Length;
  if (!width || !height || !length) return null;

  const scale = normalizeVx27ContainerScale(group.userData.vx27Scale);
  const shell =
    group.userData.vx27Shell ?? VX27_SHELL_THICKNESS * scale;
  const insets = normalizeVx27InteriorInsets(group.userData.vx27InteriorInsets);
  const floorY = group.position.y - height / 2;
  const opening = getVx27EndcapOpening(width, height, length, shell, floorY, insets);
  ...
  return {
    ...
    playHalfW,
    playHalfL,
    innerHalfW: width / 2 - shell,
    innerHalfL: opening.innerHalfL,
    openHalfW: opening.openHalfW,
    openCenterX: opening.openCenterX,
 ...
 * @param {THREE.Group} group
 */
export function isPointInsideVx27ContainerGroup(worldX, worldZ, group) {
  const spec = vx27ContainerStandoffSpecFromGroup(group);
  if (!spec) return false;
  return isPointInsideVx27ContainerStandoffSpec(worldX, worldZ, spec);
}

/**
 * Opening footprint in container-local space — includes capsule radius so wall hugs still count.
 * @param {number} worldX
 * @param {number} worldZ
 * @param {Vx27ContainerStandoffSpec} spec
 * @param {number} [radius=PLAYER_RADIUS]
 */
function isPointInsideVx27ContainerInteriorSpec(worldX, worldZ, spec, radius = PLAYER_RADIUS) {
  const { lx, lz } = worldToContainerLocal(
    spec.cx,
    spec.cz,
    spec.rotY,
  ...
  const halfW = Math.max(0.05, spec.openHalfW ?? spec.playHalfW ?? 0.05);
  const halfL = Math.max(0.05, spec.innerHalfL ?? spec.playHalfL ?? 0.05);
  return Math.abs(relX) <= halfW + pad && Math.abs(relZ) <= halfL + pad;
}
 ...
 */
function vx27ContainerGroupForStandoffSpec(containers, spec, colliders) {
  for (const box of colliders ?? []) {
    if (box.kind !== "vx27ContainerWall" || box.active === false) continue;
    if (box.containerCx == null || box.containerCz == null) continue;
    if (Math.abs(box.containerCx - spec.cx) > 0.02) continue;
    if (Math.abs(box.containerCz - spec.cz) > 0.02) continue;
    const propId = box.propId;
    if (!propId) continue;
    const match = (containers ?? []).find(
      (c) => c.userData?.vx27PropId === propId
    );
    if (match) return match;
  }
  for (const container of containers ?? []) {
    if (Math.abs(container.position.x - spec.cx) > 0.02) continue;
    if (Math.abs(container.position.z - spec.cz) > 0.02) continue;
    if (Math.abs((container.rotation?.y ?? 0) - spec.rotY) > 0.02) continue;
    return container;
  }
  return null;
}
...
/**
 * Full interior footprint (matches floor collider / play volume), not the shrunk standoff box.
 * @param {number} worldX
 * @param {number} worldZ
 * @param {THREE.Group} group
 */
export function isPointInsideVx27ContainerInterior(worldX, worldZ, group) {
  const spec = vx27ContainerStandoffSpecFromGroup(group);
  if (!spec) return false;
  return isPointInsideVx27ContainerInteriorSpec(worldX, worldZ, spec);
}

/**
 * Which VX-27 container the player is inside (rotation-aware, capsule-padded interior).
 * @param {number} worldX
 * @param {number} worldZ
 * @param {THREE.Object3D[]} containers
 * @param {import("../physics/Collision.js").ColliderBox[]} colliders
 * @returns {THREE.Group | null}
 */
export function resolveVx27ContainerForPlayer(worldX, worldZ, containers, colliders) {
  for (const spec of getVx27ContainerStandoffSpecs(colliders)) {
    if (!isPointInsideVx27ContainerInteriorSpec(worldX, worldZ, spec)) continue;
    const container = vx27ContainerGroupForStandoffSpec(containers, spec, colliders);
    if (container) return container;
  }

  const floorPad = PLAYER_RADIUS + 0.1;
  for (const box of colliders ?? []) {
    if (box.kind !== "vx27ContainerWall" || box.active === false) continue;
    if (box.containerPart !== "floor") continue;
    if (box.x == null || box.z == null || box.halfX == null || box.halfZ == null) {
    ...
    const { lx, lz } = worldToBoxLocal(box, worldX, worldZ);
    if (Math.abs(lx) > box.halfX + floorPad || Math.abs(lz) > box.halfZ + floorPad) {
      continue;
    }
    const propId = box.propId;
    if (propId) {
  ...
  for (const container of containers ?? []) {
    if (isPointInsideVx27ContainerInterior(worldX, worldZ, container)) {
      return container;
    }
  }
  return null;
...
/**
 * Ragdoll physics only needs the local container shell when a kill happens inside one.
 * @param {number} worldX
 * @param {number} worldZ
 * @param {import("../physics/Collision.js").ColliderBox[]} colliders
 * @param {THREE.Object3D[]} containers
 */
export function collidersForRagdollNear(
  worldX,
  worldZ,
  colliders,
  containers
) {
  const container = resolveVx27ContainerForPlayer(
    worldX,
    worldZ,
    containers,
    colliders
  ...
  const propId = container.userData?.vx27PropId;
  if (!propId) return colliders;
  return colliders.filter((box) => {
    if (box.kind !== "vx27ContainerWall") return true;
    return box.propId === propId;
  });
}

const _containerPassFrustum = new THREE.Frustum();
const _containerPassProjScreenMatrix = new THREE.Matrix4();
const _containerPassBBox = new THREE.Box3();

/**
 * Whether the room-interior pass should run for VX-27 container geometry.
 *
 * @param {THREE.Camera} camera
 * @param {THREE.Object3D[]} containerMeshes
 * @param {number} playerX
 * @param {number} playerZ
 * @param {import("../physics/Collision.js").ColliderBox[]} colliders
 */
export function isAnyVx27ContainerInteriorPassNeeded(
  camera,
  containerMeshes,
  playerX,
  playerZ,
  ...
  if (isPointInsideVx27ContainerPlayVolume(playerX, playerZ, colliders)) {
    return true;
  }
  if (!containerMeshes?.length) return false;

  _containerPassProjScreenMatrix.multiplyMatrices(
    camera.projectionMatrix,
    camera.matrixWorldInverse
  );
  _containerPassFrustum.setFromProjectionMatrix(_containerPassProjScreenMatrix);

  for (const container of containerMeshes) {
    if (container.visible === false) continue;
    container.updateMatrixWorld(true);
    _containerPassBBox.setFromObject(container);
    if (_containerPassFrustum.intersectsBox(_containerPassBBox)) {
      return true;
    }
  }
  return false;
...
/** Preview-only container — level1 cargo module tuning, origin-centered for credits UI. */
export function createVx27ContainerPreviewMesh() {
  const propDef = getCargoModulePreviewPropDef();
  const root = new THREE.Group();
  createVx27Container(root, 0, 0, 0, 0, {
    scale: propDef.scale,
    propDef,
  });
  cloneVx27PreviewMaterials(root);
  return root.getObjectByName("vx27_container") ?? root.children[0] ?? root;
}

/**
 * Credits / marketing canvas uses one camera pass — flatten game render layers and
 * hide shadow-only occluders so the full shell reads instead of emissive edge lines.
 * @param {THREE.Object3D | null | undefined} container
 */
export function prepareVx27ContainerCreditsPreview(container) {
  if (!container) return;
  ...
  container.traverse((obj) => {
    ...
    if (obj.isLight && obj.userData?.vx27ContainerLight) {
      pinLightToLayers(obj, WORLD_LAYER, ROOM_INTERIOR_LAYER);
    }
  });

  const occluders = container.getObjectByName("vx27_container_sun_occluders");
  if (occluders) occluders.visible = false;
}
...
export function disposeVx27ContainerPreviewMesh(group) {
  ...
  // Preview materials clone maps from the shared VX-27 cache — never mat.dispose().
