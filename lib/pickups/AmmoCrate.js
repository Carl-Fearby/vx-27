...
import { updateEntityForFloorHole } from "../physics/Collision.js";
...
function ensureTextures() {
  if (!_frontTex) _frontTex = loadTex("/ui/crate/front.webp");
  if (!_topTex) _topTex = loadTex("/ui/crate/top.webp");
...
export function preloadAmmoCrateAssets() {
  ...
  _cratePreloadPromise = Promise.all([
    _loader.loadAsync("/ui/crate/front.webp"),
    _loader.loadAsync("/ui/crate/top.webp"),
...
function removeLegacyPickupShadowProxy(mesh) {
  ...
  if (!proxy) return;
  proxy.customDepthMaterial = undefined;
...
function syncPickupShadowCaster(mesh) {
  ...
  const castSun =
    mesh.userData?.pickupSunShadow !== false &&
    mesh.visible !== false &&
    mesh.parent != null &&
    caster.parent != null;
  caster.castShadow = castSun;
}

/** Track pickup surface (catwalk vs floor) for gameplay; sun shadows stay enabled. */
export function setPickupSurface(mesh, surface) {
  if (!mesh) return;
  mesh.userData.pickupSurface = surface;
  mesh.userData.pickupSunShadow = true;
...
/** Keep the invisible shadow box aligned after pickup reparenting or shadow refit. */
...
/** Re-apply shadow casters after spawn, GPU preload, or shadow refit. */
export function refreshLevelPickupShadows(pickupsRoot, meshes, levelRoot = null) {
  ...
  if (levelRoot) {
    levelRoot.traverse((obj) => {
      ...
      obj.receiveShadow = obj.userData.shadowReceive !== false;
    });
  }
  for (const mesh of meshes) {
    ...
    const caster = mesh.userData?.pickupShadowCaster;
    if (caster) {
      caster.castShadow = mesh.userData?.pickupSunShadow !== false;
      caster.visible = true;
    }
  }
...
export function disposeOrphanedPickupShadowCasters(root) {
  ...
  for (const caster of orphans) {
    caster.customDepthMaterial?.dispose?.();
    caster.customDepthMaterial = undefined;
    caster.geometry?.dispose();
    caster.parent?.remove(caster);
  }
}
...
const AMMO_DROP_COLLECT_RADIUS = 0.6;
const AMMO_STATIC_COLLECT_RADIUS = 0.6;
...
const LEVEL_COLLECT_RADIUS = 0.6;
...
export function canCollectLevelCollectible(
  ...
  return requireWasmMethod(d.gameCore, "resolvePickupCollect")({
    itemX: d.mesh.position.x,
    itemZ: d.mesh.position.z,
    playerX,
    playerZ,
    collectRadius: LEVEL_COLLECT_RADIUS,
    permanent: true,
    playerFootY,
    floorY: d.floorY,
    floorSlack: slack,
...
export function tickLevelCollectibleCollectFade(d, dt) {
  ...
  const fade = requireWasmMethod(d.gameCore, "resolveCollectFade")({
    time: d.time,
...
export function spawnAmmoDrop(scene, position, floorY, gameCore = null) {
  const mesh = new THREE.Mesh(getGeometry(), cloneOpaquePickupMaterials());
  mesh.scale.setScalar(AMMO_DROP_SCALE);
  ...
  for (let i = drops.length - 1; i >= 0; i--) {
    ...
    if (!d.collected) {
      ...
      const collect = requireWasmMethod(gameCore, "resolvePickupCollect")({
        ...
        playerZ: playerPos.z,
        collectRadius,
        permanent: Boolean(d.permanent),
        playerFootY: footY,
        floorY: d.floorY,
        floorSlack: AMMO_STATIC_FLOOR_SLACK,
      });
