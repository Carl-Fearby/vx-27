...
import { CONTROL_PANEL_SHELF_D_BRIGHTNESS } from "./ControlPanelShelfDTuning.js";
...
import { syncControlPanelProfileCornerStrips } from "./ControlPanelProfileCornerStrips.js";
...
/** Side-profile corner fillet (metres) — applied at every profile vertex. */
export const CONTROL_PANEL_PROFILE_CORNER_RADIUS_M = 0.06;

/** Max fillet as a fraction of the shorter adjacent edge (keeps tiny steps roundable). */
export const CONTROL_PANEL_PROFILE_CORNER_EDGE_FRACTION = 0.42;
...
export function profileCornerFilletRadius(pPrev, pCurr, pNext, maxR) {
  ...
  const edgeCap =
    Math.min(lenA, lenB) * CONTROL_PANEL_PROFILE_CORNER_EDGE_FRACTION;
  return Math.min(maxR, edgeCap);
}
...
/**
 * World-space samples along a profile corner fillet (matches extrude quadratic).
 * @param {ControlPanelProfilePoint[]} profile
 * @param {number} height
 * @param {number} depth
 ...
 * @returns {{ arc: { x: number, y: number }[], r: number } | null}
 */
export function getProfileCornerFilletArcWorld(
  profile,
  height,
  depth,
  vertexIndex,
  ...
  for (let s = 0; s <= segments; s += 1) {
    ...
    const mt = 1 - t;
    arc.push({
      x: mt * mt * start[0] + 2 * mt * t * pCurr[0] + t * t * end[0],
      y: mt * mt * start[1] + 2 * mt * t * pCurr[1] + t * t * end[1],
    });
  }
  return { arc, r };
}
...
function finishControlPanelGroup(group) {
  ...
  syncControlPanelProfileCornerStrips(group);
}
...
export function syncControlPanelScreenMaterials(groups) {
  ...
  updateControlPanelShelfDBrightness(CONTROL_PANEL_SHELF_D_BRIGHTNESS);
