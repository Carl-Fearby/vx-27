...
/** Foot Y for walking targets — arena-floor walkers ignore overhead catwalk deck slabs. */
export function sampleWalkerFootYAt(x, z, currentFootY, ctx) {
  const floorY = ctx.floorY ?? 0;
  const onElevatedDeck = currentFootY > floorY + 0.35;
  const groundSurfaces = (ctx.groundSurfaces ?? []).filter(
    (surf) =>
      onElevatedDeck || (!surf.arenaCatwalkDeck && !surf.roomCatwalkDeck),
  );
  const refY = onElevatedDeck ? currentFootY + 0.08 : floorY + 0.35;
  return sampleSupportYAt(x, z, refY, { ...ctx, groundSurfaces });
}
