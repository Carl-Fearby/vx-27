...
export function tryWallWeaponShopPurchase(ctx, offer, now) {
  ...
  const starting = getPrimaryWeaponStartingAmmo(weaponId);
  const stage = getWallShopStage(ctx, weaponId);
  const weaponAvailable =
    weaponId === "pistol" ? Boolean(ctx.primaryWeapons?.pistol) : true;
  const purchase =
    ctx.gameCore?.purchaseWallWeapon?.(
      score,
      stage,
      weaponAvailable,
      store?.rounds ?? 0,
      ...
      starting.rounds,
      starting.spare,
      offer.resupplyMagCount,
    ) ?? null;

  if (purchase ? !purchase.purchased : score < cost) {
    ctx.sounds?.playShopDenied?.();
    return false;
  }

  ctx.playerScoreRef.current = purchase?.playerScore ?? score - cost;
  ctx.gameCore?.syncPlayerScore?.(ctx.playerScoreRef.current);
  ctx.updateScoreHud(ctx.scoreHudRef.current, ctx.playerScoreRef.current);

  const firstUnlock = purchase?.firstUnlock ?? !canResupply;
  if (firstUnlock) {
    if (ctx.wallShopStageRef?.current) {
      ctx.wallShopStageRef.current[weaponId] = purchase?.stage ?? 1;
    }
    if (weaponId === "rifle" && (purchase?.weaponUnlocked ?? true)) {
      ctx.rifleUnlockedRef.current = true;
      ctx.setRifleUnlocked?.(true);
    }
  } else {
    if (ctx.wallShopStageRef?.current) {
      ctx.wallShopStageRef.current[weaponId] =
        purchase?.stage ?? Math.max(1, stage) + 1;
    }
  }

  if (purchase) {
    store.rounds = purchase.rounds;
    store.spare = purchase.spare;
    if (ctx.ammoPoolSnapshotRef?.current) {
      ctx.ammoPoolSnapshotRef.current[weaponId] = {
        rounds: store.rounds,
        spare: store.spare,
  ...
  if (ctx.activePrimaryId === weaponId && ctx.primaryWeapons?.[weaponId]) {
    ctx.loadActiveAmmo?.(weaponId);
  } else {
    scheduleEquipWallWeaponAfterPurchase(ctx, weaponId, now);
  }
