...
export function canWallWeaponResupply(ctx, offer) {
  return requireWasmMethod(ctx.gameCore, "canWallWeaponResupply")({
    weaponId: offer.weaponId,
    stage: getWallShopStage(ctx, offer.weaponId),
    pistolOwned: Boolean(ctx.primaryWeapons?.pistol),
  });
}
...
export function tryWallWeaponShopPurchase(ctx, offer, now) {
  ...
  const purchase = requireWasmMethod(ctx.gameCore, "purchaseWallWeapon")(
    score,
    stage,
    weaponAvailable,
    store?.rounds ?? 0,
  ...
  ctx.playerScoreRef.current = purchase.playerScore;
  ...
  const firstUnlock = purchase.firstUnlock;
  if (firstUnlock) {
    if (ctx.wallShopStageRef?.current) {
      ctx.wallShopStageRef.current[weaponId] = purchase.stage;
    }
    if (weaponId === "rifle" && purchase.weaponUnlocked) {
      ctx.rifleUnlockedRef.current = true;
      ctx.setRifleUnlocked?.(true);
    }
  } else {
  ...
  requireWasmMethod(ctx.gameCore, "syncWeaponAmmo")(
    weaponId,
    store.rounds,
    store.spare,
    cfg.magazineSize,
