...
import {
  ...
  collectProjectileNearbyColliders,
  resolveProjectileAgainstColliders,
  sampleColliderFloorTopAt,
} from "../physics/Collision.js";
import { sampleSupportYAt } from "../physics/GroundSupport.js";
import { hasLineOfSightToPoint } from "./LineOfSight.js";
import {
  computeThrowVelocity as computeThrowVelocityCore,
  GRENADE_RADIUS,
  projectileApplyGroundRoll,
  projectileFuseTick,
  projectileIntegrate,
  projectileResolveBounds,
  projectileResolveFloorLive,
  projectileSubstepCount,
  PROJECTILE_MAX_MOVE,
  PROJECTILE_MAX_SUBSTEPS,
} from "./ProjectilePhysics.js";
import {
  applyTraverseRewardExpireVisual,
  getRewardExpireVisuals,
  initTraversePickupMaterials,
...
const defaults = {
  ...
  /** Camera-eye fallback when no view weapon anchor is available (metres). */
  throwOriginRight: 0.22,
  throwOriginDown: 0.3,
  throwOriginForward: 0.4,
  /** Extra release offset from the active view-weapon holder (right hand). */
  throwReleaseRight: 0.18,
  throwReleaseDown: 0.05,
  throwReleaseForward: 0.24,
};
...
const _nearProjectileColliders = [];
const GRENADE_SEGMENTS = 12;
const SIM_STEP = 0.02;
const MAX_SIM_STEPS = 500;
const ARC_MAX_POINTS = 300;
const PREVIEW_UPDATE_INTERVAL_MS = 33;
const PREVIEW_AIM_EPS_SQ = 4e-6;
const PREVIEW_ORIGIN_EPS_SQ = 1e-4;

function previewMaxSimSteps() {
  return Math.min(MAX_SIM_STEPS, Math.ceil(_params.fuseTime / SIM_STEP) + 30);
}

/** @param {import("../physics/GroundSupport.js").GroundSupportContext | null | undefined} groundSupport */
function resolveEffectiveSupportY(x, z, refY, floorY, groundSupport, colliders) {
  const supportY = groundSupport
    ? sampleSupportYAt(x, z, refY, groundSupport)
    : floorY;
  const colliderFloor = colliders?.length
    ? sampleColliderFloorTopAt(x, z, refY, colliders, GRENADE_RADIUS)
    : null;
  return Math.max(supportY, colliderFloor ?? Number.NEGATIVE_INFINITY);
}
...
function ensureTex(anisotropy = 8) {
  if (_tex) return _tex;
  _tex = {
    body: loadTex("body_wrap_albedo.webp", true, anisotropy),
    top: loadTex("top_cap_albedo.webp", true, anisotropy),
    bot: loadTex("bottom_cap_albedo.webp", true, anisotropy),
  };
  return _tex;
}
...
export function preloadGrenadeAssets(anisotropy = 8) {
  ...
  _preloadPromise = Promise.all([
    _texLoader.loadAsync(TEX_PATH + "body_wrap_albedo.webp"),
    _texLoader.loadAsync(TEX_PATH + "top_cap_albedo.webp"),
    _texLoader.loadAsync(TEX_PATH + "bottom_cap_albedo.webp"),
  ])
    .then(([body, top, bot]) => {
      _tex = {
...
/**
 * Grenade projectile, explosion VFX, and trajectory preview — optional GPU preload helper.
 */
export async function warmupGrenadeThrow(renderer, scene, camera, compileOpts = {}, opts = {}) {
  ...
  if (floorY != null && colliders) {
    ...
    await compileInPlace(previewGroup);
...
function simulateTrajectory(
  ...
  const pos = out?.pos ?? origin.clone();
  const vel = out?.vel ?? velocity.clone();
  pos.copy(origin);
  vel.copy(velocity);
  let pointCount = 0;
  if (out) {
    out.pointCount = 0;
    out.hasLandPos = false;
    out.hasBouncePos = false;
    if (!out.positions) {
      if (out.points) out.points.length = 0;
      else out.points = [];
    }
  } else {
    out = { points: [] };
  ...
  const pushPoint = () => {
    if (!out.positions) {
      if (out.points.length < ARC_MAX_POINTS) out.points.push(pos.clone());
      return;
    }
    if (pointCount >= ARC_MAX_POINTS) return;
    const o = pointCount * 3;
    out.positions[o] = pos.x;
    out.positions[o + 1] = pos.y;
    out.positions[o + 2] = pos.z;
    if (pointCount === 0) {
      out.lineDistances[pointCount] = 0;
    } else {
      const dx = pos.x - out.lastX;
      const dy = pos.y - out.lastY;
      const dz = pos.z - out.lastZ;
      out.lineDistances[pointCount] =
        out.lineDistances[pointCount - 1] + Math.hypot(dx, dy, dz);
    }
    out.lastX = pos.x;
    out.lastY = pos.y;
    out.lastZ = pos.z;
  ...
  const physicsParams = {
    ...
    bounceRestitution: _params.bounceRestitution,
    bounceFriction: _params.bounceFriction,
  };
  const maxSteps = previewMaxSimSteps();
  const { gravity, bounceRestitution, bounceFriction } = physicsParams;
  const slideRetain = 1 - bounceFriction;

  for (let i = 0; i < maxSteps; i++) {
    // Euler integration — equivalent to projectile_integrate in Rust, no WASM call needed.
    previewState.pos.x += previewState.vel.x * SIM_STEP;
    previewState.pos.y += previewState.vel.y * SIM_STEP;
    previewState.pos.z += previewState.vel.z * SIM_STEP;
    previewState.vel.y -= gravity * SIM_STEP;
    pos.set(previewState.pos.x, previewState.pos.y, previewState.pos.z);
    vel.set(previewState.vel.x, previewState.vel.y, previewState.vel.z);

    let physicsColliders = colliders;
    if (colliders) {
      const travelMargin =
        Math.hypot(previewState.vel.x, previewState.vel.y, previewState.vel.z) * SIM_STEP +
        GRENADE_RADIUS * 2;
      // Pass null for gameCore → pure-JS brute-force collider search, no WASM call.
      collectProjectileNearbyColliders(
        previewState.pos.x,
        previewState.pos.y,
        previewState.pos.z,
        GRENADE_RADIUS,
        colliders,
        _nearProjectileColliders,
        0.35 + travelMargin,
        null,
      );
      physicsColliders = _nearProjectileColliders;
      // gameCore: null → pure-JS sphere-vs-box resolution, no WASM call.
      resolveProjectileAgainstColliders(pos, GRENADE_RADIUS, vel, physicsColliders, {
        restitution: bounceRestitution,
        friction: bounceFriction,
      });
      previewState.pos.x = pos.x;
      previewState.pos.y = pos.y;
      previewState.pos.z = pos.z;
      previewState.vel.x = vel.x;
      previewState.vel.y = vel.y;
      previewState.vel.z = vel.z;
    }

    const effectiveSupportY = resolveEffectiveSupportY(
      pos.x,
      pos.z,
      pos.y,
      floorY,
    ...
    const floorTop = effectiveSupportY + GRENADE_RADIUS;

    // Inline equivalent of projectile_preview_floor_and_bounds (Rust) — no WASM call.
    let previewLanded = false, previewRecordBounce = false, previewStopSim = false;
    if (pos.y <= floorTop) {
      pos.y = floorTop;
      previewState.pos.y = floorTop;
      if (bounceCount === 0) previewLanded = true;
      else if (bounceCount === 1) previewRecordBounce = true;
      bounceCount++;
      previewState.bounceCount = bounceCount;
      const newVelY = -previewState.vel.y * bounceRestitution;
      previewState.vel.y = newVelY;
      previewState.vel.x *= slideRetain;
      previewState.vel.z *= slideRetain;
      vel.y = newVelY;
      vel.x = previewState.vel.x;
      vel.z = previewState.vel.z;
      if (Math.abs(newVelY) < 0.1 && bounceCount > 1) previewStopSim = true;
    }
    if (bounds) {
      const r = GRENADE_RADIUS;
      if (pos.x < bounds.minX + r) { pos.x = bounds.minX + r; previewState.pos.x = pos.x; vel.x = Math.abs(vel.x) * bounceRestitution; previewState.vel.x = vel.x; }
      if (pos.x > bounds.maxX - r) { pos.x = bounds.maxX - r; previewState.pos.x = pos.x; vel.x = -Math.abs(vel.x) * bounceRestitution; previewState.vel.x = vel.x; }
      if (pos.z < bounds.minZ + r) { pos.z = bounds.minZ + r; previewState.pos.z = pos.z; vel.z = Math.abs(vel.z) * bounceRestitution; previewState.vel.z = vel.z; }
      if (pos.z > bounds.maxZ - r) { pos.z = bounds.maxZ - r; previewState.pos.z = pos.z; vel.z = -Math.abs(vel.z) * bounceRestitution; previewState.vel.z = vel.z; }
    }

    if (previewLanded && !landed) {
      landed = true;
      if (out?.firstLandPos) out.firstLandPos.copy(pos);
      else firstLandPos = pos.clone();
    } else if (previewRecordBounce) {
      if (out?.bouncePos) {
        out.bouncePos.copy(pos);
        out.hasBouncePos = true;
      } else {
        bouncePos = pos.clone();
      }
    }
  ...
  return { points: out.points, landPos, bouncePos };
}

/* ── Throw velocity ───────────────────────────────────────────────── */

function computeThrowVelocity(aimDir, gameCore = null) {
  return computeThrowVelocityCore(
    gameCore,
    aimDir,
    { throwSpeed: _params.throwSpeed, loftAngle: _params.loftAngle },
    new THREE.Vector3(),
  );
}

function computeThrowVelocityInto(aimDir, out, gameCore = null) {
  return computeThrowVelocityCore(
    gameCore,
    aimDir,
    { throwSpeed: _params.throwSpeed, loftAngle: _params.loftAngle },
...
const _throwOriginBase = new THREE.Vector3();
const _spawnOriginScratch = new THREE.Vector3();
const _throwCamRight = new THREE.Vector3();
const _throwCamUp = new THREE.Vector3();
const _throwCamForward = new THREE.Vector3();

function applyThrowOriginOffset(camera, base, right, down, forward, out) {
  _throwCamRight.set(1, 0, 0).applyQuaternion(camera.quaternion);
  _throwCamUp.set(0, 1, 0).applyQuaternion(camera.quaternion);
  _throwCamForward.set(0, 0, -1).applyQuaternion(camera.quaternion).normalize();
  return out
    .copy(base)
    .addScaledVector(_throwCamRight, right)
    .addScaledVector(_throwCamUp, -down)
    .addScaledVector(_throwCamForward, forward);
}

/**
 * World-space release point — right hand, offset from centre so the arc stays visible.
 * @param {THREE.Camera} camera
 * @param {THREE.Vector3} [out]
 * @param {{ holder?: THREE.Object3D } | null | undefined} [weapon]
 */
export function computeGrenadeThrowOrigin(camera, out = _throwOriginBase, weapon = null) {
  const holder = weapon?.holder;
  if (holder?.parent) {
    holder.updateMatrixWorld(true);
    return applyThrowOriginOffset(
      camera,
      holder.position,
      _params.throwReleaseRight,
      _params.throwReleaseDown,
      _params.throwReleaseForward,
      out
    );
  }
  return applyThrowOriginOffset(
    camera,
    camera.position,
    _params.throwOriginRight,
    _params.throwOriginDown,
    _params.throwOriginForward,
    out
  );
}
...
const _blastLosTarget = new THREE.Vector3();
const _previewAimDir = new THREE.Vector3();
const _previewOrigin = new THREE.Vector3();
const _previewVelocity = new THREE.Vector3();
const _previewThrowRight = new THREE.Vector3();
const _previewThrowUp = new THREE.Vector3();
const _previewWorldUp = new THREE.Vector3(0, 1, 0);
const _previewPositions = new Float32Array(ARC_MAX_POINTS * 3);
const _previewLineDistances = new Float32Array(ARC_MAX_POINTS);
const _previewSim = {
  pos: new THREE.Vector3(),
  vel: new THREE.Vector3(),
  firstLandPos: new THREE.Vector3(),
  landPos: new THREE.Vector3(),
  bouncePos: new THREE.Vector3(),
  positions: _previewPositions,
  lineDistances: _previewLineDistances,
  pointCount: 0,
  hasLandPos: false,
  hasBouncePos: false,
  lastX: 0,
...
/** @param {THREE.Vector3} explosionPos @param {THREE.Mesh} mesh @param {THREE.Object3D[]} [hitMeshes] */
function canBlastReachTarget(explosionPos, mesh, hitMeshes) {
  if (!hitMeshes?.length) return true;
  targetBlastAimPoint(mesh, _blastLosTarget);
  return hasLineOfSightToPoint(explosionPos, _blastLosTarget, hitMeshes, {
    blockEpsilon: 0.35,
  });
}

/** @param {THREE.Mesh} mesh @param {THREE.Vector3} out */
function targetBlastAimPoint(mesh, out) {
  out.copy(mesh.position);
  out.y += (mesh.userData.height ?? 2.2) * 0.55;
  return out;
}
let _previewBounceCircle = null;
let _previewGroup = null;
const _previewInputCache = {
  aimX: NaN,
  aimY: NaN,
  aimZ: NaN,
  ox: NaN,
...
function makePreviewArc(group) {
  const geo = new THREE.BufferGeometry();
  const positionAttribute = new THREE.BufferAttribute(_previewPositions, 3);
  const distanceAttribute = new THREE.BufferAttribute(_previewLineDistances, 1);
  ...
  const line = new THREE.Line(geo, _arcMat);
  line.frustumCulled = false;
  group.add(line);
  return line;
}

function makeCircle(color, opacity) {
...
function ensurePreviewCircle(group, key) {
  if (key === "land") {
    if (!_previewLandCircle) {
      _previewLandCircle = makeCircle(0x3399ff, 0.25);
      group.add(_previewLandCircle);
    }
    return _previewLandCircle;
  }
  if (!_previewBounceCircle) {
    _previewBounceCircle = makeCircle(0x66bbff, 0.15);
    group.add(_previewBounceCircle);
  }
  return _previewBounceCircle;
}

function disposePreviewObject(obj, disposeMaterial = true) {
  if (!obj) return;
  obj.parent?.remove(obj);
  obj.geometry?.dispose?.();
  if (disposeMaterial) obj.material?.dispose?.();
}
...
export function updateTrajectoryPreview(
  ...
  _previewAimDir.set(0, 0, -1).applyQuaternion(camera.quaternion).normalize();
  computeGrenadeThrowOrigin(camera, _previewOrigin, weapon);

  const now = performance.now();

  // Hard time-gate: never re-simulate faster than PREVIEW_UPDATE_INTERVAL_MS (~30 Hz).
  // Previously all four conditions were ANDed, so any player movement (dOrigin > eps)
  // broke the time-gate and caused simulateTrajectory to run every frame.
  if (_previewArc && now - _previewInputCache.atMs < PREVIEW_UPDATE_INTERVAL_MS) {
    return;
  }

  // Secondary skip: timer expired but nothing changed — no need to touch GPU buffers.
  const dAim =
    (_previewAimDir.x - _previewInputCache.aimX) ** 2 +
    (_previewAimDir.y - _previewInputCache.aimY) ** 2 +
    (_previewAimDir.z - _previewInputCache.aimZ) ** 2;
  const dOrigin =
    (_previewOrigin.x - _previewInputCache.ox) ** 2 +
    (_previewOrigin.y - _previewInputCache.oy) ** 2 +
    (_previewOrigin.z - _previewInputCache.oz) ** 2;
  if (_previewArc && dAim < PREVIEW_AIM_EPS_SQ && dOrigin < PREVIEW_ORIGIN_EPS_SQ) {
    _previewInputCache.atMs = now; // keep timer fresh so next real change triggers immediately
    return;
  }
  _previewInputCache.aimX = _previewAimDir.x;
  _previewInputCache.aimY = _previewAimDir.y;
  _previewInputCache.aimZ = _previewAimDir.z;
  _previewInputCache.ox = _previewOrigin.x;
  _previewInputCache.oy = _previewOrigin.y;
  _previewInputCache.oz = _previewOrigin.z;
  _previewInputCache.atMs = now;

  computeThrowVelocityInto(_previewAimDir, _previewVelocity, gameCore);
  ...
  if (!_previewArc) _previewArc = makePreviewArc(group);
  _previewArc.geometry.setDrawRange(0, sim.pointCount);
  _previewArc.geometry.attributes.position.needsUpdate = true;
  _previewArc.geometry.attributes.lineDistance.needsUpdate = true;

  const landCircle = ensurePreviewCircle(group, "land");
  landCircle.visible = sim.hasLandPos;
  if (sim.hasLandPos) {
    landCircle.position.set(sim.landPos.x, sim.landPos.y + 0.02, sim.landPos.z);
    landCircle.scale.setScalar(_params.blastRadius);
  }

  if (_previewBounceCircle) _previewBounceCircle.visible = false;
}

export function hideTrajectoryPreview() {
  _previewInputCache.atMs = 0;
  if (_previewGroup) {
    if (_previewArc) _previewArc.geometry.setDrawRange(0, 0);
    if (_previewLandCircle) _previewLandCircle.visible = false;
    if (_previewBounceCircle) _previewBounceCircle.visible = false;
  }
}
...
export function disposePreview() {
  hideTrajectoryPreview();
  disposePreviewObject(_previewArc, false);
  disposePreviewObject(_previewLandCircle);
  disposePreviewObject(_previewBounceCircle);
  _previewArc = null;
...
export function spawnGrenade(
  ...
  const origin = computeGrenadeThrowOrigin(camera, _spawnOriginScratch, weapon);
  const vel = computeThrowVelocity(aimDir, gameCore);
...
/** Scale VFX cost when several blasts overlap. */
function concurrentBlastParticleMul(activeExplosions, detonatingThisFrame) {
  const total = activeExplosions + detonatingThisFrame;
  if (total <= 1) return 1;
...
export function updateGrenades(grenades, dt, scene, getLiveTargets, applyHitFn, startDeathFn, deathOpts) {
  ...
  const deathGroundSupport = deathOpts?.groundSupport ?? null;
  let activeExplosions = 0;
  let detonatingThisFrame = 0;
  for (const g of grenades) {
    if (g.detonated && g.explosion) activeExplosions++;
    else if (!g.detonated && g.time + dt >= _params.fuseTime) detonatingThisFrame++;
  }
  const blastParticleMul = concurrentBlastParticleMul(
    activeExplosions,
    detonatingThisFrame,
  );

  const gameCore = deathOpts?.gameCore ?? null;
  const physicsParams = {
    bounceRestitution: _params.bounceRestitution,
    bounceFriction: _params.bounceFriction,
    groundRollFriction: _params.groundRollFriction ?? 16,
  };
  ...
  for (let i = grenades.length - 1; i >= 0; i--) {
    ...
    if (!g.groundSupport && deathGroundSupport) g.groundSupport = deathGroundSupport;

    if (!g.detonated) {
      const speed0 = Math.hypot(g.vel.x, g.vel.y, g.vel.z);
      const substeps = projectileSubstepCount(gameCore, speed0, dt);
      const subDt = dt / substeps;

      const px0 = g.mesh.position.x;
      const pz0 = g.mesh.position.z;
      const py0 = g.mesh.position.y;
      const groundSupport = g.groundSupport ?? deathGroundSupport;
      const travelMargin = speed0 * dt + GRENADE_RADIUS * 2;
      const nearColliders = g.colliders
        ? collectProjectileNearbyColliders(
            px0,
            py0,
            pz0,
            GRENADE_RADIUS,
            g.colliders,
            g._nearColliders ?? (g._nearColliders = []),
            0.35 + travelMargin,
            gameCore,
          )
        : null;
      const physicsColliders = nearColliders ?? g.colliders;
      const effectiveSupportY0 = resolveEffectiveSupportY(
        px0,
        pz0,
        py0,
        g.floorY,
      ...
      const overHole =
        effectiveSupportY0 <= g.floorY + 0.02 &&
        pointInFloorHole(px0, pz0, g.floorHoles, GRENADE_RADIUS, gameCore);
      if (overHole) beginHoleFall(g);

      for (let step = 0; step < substeps; step++) {
        const integrated = projectileIntegrate(
          gameCore,
          g.mesh.position,
          g.vel,
          subDt,
        ...
        g.mesh.position.set(integrated.pos.x, integrated.pos.y, integrated.pos.z);
        g.vel.set(integrated.vel.x, integrated.vel.y, integrated.vel.z);

        if (physicsColliders && !g.fallingThroughHole) {
          resolveProjectileAgainstColliders(
            g.mesh.position,
            GRENADE_RADIUS,
            g.vel,
            physicsColliders,
            {
              restitution: _params.bounceRestitution,
              friction: _params.bounceFriction,
              gameCore,
            },
          );
        }
        ...
        if (!g.fallingThroughHole) {
          ...
          const floorTop = effectiveSupportY + GRENADE_RADIUS;
          const floor = projectileResolveFloorLive(
            gameCore,
            g.mesh.position,
            g.vel,
            floorTop,
          ...
          g.mesh.position.set(floor.pos.x, floor.pos.y, floor.pos.z);
          g.vel.set(floor.vel.x, floor.vel.y, floor.vel.z);
          g.airborne = floor.airborne;
          if (floor.bounced) g.bounceCount++;
          if (floor.floorHit) {
            deathOpts?.onFloorHit?.(g.mesh.position.clone(), floor.floorHitImpact);
          }
        }
      }
      ...
      if (!g.fallingThroughHole && !g.airborne) {
        const roll = projectileApplyGroundRoll(
          gameCore,
          g.vel.x,
          g.vel.z,
          dt,
      ...
      if (g.bounds) {
        const resolved = projectileResolveBounds(
          ...
          g.vel,
          g.bounds,
          _params.bounceRestitution,
        );
        g.mesh.position.set(resolved.pos.x, resolved.pos.y, resolved.pos.z);
        g.vel.set(resolved.vel.x, resolved.vel.y, resolved.vel.z);
      }

      const clipDur = deathOpts?.countdownDuration ?? 0;
      const fuse = projectileFuseTick(gameCore, g, dt, _params.fuseTime, clipDur);
      g.time = fuse.time;
      g.countdownPlayed = fuse.countdownPlayed;
      if (fuse.shouldPlayCountdown) {
        deathOpts?.onCountdown?.(
          g.mesh.position.clone(),
          fuse.countdownPlaybackRate,
        );
      }

      if (fuse.shouldDetonate) {
        g.detonated = true;
        g.justDetonated = true;
        g.explosionElapsed = 0;
        g.explosionPos = g.mesh.position.clone();
        const isFlashbang = g.type === PROJECTILE_FLASHBANG;
        deathOpts?.onExplode?.(g.explosionPos.clone(), isFlashbang);
        ...
        if (isFlashbang) {
          ...
          const hitMeshes = deathOpts?.hitMeshes ?? null;
          for (const mesh of targets) {
            const dist = mesh.position.distanceTo(g.explosionPos);
            if (dist >= blindRadius) continue;
            if (!canBlastReachTarget(g.explosionPos, mesh, hitMeshes)) continue;
          ...
          const hitMeshes = deathOpts?.hitMeshes ?? null;
          const blastRadiusSq = _params.blastRadius * _params.blastRadius;
          const blastDeathClips = Math.random() < 0.5 ? [0, 1] : [1, 0];
          let blastVictimIndex = 0;
          // Area damage
          for (const mesh of targets) {
            const dx = mesh.position.x - g.explosionPos.x;
            const dy = mesh.position.y - g.explosionPos.y;
            const dz = mesh.position.z - g.explosionPos.z;
            const distSq = dx * dx + dy * dy + dz * dz;
            if (distSq >= blastRadiusSq) continue;
            const dist = Math.sqrt(distSq);
            if (!canBlastReachTarget(g.explosionPos, mesh, hitMeshes)) continue;
            const blast = deathOpts.calculateGrenadeBlastHit(
              dist,
              _params.blastRadius,
              _params.maxDamage,
              _params.falloffPower,
            );
            if (!blast.hit) continue;
            const fakeDir = mesh.position.clone().sub(g.explosionPos).normalize();
            const result = applyHitFn(mesh, g.explosionPos, fakeDir, blast.damage);
            if (result?.killed) {
              startDeathFn(mesh, fakeDir, {
                ...deathOpts,
                hitZone: "grenade",
                knockbackMul: blast.knockbackMul,
                blastFalloff: blast.falloff,
                blastOrigin: g.explosionPos,
                blastDeathClips,
                blastVictimIndex: blastVictimIndex++,
              });
            }
          }
          ...
          g.explosion = createExplosion(scene, g.explosionPos, {
            onGround: !g.airborne,
            particleCountMul: blastParticleMul,
          });
        }
      ...
      // Update explosion
      if (g.explosion) {
        g.explosionElapsed = (g.explosionElapsed ?? 0) + dt;
        const alive = updateExplosion(
          g.explosion,
          g.explosionElapsed,
          deathOpts?.viewerPos,
        );
...
function explosionParticleCount(count, mul = 1) {
  return Math.max(0, Math.round(count * mul));
}
...
function updateExplosionLight(expl, vfx, elapsed) {
  ...
  const intensity = flashIntensity + sustainIntensity;
  expl.light.intensity = intensity;
  expl.light.visible = intensity > 0.001;
...
function resolveExplosionVfx(opts = {}) {
  ...
  const mul = opts.particleCountMul ?? 1;
  if (mul >= 0.999) return base;
  return {
    ...base,
    sparkCount: explosionParticleCount(base.sparkCount ?? SPARK_COUNT_DEFAULT, mul),
    emberCount: explosionParticleCount(base.emberCount ?? EMBER_COUNT_DEFAULT, mul),
    debrisCount: explosionParticleCount(base.debrisCount ?? DEBRIS_COUNT_DEFAULT, mul),
  };
}
...
function resetExplosionForReuse(expl) {
  ...
  if (expl.light) {
    expl.light.intensity = 0;
    expl.light.visible = true;
  }
...
function buildExplosion(scene, pos, opts = {}, vfx = resolveExplosionVfx(opts)) {
  ...
  if (vfx.embers) {
    emberLayers = createEmberLayers(
      explosionParticleCount(vfx.emberCount ?? EMBER_COUNT_DEFAULT),
      1,
      2.2,
      6.5,
      vfx.emberSize,
  ...
  if (vfx.sparks) {
    sparks = createParticlePoints(
      initParticleCloud(
        explosionParticleCount(vfx.sparkCount ?? SPARK_COUNT_DEFAULT),
        1,
        4.5,
        11,
        sparkPalette,
  ...
  if (vfx.debris) {
    debris = createParticlePoints(
      initParticleCloud(
        explosionParticleCount(vfx.debrisCount ?? DEBRIS_COUNT_DEFAULT),
        1,
        2,
        6.5,
        debrisPalette,
...
let _shakeTime = 0;
let _shakeDuration = 0.35;
let _shakeDecay = 4;
let _shakeFreqX = 60;
let _shakeFreqY = 45;
const SHAKE_DURATION = 0.35;
const SHAKE_DECAY = 4;
/** Player hurt — snappier than explosion shake, stacks with vignette flash. */
const HURT_SHAKE_INTENSITY = 0.022;
const HURT_SHAKE_DURATION = 0.38;
const HURT_SHAKE_DECAY = 5.5;
...
export function triggerScreenShake(playerPos, explosionPos) {
  ...
  _shakeTime = 0;
  _shakeDuration = SHAKE_DURATION;
  _shakeDecay = SHAKE_DECAY;
  _shakeFreqX = 60;
  _shakeFreqY = 45;
}

/** Standard hurt feedback shake (fire, grenades, etc.). */
export function triggerHurtScreenShake() {
  _shakeIntensity = Math.max(_shakeIntensity, HURT_SHAKE_INTENSITY);
  _shakeTime = 0;
  _shakeDuration = HURT_SHAKE_DURATION;
  _shakeDecay = HURT_SHAKE_DECAY;
  _shakeFreqX = 78;
  _shakeFreqY = 62;
}
...
export function applyScreenShake(camera, dt) {
  ...
  _shakeTime += dt;
  const decay = Math.exp(-_shakeDecay * _shakeTime);
  const amp = _shakeIntensity * decay;
  camera.position.x += Math.sin(_shakeTime * _shakeFreqX) * amp;
  camera.position.y += Math.cos(_shakeTime * _shakeFreqY) * amp * 0.75;
  camera.position.z +=
    Math.sin(_shakeTime * _shakeFreqX * 0.85 + 1.2) * amp * 0.45;
...
const GREN_DROP_COLLECT_RADIUS = 0.6;
...
function enablePickupMeshShadows(root) {
  root.traverse((child) => {
...
export function spawnGrenadeDrop(scene, position, floorY, gameCore = null) {
  const mesh = cloneGrenadeModel();
  mesh.scale.setScalar(2.0);
  mesh.rotation.x = Math.PI / 2;
  enablePickupMeshShadows(mesh);
  const spawnY = Math.max(position.y, floorY + 0.5);
  mesh.position.set(position.x, spawnY, position.z);
  mesh.visible = true;
  scene.add(mesh);
  const { flashMats, flashBase } = initTraversePickupMaterials(mesh, {
    emissiveIntensity: 0,
  });
...
export function updateGrenadeDrops(drops, dt, playerPos, onCollect, colliders, bounds, floorHoles = [], gameCore = null) {
  for (let i = drops.length - 1; i >= 0; i--) {
    ...
    if (!d.collected) {
      const collect = requireWasmMethod(gameCore, "resolvePickupCollect")({
        itemX: d.mesh.position.x,
        itemZ: d.mesh.position.z,
        playerX: playerPos.x,
        playerZ: playerPos.z,
        collectRadius: GREN_DROP_COLLECT_RADIUS,
      });
    ...
    if (d.collected) {
      ...
      if (fade.remove) remove = true;
    } else {
      const vis = getRewardExpireVisuals(d.time, GREN_DROP_LIFETIME);
      applyTraverseRewardExpireVisual(d.mesh, vis, d);
