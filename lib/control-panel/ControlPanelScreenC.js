...
const SCREEN_EMISSIVE_COLOR = 0x3db8ff;
...
export function updateControlPanelScreenBrightness(brightness) {
  applyControlPanelScreenBrightness(_screenMat, { brightness });
}
...
export function preloadControlPanelScreenCTextures() {
  ...
  _preloadPromise = (async () => {
    try {
      ...
      _screenMat.polygonOffset = true;
      _screenMat.polygonOffsetFactor = -2;
      _screenMat.polygonOffsetUnits = -2;
...
export function attachControlPanelScreenC(group) {
  ...
  const mesh = new THREE.Mesh(geo, _screenMat);
