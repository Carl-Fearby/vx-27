...
/** Stay below oil-barrel flame meshes (10+) and viewmodel overlays. */
...
/**
 * Brief coloured point-light flash at impact — same hue as the muzzle flash.
 * @param {THREE.Mesh} surfaceMesh
 * @param {THREE.Vector3} hitPoint
 * @param {THREE.Vector3} worldNormal
...
/**
 * Light GPU warm for impact flashes + one bullet hole (first wall hit).
 * @param {THREE.Object3D | null | undefined} levelGroup
 ...
 * }} [opts]
 */
export async function warmupBulletImpactGpu(levelGroup, camera, opts = {}) {
  if (!levelGroup || !camera) return;
  ...
  levelGroup.traverse((child) => {
    if (child.isMesh && !isBulletPassthroughMesh(child)) meshes.push(child);
  });
  const mesh = pickWarmupBulletSurfaces(meshes, 1)[0];
  if (!mesh) return;
  ...
  spawnBulletHole(mesh, point, null, _warmupBulletDir);

  const dt = FLASH_DURATION / Math.max(2, frames);
