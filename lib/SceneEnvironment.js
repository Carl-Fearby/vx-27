...
/**
 * @param {THREE.Scene} scene
 * @param {Map<THREE.Object3D, boolean>} saved Reused map — cleared each call.
 */
function hideWorldMeshesExceptSky(scene, saved) {
  saved.clear();
  scene.traverse((obj) => {
    if (!(obj.isMesh || obj.isSprite)) return;
    if (!obj.layers.isEnabled(WORLD_LAYER)) return;
...
function hideSkyMeshes(scene, saved) {
  saved.clear();
  scene.traverse((obj) => {
    if (!(obj.isMesh || obj.isSprite)) return;
    if (!obj.userData.isSkyDome) return;
...
const _hiddenWorldSaved = new Map();
const _hiddenSkySaved = new Map();
 ...
 * block outdoor lights from room meshes. Sky billboards use depthTest:false, so
 * they must render in a dedicated pass before world geometry composites on top
 * (catwalk ceilings, room shells) — otherwise sun disc and lens flares bleed
 * through solid geometry. Room interiors use a third pass on ROOM_INTERIOR_LAYER
 * (PCF only — no mid-frame VSM toggle on r182+).
 *
 * @param {THREE.WebGLRenderer} renderer
 * @param {THREE.Scene} scene
...
export function renderSceneWithLayeredLighting(renderer, scene, camera, options = {}) {
  ...
  try {
    ...
    if (hasSky) {
      const hiddenWorld = hideWorldMeshesExceptSky(scene, _hiddenWorldSaved);
      try {
      ...
      const hiddenSky = hideSkyMeshes(scene, _hiddenSkySaved);
      try {
    ...
    if (!skipRoomPass) {
      // Sun/moon layers do not exclude their shadow maps from the room pass —
      // disable outdoor shadow casting so catwalk pickup shadows do not bleed
      // onto interior walls/ceilings.
