 ...
 *   shop_denied.aac — wall weapon shop purchase rejected (insufficient credits)
 *   shop_purchase.aac — wall weapon shop successful purchase
 *   rain_thunderstorm_ambient.aac — looping rain/thunder bed while weather rain is active
 */
const CLIP_SOURCES = {
...
/** Rain ambient — volume tracks weather rain fade (attack/release slightly slower than particles). */
...
export function createSoundManager(camera) {
  ...
  const ENEMY_VOCAL_REF_DISTANCE = 3.5;
  const ENEMY_VOCAL_MAX_DISTANCE = 42;
  const ENEMY_VOCAL_ROLLOFF = 1.35;
   ...
   * @param {{ volume?: number, refDistance?: number, maxDistance?: number, rolloffFactor?: number, playbackRate?: number, onEnded?: () => void }} [opts]
   * @returns {{ holder: THREE.Object3D, audio: THREE.PositionalAudio } | null}
   */
  function playWorldBuffer(buffer, scene, position, opts = {}) {
    resume();
    if (!buffer || !scene || !position) return null;
    ...
    if (finalVolume < 0.02) return null;
  ...
  function playEnemyDeath(
    ...
    playWorldBuffer(deathVocalBuffers[index], scene, position, {
      ...
      refDistance: ENEMY_VOCAL_REF_DISTANCE,
      maxDistance: ENEMY_VOCAL_MAX_DISTANCE,
      rolloffFactor: ENEMY_VOCAL_ROLLOFF,
    });
  }
  ...
  function stopEnemyHitVoice() {
    ...
    enemyHitVoice.holder?.removeFromParent();
    enemyHitVoice = { holder: null, audio: null, pending: null, playing: false };
  }

  function playEnemyHitVoiceNow(opts = {}) {
    const { volume = 0.88, headshot = false, scene, position } = opts;
    if (!scene?.isScene || !position) {
    ...
    const spawned = playWorldBuffer(buffer, scene, position, {
      volume: volume * (headshot ? 1.05 : 1),
      ...
      onEnded: () => {
        enemyHitVoice.playing = false;
        enemyHitVoice.holder = null;
        enemyHitVoice.audio = null;
    ...
    enemyHitVoice.holder = spawned.holder;
    enemyHitVoice.audio = spawned.audio;
    return true;
  }
  ...
  function playEnemyHit(
    ...
    resume();
    if (!scene?.isScene || !position) return;

    listener.getWorldPosition(_listenerPos);
    const dist = _listenerPos.distanceTo(position);
    if (
      distanceVolumeScale(dist, {
        refDistance: ENEMY_VOCAL_REF_DISTANCE,
        maxDistance: ENEMY_VOCAL_MAX_DISTANCE,
      }) < 0.02
    ) {
    ...
    const opts = { volume, headshot, scene, position };
  ...
  function playShopDenied({ volume = 0.78 } = {}) {
    play("shop_denied", { volume });
  }

  function playShopPurchase({ volume = 0.82 } = {}) {
    play("shop_purchase", { volume });
  }
  ...
  /**
   * Looping thunderstorm bed — fades with {@link weatherTransition.rainFade}.
   * @param {number} rainFade 0–1
   * @param {number} [nowMs] performance.now()
   */
  function updateRainAmbient(rainFade, nowMs = performance.now()) {
    ...
    const target =
      Math.min(1, Math.max(0, rainFade)) * RAIN_AMBIENT_MAX_VOLUME;
    ...
    const buffer = buffers.get("rain_thunderstorm_ambient");
    if (!buffer) return;

    resume();
    if (!rainAmbientAudio) {
      rainAmbientAudio = new THREE.Audio(listener);
      rainAmbientAudio.setBuffer(buffer);
      rainAmbientAudio.setLoop(true);
    }

    if (!rainAmbientAudio.isPlaying) {
      rainAmbientAudio.setVolume(0);
      rainAmbientAudio.play();
    }
    rainAmbientAudio.setVolume(rainAmbientGainSmooth);
  }
