...
import {
  ...
  projectileFuseTick,
  projectileIntegrate,
  projectilePreviewFloorAndBounds,
...
function simulateTrajectory(
  ...
  // Preview arc avoids WASM per substep — full collider arrays were serialized
  // hundreds of times per frame while holding G.
  const simCore = null;
  ...
  for (let i = 0; i < maxSteps; i++) {
    ...
    previewState.pos = integrated.pos;
    previewState.vel = integrated.vel;
    ...
    const preview = projectilePreviewFloorAndBounds(
      simCore,
      previewState,
      floorTop,
      bounds,
    ...
    previewState.bounceCount = preview.bounceCount;
    pos.set(previewState.pos.x, previewState.pos.y, previewState.pos.z);
    vel.set(previewState.vel.x, previewState.vel.y, previewState.vel.z);
    bounceCount = previewState.bounceCount;

    if (preview.landed && !landed) {
      landed = true;
      if (out?.firstLandPos) out.firstLandPos.copy(pos);
      else firstLandPos = pos.clone();
    } else if (preview.recordBounce) {
      if (out?.bouncePos) {
...
export function updateTrajectoryPreview(
  ...
  if (
    _previewArc &&
    now - _previewInputCache.atMs < PREVIEW_UPDATE_INTERVAL_MS &&
    dAim < PREVIEW_AIM_EPS_SQ &&
    dOrigin < PREVIEW_ORIGIN_EPS_SQ
  ) {
    return;
  }
        ...
        if (isFlashbang) {
          ...
          for (const mesh of targets) {
            ...
            const blast = deathOpts?.calculateGrenadeBlastHit?.(
              ...
              hit: true,
              falloff: 1 - Math.pow(dist / _params.blastRadius, _params.falloffPower),
              damage: _params.maxDamage * (1 - Math.pow(dist / _params.blastRadius, _params.falloffPower)),
              knockbackMul: THREE.MathUtils.lerp(
                0.85,
                1.35,
                1 - Math.pow(dist / _params.blastRadius, _params.falloffPower),
              ),
            };
